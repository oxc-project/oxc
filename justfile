...
# Clone or update submodules in parallel
# Usage: just submodules [test262] [babel] [typescript] [prettier] [acorn-test262]
# Arguments: "true" or "false" for each submodule (defaults to "true")
submodules test262="true" babel="true" typescript="true" prettier="true" acorn_test262="true":
  #!/usr/bin/env bash
  set -euo pipefail
  
  # Call the parallel clone script
  .github/actions/clone-submodules/clone-parallel.sh "{{test262}}" "{{babel}}" "{{typescript}}" "{{prettier}}" "{{acorn_test262}}"
  
  # Only update transformer fixtures if babel was cloned
  if [[ "{{babel}}" == "true" ]] && [[ -d "tasks/coverage/babel" ]]; then
