...
if (!nativeBinding || forceWasi) {
  ...
  if (!wasiBindingLoaded && (!__napiWasiFlavorRequested || __napiWasiFlavor === 'wasm32-wasip1')) {
    ...
    let candidateFailed = false
    try {
      candidateError = __napiWasiResolveCandidate('./transform-relay.wasip1.cjs', false, ['./transform-relay.wasm32-wasip1.debug.wasm', './transform-relay.wasm32-wasip1.wasm'])
      candidateFailed = candidateError !== null
      if (!candidateFailed) {
        wasiBinding = require('./transform-relay.wasip1.cjs')
        nativeBinding = wasiBinding
  ...
  if (!wasiBindingLoaded && (!__napiWasiFlavorRequested || __napiWasiFlavor === 'wasm32-wasip1')) {
    ...
    let candidateFailed = false
    try {
      candidateError = __napiWasiResolveCandidate('@oxc-transform-relay/binding-wasm32-wasip1', true, undefined)
      candidateFailed = candidateError !== null
      if (!candidateFailed) {
        if (process.env.NAPI_RS_ENFORCE_VERSION_CHECK && process.env.NAPI_RS_ENFORCE_VERSION_CHECK !== '0') {
          const bindingPackageVersion = require('@oxc-transform-relay/binding-wasm32-wasip1/package.json').version
          if (bindingPackageVersion !== '0.151.0') {
            throw new Error(`WASI binding package version mismatch, expected 0.151.0 but got ${bindingPackageVersion}. You can reinstall dependencies to fix this issue.`)
          }
        }
        wasiBinding = require('@oxc-transform-relay/binding-wasm32-wasip1')
        nativeBinding = wasiBinding
        __napiLoadedBindingTarget = 'wasm32-wasip1'
        wasiBindingLoaded = true
      }
