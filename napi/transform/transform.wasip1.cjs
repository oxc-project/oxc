...
let __wasmFilePath = __nodePath.join(__dirname, 'transform.wasm32-wasip1.wasm')
const __wasmDebugFilePath = __nodePath.join(__dirname, 'transform.wasm32-wasip1.debug.wasm')

if (__nodeFs.existsSync(__wasmDebugFilePath)) {
  __wasmFilePath = __wasmDebugFilePath
} else if (!__nodeFs.existsSync(__wasmFilePath)) {
  const __wasiPackageEntry = require.resolve('@oxc-transform/binding-wasm32-wasip1')
  ...
  if (!__nodeFs.existsSync(__packagedWasmFilePath)) {
    throw new Error(
      '@oxc-transform/binding-wasm32-wasip1 is installed but is missing transform.wasm32-wasip1.wasm.',
    )
  }
