...
if (!nativeBinding || forceWasi) {
  ...
  if (!wasiBindingLoaded && (!__napiWasiFlavorRequested || __napiWasiFlavor === "wasm32-wasip1")) {
    ...
    let candidateFailed = false
    try {
      candidateError = __napiWasiResolveCandidate('./minify.wasip1.cjs', false, ["./minify.wasm32-wasip1.debug.wasm","./minify.wasm32-wasip1.wasm"])
      candidateFailed = candidateError !== null
      if (!candidateFailed) {
        wasiBinding = require('./minify.wasip1.cjs')
        nativeBinding = wasiBinding
        wasiBindingLoaded = true
      }
    } catch (err) {
  ...
  if (!wasiBindingLoaded && (!__napiWasiFlavorRequested || __napiWasiFlavor === "wasm32-wasip1")) {
    ...
    let candidateFailed = false
    try {
      candidateError = __napiWasiResolveCandidate('@oxc-minify/binding-wasm32-wasip1', true, undefined)
      candidateFailed = candidateError !== null
      if (!candidateFailed) {
        if (process.env.NAPI_RS_ENFORCE_VERSION_CHECK && process.env.NAPI_RS_ENFORCE_VERSION_CHECK !== '0') {
          const bindingPackageVersion = require('@oxc-minify/binding-wasm32-wasip1/package.json').version
        ...
        wasiBinding = require('@oxc-minify/binding-wasm32-wasip1')
        nativeBinding = wasiBinding
        wasiBindingLoaded = true
      }
    } catch (err) {
