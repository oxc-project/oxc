...
back under the same options for other unminified input. Property mappings are single-application:
do not minify already-mangled output with the same cache. Custom target names may be shared
deliberately, but must be valid JavaScript `IdentifierName` values and cannot be `__proto__`,
