...
import generatePrimitiveDeserializerCallGenerator from './primitives.mjs';
import {
...
for (const type of Object.values(typesByName)) {
    if (type.kind === 'struct') {
        ...
        generatePrimitiveDeserializerCallGenerator(type);
        if (type.name === 'Bool') {
            type.niche = {offset: 0, size: 1, min: 2, max: 255};
...
function generateDeserializer(type) {
    ...
    const generator = generators[type.kind];
    if (generator) code += generator(type) + '\n\n';
