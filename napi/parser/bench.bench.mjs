...
const { getCompiledVisitor } = require('./raw-transfer/visitor.js');
...
for (const { filename, code } of fixtures) {
  describe(filename, () => {
    ...
    const compiledDebuggerVisitor = getCompiledVisitor(debuggerVisitor);
    const compiledIdentVisitor = getCompiledVisitor(identVisitor);
    ...
    benchRaw('parser_napi_raw_lazy_visit_only(debugger)', () => {
      ast.nodes = new Map();
      debuggerCount = 0;
      walkProgram(programPos, ast, compiledDebuggerVisitor);
    });

    benchRaw('parser_napi_raw_lazy_visit_only(ident)', () => {
      ast.nodes = new Map();
      identCount = 0;
      walkProgram(programPos, ast, compiledIdentVisitor);
    });
  });
}
