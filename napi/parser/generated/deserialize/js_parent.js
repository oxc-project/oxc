...
function deserializeProgram(pos) {
	let start = deserializeU32(pos), end = deserializeU32(pos + 4), program = parent = {
		type: "Program",
		body: null,
...
function deserializeTemplateElement(pos) {
	let tail = deserializeBool(pos + 40), start = deserializeU32(pos), end = deserializeU32(pos + 4), value = deserializeTemplateElementValue(pos + 8);
...
function deserializeAssignmentTargetPropertyIdentifier(pos) {
	let start = deserializeU32(pos), end = deserializeU32(pos + 4), previousParent = parent, node = parent = {
		...
		parent
	}, key = deserializeIdentifierReference(pos + 8), value = {
		type: "Identifier",
		name: key.name,
		start: key.start,
		end: key.end,
...
function deserializeFormalParameters(pos) {
	...
	if (uint32[pos + 32 >> 2] !== 0 && uint32[pos + 36 >> 2] !== 0) {
		pos = uint32[pos + 32 >> 2];
		let previousParent = parent, rest = parent = {
			type: "RestElement",
			argument: null,
			start: deserializeU32(pos),
			end: deserializeU32(pos + 4),
