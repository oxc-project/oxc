...
const textDecoder = new TextDecoder("utf-8", { ignoreBOM: true }), decodeStr = textDecoder.decode.bind(textDecoder), { fromCodePoint } = String;
...
function deserializeProgram(pos) {
	let end = deserializeU32(pos + 4), program = {
		type: "Program",
		body: null,
...
function deserializeIdentifierName(pos) {
	let start = deserializeU32(pos), end = deserializeU32(pos + 4), node = {
		type: "Identifier",
		decorators: null,
...
function deserializeTemplateElement(pos) {
	...
	value.cooked !== null && deserializeBool(pos + 41) && (value.cooked = value.cooked.replace(/\uFFFD(.{4})/g, (_, hex) => String.fromCodePoint(parseInt(hex, 16))));
	return {
...
function deserializeFormalParameter(pos) {
	let param;
	{
		let accessibility = deserializeOptionTSAccessibility(pos + 64), readonly = deserializeBool(pos + 65), override = deserializeBool(pos + 66);
...
function deserializeArrowFunctionExpression(pos) {
	let expression = deserializeBool(pos + 44), start = deserializeU32(pos), end = deserializeU32(pos + 4), node = {
...
function deserializeBooleanLiteral(pos) {
	let value = deserializeBool(pos + 8), start = deserializeU32(pos), end = deserializeU32(pos + 4), node = {
		type: "Literal",
		value,
		raw: null,
		start,
...
function deserializeStringLiteral(pos) {
	...
	deserializeBool(pos + 40) && (value = value.replace(/\uFFFD(.{4})/g, (_, hex) => String.fromCodePoint(parseInt(hex, 16))));
	node.value = value;
	return node;
}
...
function deserializeTSModuleDeclaration(pos) {
	let kind = deserializeTSModuleDeclarationKind(pos + 84), start = deserializeU32(pos), end = deserializeU32(pos + 4), declare = deserializeBool(pos + 85), node, body = deserializeOptionTSModuleDeclarationBody(pos + 64);
	if (body === null) {
		node = {
...
function deserializeTSMappedType(pos) {
	let start = deserializeU32(pos), end = deserializeU32(pos + 4), node = {
		...
		range: [start, end]
	}, typeParameter = deserializeBoxTSTypeParameter(pos + 8), key = typeParameter.name, { constraint } = typeParameter, optional = deserializeOptionTSMappedTypeModifierOperator(pos + 52);
...
function deserializeImportImportName(pos) {
	switch (uint8[pos]) {
		...
		default: throw Error(`Unexpected discriminant ${uint8[pos]} for ImportImportName`);
	}
}
...
function deserializeExportImportName(pos) {
	switch (uint8[pos]) {
		...
		default: throw Error(`Unexpected discriminant ${uint8[pos]} for ExportImportName`);
	}
}
...
function deserializeExportExportName(pos) {
	switch (uint8[pos]) {
		...
		default: throw Error(`Unexpected discriminant ${uint8[pos]} for ExportExportName`);
	}
}
...
function deserializeExportLocalName(pos) {
	switch (uint8[pos]) {
		...
		default: throw Error(`Unexpected discriminant ${uint8[pos]} for ExportLocalName`);
	}
}
...
function deserializeErrorSeverity(pos) {
	switch (uint8[pos]) {
		...
		default: throw Error(`Unexpected discriminant ${uint8[pos]} for ErrorSeverity`);
	}
}
