...
let __wasmFilePath = __nodePath.join(__dirname, 'parser.wasm32-wasip1.wasm')
const __wasmDebugFilePath = __nodePath.join(__dirname, 'parser.wasm32-wasip1.debug.wasm')

if (__nodeFs.existsSync(__wasmDebugFilePath)) {
  __wasmFilePath = __wasmDebugFilePath
} else if (!__nodeFs.existsSync(__wasmFilePath)) {
  const __wasiPackageEntry = require.resolve('@oxc-parser/binding-wasm32-wasip1')
  ...
  if (!__nodeFs.existsSync(__packagedWasmFilePath)) {
    throw new Error(
      '@oxc-parser/binding-wasm32-wasip1 is installed but is missing parser.wasm32-wasip1.wasm.',
    )
  }
