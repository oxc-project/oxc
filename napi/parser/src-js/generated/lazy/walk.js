...
function walkAstroScript(pos, ast, visitors) {
  ...
  if (enterExit !== null) {
    ...
    node = new AstroScript(pos, ast);
    if (enter !== null) enter(node);
  }

  walkProgram(pos + 8, ast, visitors);

  if (exit !== null) exit(node);
}

function walkAstroDoctype(pos, ast, visitors) {
  const visit = visitors[0];
  if (visit !== null) visit(new AstroDoctype(pos, ast));
}
...
function walkJSXChild(pos, ast, visitors) {
  switch (ast.buffer[pos]) {
    ...
    case 4:
      ...
      return;
    case 5:
      walkBoxAstroScript(pos + 8, ast, visitors);
      return;
    case 6:
      walkBoxAstroDoctype(pos + 8, ast, visitors);
      return;
    default:
...
function walkBoxAstroScript(pos, ast, visitors) {
  return walkAstroScript(ast.buffer.uint32[pos >> 2], ast, visitors);
}

function walkBoxAstroDoctype(pos, ast, visitors) {
  return walkAstroDoctype(ast.buffer.uint32[pos >> 2], ast, visitors);
}
