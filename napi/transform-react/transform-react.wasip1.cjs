...
let __wasmFilePath = __nodePath.join(__dirname, 'transform-react.wasm32-wasip1.wasm')
const __wasmDebugFilePath = __nodePath.join(__dirname, 'transform-react.wasm32-wasip1.debug.wasm')

if (__nodeFs.existsSync(__wasmDebugFilePath)) {
  __wasmFilePath = __wasmDebugFilePath
} else if (!__nodeFs.existsSync(__wasmFilePath)) {
  const __wasiPackageEntry = require.resolve('@oxc-transform-react/binding-wasm32-wasip1')
  const __packagedWasmFilePath = __nodePath.join(
    __nodePath.dirname(__wasiPackageEntry),
    'transform-react.wasm32-wasip1.wasm',
  )
  if (!__nodeFs.existsSync(__packagedWasmFilePath)) {
    throw new Error(
      '@oxc-transform-react/binding-wasm32-wasip1 is installed but is missing transform-react.wasm32-wasip1.wasm.',
    )
  }
