...
use super::{
    ...
    branchless_utils::scan_string_content,
};
...
/// Optimized macro to handle a string literal using branchless scanning.
///
/// # SAFETY
/// `$delimiter` must be an ASCII byte.
/// Next char in `lexer.source` must be ASCII.
macro_rules! handle_string_literal_optimized {
    ($lexer:ident, $delimiter:literal, $table:ident, $escaped_table:ident) => {{
        ...
        // Use branchless scanning to find string content end
        let content_len = scan_string_content(remaining, $delimiter);
...
impl<'a> Lexer<'a> {
    ...
    /// Uses branchless optimizations for better performance.
    /// # SAFETY
    ...
    pub(super) unsafe fn read_string_literal_double_quote(&mut self) -> Kind {
        ...
        unsafe {
            handle_string_literal_optimized!(
                self,
                b'"',
