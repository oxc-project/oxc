...
impl<C: ParserConfig> Lexer<'_, C> {
    ...
    const BYTE_HANDLERS: [ByteHandler<C>; 256] = byte_handlers();
...
#[rustfmt::skip]
const fn byte_handlers<C: ParserConfig>() -> [ByteHandler<C>; 256] {
    [
...
/// fn SPS<C: ParserConfig>(lexer: &mut Lexer<'_, C>) -> Kind {
...
macro_rules! ascii_byte_handler {
    ($id:ident($lex:ident) $body:expr) => {
        #[expect(non_snake_case)]
        fn $id<C: ParserConfig>($lex: &mut Lexer<'_, C>) -> Kind {
            // SAFETY: This macro is only used for ASCII characters
            unsafe {
...
macro_rules! ascii_identifier_handler {
    ($id:ident($str:ident) $body:expr) => {
        #[expect(non_snake_case)]
        fn $id<C: ParserConfig>(lexer: &mut Lexer<'_, C>) -> Kind {
            // SAFETY: This macro is only used for ASCII characters
...
#[expect(non_snake_case)]
fn UNI<C: ParserConfig>(lexer: &mut Lexer<'_, C>) -> Kind {
...
#[expect(non_snake_case)]
fn UER<C: ParserConfig>(_lexer: &mut Lexer<'_, C>) -> Kind {
    unreachable!();
}
