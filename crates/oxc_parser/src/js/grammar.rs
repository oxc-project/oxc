...
impl<'a, C: Config> ParserImpl<'a, C> {
    ...
    pub(crate) fn refine_arrow_params(
        ...
        let Expression::Identifier(ident) = expr else {
            // `ArrowKind::Cover` only reaches here with a single parenthesized identifier.
            return self.unexpected();
        };
        let pattern = self.ast.binding_pattern_binding_identifier(ident.span, ident.name);
        let param = self.ast.plain_formal_parameter(ident.span, pattern);
