...
impl<'a> ParserImpl<'a> {
    pub(crate) fn eat_modifiers_before_declaration(&mut self) -> Modifiers<'a> {
        ...
        while self.at_modifier() {
            ...
            let modifier = self.parse_modifier(modifier_kind);
    ...
    fn try_parse_modifier(
        ...
        let modifier_kind = match kind {
            Kind::Const => {
                if !permit_const_as_modifier
                    || !is_token_on_same_line_and_can_follow_modifier(self.lexer.peek_token())
                {
                    return None;
                }
                ModifierKind::Const
            ...
            Kind::Static => {
                ...
                } else if !can_token_follow_modifier(next_token) {
                    return None;
                }
                ModifierKind::Static
            }
        ...
        Some(self.parse_modifier(modifier_kind))
    }

    fn parse_modifier(&mut self, kind: ModifierKind) -> Modifier {
    ...
    pub(crate) fn parse_contextual_modifier(&mut self, kind: Kind) -> bool {
        self.at(kind) && self.try_parse(Self::next_token_can_follow_get_or_set).is_some()
    }

    fn next_token_can_follow_get_or_set(&mut self) {
        self.bump_any();
        if !self.can_follow_get_or_set_keyword() {
            self.set_unexpected();
        }
    }
