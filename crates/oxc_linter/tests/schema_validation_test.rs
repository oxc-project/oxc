...
fn get_fixtures_path() -> PathBuf {
    get_project_root()
        .unwrap()
        .join("crates/oxc_linter/tests/fixtures/schema_validation")
}
...
fn load_schema() -> Validator {
    ...
    let schema: serde_json::Value = serde_json::from_str(&schema_json)
        .expect("Failed to parse generated schema as JSON");
...
fn test_schema_is_valid() {
    ...
    Validator::new(&schema)
        .expect("Generated schema should be valid JSON Schema");
}
