...
#[derive(Clone)]
pub(super) struct LintContextData<'a> {
...
#[derive(Clone)]
#[repr(transparent)]
pub struct LintContext<'a>(LintContextData<'a>);
...
impl<'a> From<LintContextData<'a>> for LintContext<'a> {
    fn from(data: LintContextData<'a>) -> Self {
        Self(data)
    }
}

impl<'a, 'b> From<&'b LintContextData<'a>> for &'b LintContext<'a> {
...
impl<'a, 'b> From<&'b LintContextData<'a>> for &'b CFGLintContext<'a> {
    fn from(data: &'b LintContextData<'a>) -> Self {
        #![allow(unsafe_code)]
...
impl<'a> LintCtx<'a> for LintContextData<'a> {
    fn new(file_path: Box<Path>, semantic: Rc<Semantic<'a>>) -> Self {
    ...
    fn diagnostic_with_fix<F: FnOnce(RuleFixer<'_, 'a>) -> Fix<'a>>(
        ...
        if self.fix {
            let fixer = RuleFixer::new(self.into());
