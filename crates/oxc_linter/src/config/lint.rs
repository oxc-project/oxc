...
impl CliRunner {
    ...
    pub(crate) fn run(self, stdout: &mut dyn Write) -> CliRunResult {
        ...
        // Print any warnings collected while building the configuration so they are captured
        // by the test harness (which passes a writer into `CliRunner::run`).
        for warning in config_builder.warnings() {
...
mod test {
    ...
    #[test]
    fn test_report_rule_without_enabled_plugin() {
        let args = &["-c", ".oxlintrc.json", "test.js"];
        Tester::new()
            .with_cwd("fixtures/report_rule_without_enabled_plugin".into())
            .test_and_snapshot(args);
    }
