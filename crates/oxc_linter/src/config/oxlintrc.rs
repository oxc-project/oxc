...
pub struct Oxlintrc {
    ...
    /// - Node.js >=22.18.0: TypeScript files are supported natively with built-in
    ///   type-stripping enabled by default.
    /// - Node.js >=22.6.0: TypeScript files are supported by setting
    ///   `NODE_OPTIONS=--experimental-strip-types`.
    /// - Node.js <22.6.0: TypeScript files are supported by setting
    ///   `NODE_OPTIONS=--import @oxc-node/core/register` and installing
    ///   `@oxc-node/core` as a dev dependency.
    ///
    /// ```json
    /// {
