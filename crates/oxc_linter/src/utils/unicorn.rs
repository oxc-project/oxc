    ...
    ) = (left, right)
    {
        ...
        match (&left.expression, &right.expression) {
            ...
            | (Expression::RegExpLiteral(regex_lit), Expression::StringLiteral(string_lit)) => {
                if string_lit.value.as_str() != Some(regex_lit.raw.as_ref().unwrap().as_str()) {
                    return false;
                }
            }
            // ex) x[`/regex/`] === x[/regex/]
            ...
            | (Expression::RegExpLiteral(regex_lit), Expression::TemplateLiteral(template_lit)) => {
                if template_lit.single_quasi().and_then(oxc_str::JSStr::as_str)
                    != Some(regex_lit.raw.as_ref().unwrap().as_str())
                {
                    return false;
                }
            }
