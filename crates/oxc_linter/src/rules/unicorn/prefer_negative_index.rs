use oxc_ast::{
    AstKind,
    ast::{
        Argument, BinaryExpression, BinaryOperator, Expression, ExpressionKind,
                ...
                ExpressionKind::ArrayExpression(array_expr) => {
                    for (j, element) in array_expr.elements.iter().enumerate() {
                        ...
                        if let Some(binary_expr) =
                            element.as_expression().and_then(|e| e.as_binary_expression())
                        {
...
fn get_binary_left_expr<'a>(
    ...
    match binary_expr.left.kind() {
        ExpressionKind::ParenthesizedExpression(paren_expr) => {
            let ExpressionKind::BinaryExpression(paren_inner_binary_expr) =
                paren_expr.expression.kind()
            else {
                return None;
            };
