...
fn identifier(span0: Span, x1: &str) -> OxcDiagnostic {
    OxcDiagnostic::warning(format!("eslint-plugin-unicorn(no-object-as-default-parameter): Do not use an object literal as default for parameter `{x1}`.")).with_labels([span0.into()])
}

fn non_identifier(span0: Span) -> OxcDiagnostic {
    OxcDiagnostic::warning("eslint-plugin-unicorn(no-object-as-default-parameter): Do not use an object literal as default").with_labels([span0.into()])
}
