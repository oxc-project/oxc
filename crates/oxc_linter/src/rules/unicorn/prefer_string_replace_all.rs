...
fn string_literal(span0: Span, x1: &str) -> OxcDiagnostic {
    OxcDiagnostic::warning(format!("eslint-plugin-unicorn(prefer-string-replace-all): This pattern can be replaced with `{x1}`.")).with_labels([span0.into()])
}

fn use_replace_all(span0: Span) -> OxcDiagnostic {
    OxcDiagnostic::warning("eslint-plugin-unicorn(prefer-string-replace-all): Prefer `String#replaceAll()` over `String#replace()` when using a regex with the global flag.")
