use oxc_ast::{
    AstKind,
    ast::{Argument, Expression, FormalParameters, FunctionBody, Statement, StatementKind},
};
...
fn check_function<'a>(
    ...
    if let Some(return_statement) = &function_body.statements[0].as_return_statement()
