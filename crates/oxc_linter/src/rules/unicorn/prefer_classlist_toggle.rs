...
fn extract_single_expression_from_statement<'a>(
    ...
    match stmt.kind() {
        ...
        StatementKind::BlockStatement(block_stmt) if block_stmt.body.len() == 1 => {
            if let Some(e) = block_stmt.body.as_expression_statement()[0] {
                Some(&e.expression)
            } else {
                None
            }
...
fn identify_add_remove_pair<'a>(
    ...
    let Some(classlist_add_member_expr) = classlist_add_callee.object.get_inner_expression().as_static_member_expression()
    else {
        return None;
    };
    let Some(classlist_remove_member_expr) = classlist_remove_callee.object.get_inner_expression().as_static_member_expression()
    else {
        return None;
    };
