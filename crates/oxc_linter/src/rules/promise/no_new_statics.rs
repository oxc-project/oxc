...
fn static_promise_diagnostic(span0: Span) -> OxcDiagnostic {
    OxcDiagnostic::warn(
        "eslint-plugin-promise(no-new-statics): Disallow calling `new` on a Promise static method",
    )
    .with_label(span0)
}
...
impl Rule for NoNewStatics {
    fn run<'a>(&self, node: &AstNode<'a>, ctx: &LintContext<'a>) {
        ...
        if matches!(
            ...
            ctx.diagnostic_with_fix(static_promise_diagnostic(new_expr.span), |fixer| {
                fixer.delete_range(Span::new(new_expr.span.start, ident.span.start))
            });
        }
    }
}
