...
impl<'a, 'b> MultipleResolvedChecker<'a, 'b> {
    ...
    #[inline]
    fn resolved_infos(&self) -> &FxHashMap<BlockNodeId, BlockResolvedInfo<'a>> {
        &self
            .current_func_resolved_info()
            .expect("Function resolved info should exist at this point")
            .resolved_infos
    }

    #[inline]
    fn resolved_infos_mut(&mut self) -> &mut FxHashMap<BlockNodeId, BlockResolvedInfo<'a>> {
        &mut self
            .current_func_resolved_info_mut()
            .expect("Function resolved info should exist at this point")
            .resolved_infos
    }

    #[inline]
    fn block_ids(&self) -> &Vec<BlockNodeId> {
        &self
            .current_func_resolved_info()
            .expect("Function resolved info should exist at this point")
            .block_ids
    ...
    #[inline]
    fn block_ids_mut(&mut self) -> &mut Vec<BlockNodeId> {
        &mut self
            .current_func_resolved_info_mut()
            .expect("Function resolved info should exist at this point")
            .block_ids
    ...
    #[inline]
    fn dominators(&self) -> &FxHashMap<BlockNodeId, BlockNodeId> {
        &self
            .current_func_resolved_info()
            .expect("Function resolved info should exist at this point")
            .dominators
    ...
    fn set_dominators(&mut self, dominators: FxHashMap<BlockNodeId, BlockNodeId>) {
        self.current_func_resolved_info_mut()
            .expect("Function resolved info should exist at this point")
            .dominators = dominators;
    }

    fn compute_block_node_dominator(&self) -> FxHashMap<BlockNodeId, BlockNodeId> {
        let mut block_nodes_dominators: FxHashMap<BlockNodeId, FxHashSet<BlockNodeId>> = self
            .block_ids()
            .iter()
            .map(|&block_id| (block_id, FxHashSet::from_iter([block_id])))
            .collect();

        self.block_ids().iter().for_each(|&block_id| {
        ...
        let block_nodes_sort: FxHashMap<_, _> =
            self.block_ids().iter().enumerate().map(|(i, &block_id)| (block_id, i)).collect();
    ...
    fn prev_resolved_info(
        ...
        for &block_id in &incoming_block_ids {
            if let Some(resolved_info) = self.resolved_infos().get(&block_id) {
        ...
        // If any source node is Potential and this node has no other target nodes, then the current node is Potential
            ...
            if incoming_block_ids.len() > 1
                && let Some(block_node_dominator) = self.dominators().get(&block_id)
            {
                if let Some(resolved_infos) = self.resolved_infos().get(block_node_dominator) {
        ...
        set_depth_first_search::<_, _, _, Control<()>, _>(graph, Some(start_block_id), |event| {
            match event {
                ...
                DfsEvent::Finish(block_id, _) => {
                    self.block_ids_mut().push(block_id);
                    Control::Continue
                }
                _ => Control::Continue,
            }
        });
        self.block_ids_mut().reverse();
    }
    ...
    fn check(&mut self, start_block_id: BlockNodeId) {
        ...
        let block_ids: Vec<_> = self.block_ids().clone();
        ...
        for &block_id in block_ids.iter().skip(1) {
            ...
            let Some(resolved_info) = self.resolved_infos_mut().get_mut(&block_id) else {
    ...
    fn check_block(&mut self, block_id: BlockNodeId) {
        ...
        if let Some(first_resolved) = resolved.first() {
            ...
            self.resolved_infos_mut().insert(
                block_id,
