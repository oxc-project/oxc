...
#[diagnostic(severity(warning), help("Try removing the invalid prop."))]
struct AriaUnsupportedElementsDiagnostic(#[label] pub Span);
            ...
            if RESERVED_HTML_TAG.contains(&el_type) {
                for attr in &jsx_el.attributes {
                    ...
                    if INVALID_ATTRIBUTES.contains(&attr_name) {
                        ctx.diagnostic(AriaUnsupportedElementsDiagnostic(attr.span));
                    }
                }
            }
        }
