...
fn comment(x0: &str, span1: Span) -> OxcDiagnostic {
    OxcDiagnostic::warning(format!("typescript-eslint(ban-ts-comment): Do not use @ts-{x0} because it alters compilation errors.")).with_labels([span1.into()])
}

fn ignore_instead_of_expect_error(span1: Span) -> OxcDiagnostic {
    OxcDiagnostic::warning("typescript-eslint(ban-ts-comment): Use \"@ts-expect-error\" instead of @ts-ignore, as \"@ts-ignore\" will do nothing if the following line is error-free.")
...
fn comment_description_not_match_pattern(x0: &str, x1: &str, span2: Span) -> OxcDiagnostic {
    OxcDiagnostic::warning(format!("typescript-eslint(ban-ts-comment): The description for the @ts-{x0} directive must match the {x1} format.")).with_labels([span2.into()])
}
                    ...
                    DirectiveConfig::Boolean(on) => {
                        if *on {
                            if directive == "ignore" {
                                ...
                                ctx.diagnostic(comment(directive, span));
                            }
                        }
                    }
                    config => {
