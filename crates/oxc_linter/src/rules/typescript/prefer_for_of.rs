            ...
            ExpressionKind::AssignmentExpression(expr) => {
                ...
                match expr.operator {
                    AssignmentOperator::Addition => {
                        matches!(expr.right.kind(), ExpressionKind::NumericLiteral(lit) if (lit.value - 1f64).abs() < f64::EPSILON)
                    }
                    AssignmentOperator::Assign => {
                        let ExpressionKind::BinaryExpression(bin_expr) = expr.right.kind() else {
                            return false;
                        };
...
impl Rule for PreferForOf {
    fn run<'a>(&self, node: &AstNode<'a>, ctx: &LintContext<'a>) {
        ...
        if !decl.init.as_ref().is_some_and(|init| {
            matches!(init.kind(), ExpressionKind::NumericLiteral(literal) if literal.value == 0f64)
        }) {
            return;
        }
        ...
        let Some(test_expr) = test.as_binary_expression() else {
            return;
        };

        if !matches!((test_expr.left.kind(), test_expr.operator),
        ...
        let (array_name, array_expr) = {
            ...
            let array_name = match array_expr.kind() {
                ...
                | ExpressionKind::PrivateFieldExpression(_) => {
                    match array_expr.to_member_expression().static_property_name() {
...
fn prevents_for_of_array_access(
    ...
    let ExpressionKind::Identifier(id) = mem_expr.object().kind() else {
        return false;
    };
