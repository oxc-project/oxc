...
fn prefer_direct_imports_diagnostic(span: Span, loaded_modules: Option<usize>) -> OxcDiagnostic {
...
pub struct PreferDirectImports;

declare_oxc_lint!(
    /// ### What it does
    ///
    /// Disallows importing from barrel files and suggests importing directly from source files instead.
    ///
    /// ### Why is this bad?
    ///
    /// Barrel files that re-export many modules can hurt performance. This rule helps you bypass
    /// barrel files by importing directly from source files, which improves tree-shaking and reduces
    /// bundle sizes.
    ///
    ...
    /// ```javascript
    /// import { User, Product, type User } from './models';
    /// ```
    ///
...
impl Rule for PreferDirectImports {
        ...
        for (_span, entries) in import_statements {
            ...
            if has_barrel_imports {
                ctx.diagnostic_with_suggestion(
                    prefer_direct_imports_diagnostic(
                        first_entry.module_request.span,
                        loaded_modules,
                    ),
                    |fixer| {
                        fix_barrel_imports(
...
import { User } from './barrel/user';"#,
    ...
    Tester::new(PreferDirectImports::NAME, PreferDirectImports::PLUGIN, pass, fail)
