...
fn no_map_spread_diagnostic(map_call: Span, spread: &Spread<'_, '_>) -> OxcDiagnostic {
    match spread {
        // Obj
        Spread::Object(obj) => OxcDiagnostic::warn(
            "Spreading to modify object properties in `map` calls is inefficient",
        )
        .with_labels([
            map_call.label("This map call spreads an object"),
            obj.span().primary_label("That could be mutated in place"),
        ])
        ...
        Spread::Array(arr) => {
            OxcDiagnostic::warn("Spreading to modify array elements in `map` calls is inefficient")
                .with_labels([
                    map_call.label("This map call spreads an array"),
                    arr.span().primary_label("That could be mutated in place"),
                ])
                .with_help(
...
impl Rule for NoMapSpread {
    ...
    fn run<'a>(&self, node: &AstNode<'a>, ctx: &LintContext<'a>) {
        ...
        // let Some((is_expression, body)) = get_map_callback(call_expr) else {
        //     return;
        // };
        ...
        match leftmost_identifier_reference(&call_expr.callee) {
            Ok(ident) => {
                if self.ignore_rereads
                    && ident
                        .reference_id()
                        // reference -> symbol being referenced
                        .and_then(|id| {
                            ctx.semantic()
                                .symbols()
                                .get_reference(id)
                                .symbol_id()
                                .map(|symbol_id| (id, symbol_id))
                        })
                        .is_some_and(|(ref_id, symbol_id)| {
                            has_reads_after(ctx, ref_id, symbol_id, call_expr.span)
                        })
                {
                    return;
                }

                // ctx.diagnostic(no_map_spread_diagnostic(call_expr.span));
                // continue
            }
