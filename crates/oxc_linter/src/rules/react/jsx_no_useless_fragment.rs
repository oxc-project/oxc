...
fn needs_more_children(span0: Span) -> OxcDiagnostic {
    OxcDiagnostic::warning("eslint-plugin-react(jsx-no-useless-fragment): Fragments should contain more than one child.").with_labels([span0.into()])
}

fn child_of_html_element(span0: Span) -> OxcDiagnostic {
    OxcDiagnostic::warning("eslint-plugin-react(jsx-no-useless-fragment): Passing a fragment to a HTML element is useless.").with_labels([span0.into()])
}
