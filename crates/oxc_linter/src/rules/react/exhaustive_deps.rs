...
impl Rule for ExhaustiveDeps {
    ...
    fn run<'a>(&self, node: &AstNode<'a>, ctx: &LintContext<'a>) {
        ...
        let callback_node = match callback_node.kind() {
        ...
        let dependencies_node = dependencies_node.and_then(|node| match node.kind() {
            Argument::SpreadElement(_) => {
        ...
        if is_effect {
            for r#ref in refs_inside_cleanups {
                if let Some(ident) = r#ref.object.get_inner_expression().as_identifier() {
        ...
        for dep in &declared_dependencies {
            if let Some(symbol_id) = dep.symbol_id
                ...
                && let Some(ExpressionKind::CallExpression(call_expr)) = &var_decl.init
...
fn func_call_without_react_namespace<'a>(call_expr: &'a CallExpression<'a>) -> Option<&'a str> {
    ...
    if let Some(ident) = inner_exp.as_identifier() {
        return Some(&ident.name);
    }

    let Some(member) = inner_exp.as_static_member_expression() else {
        return None;
    };
