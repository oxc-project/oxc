...
use oxc_ast::{
    AstKind,
    ast::{
        ArrayExpression, CallExpression, Expression, JSXAttributeItem, JSXAttributeName, JSXChild,
        JSXElement, JSXFragment, Statement,
    },
};
                ...
                match value {
                    ...
                    oxc_ast::ast::JSXAttributeValue::ExpressionContainer(container) => {
                        ...
                        match &container.expression {
                            oxc_ast::ast::JSXExpression::StringLiteral(lit) => {
                                return Some((lit.value.to_string(), attr.span));
                            }
                            oxc_ast::ast::JSXExpression::NumericLiteral(lit) => {
                                return Some((lit.value.to_string(), attr.span));
                            }
                            oxc_ast::ast::JSXExpression::TemplateLiteral(lit)
...
fn check_duplicate_keys_in_array<'a>(array_expr: &ArrayExpression<'a>, ctx: &LintContext<'a>) {
    ...
    for element in &array_expr.elements {
        ...
        if let oxc_ast::ast::ArrayExpressionElement::JSXElement(jsx_elem) = element
