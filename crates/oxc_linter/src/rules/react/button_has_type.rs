...
fn missing_type_prop(span0: Span) -> OxcDiagnostic {
    OxcDiagnostic::warning("eslint-plugin-react(button-has-type): `button` elements must have an explicit `type` attribute.")
        .with_help("Add a `type` attribute to the `button` element.")
        .with_labels([span0.into()])
}

fn invalid_type_prop(span0: Span) -> OxcDiagnostic {
    OxcDiagnostic::warning("eslint-plugin-react(button-has-type): `button` elements must have a valid `type` attribute.")
