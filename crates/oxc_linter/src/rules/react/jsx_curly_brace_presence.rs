            ...
            JSXAttributeValue::Element(el) => {
                if self.prop_element_values.is_always() {
                    ctx.diagnostic_with_fix(
                        ...
                        |fixer| {
                            let text = ctx.source_range(el.span).cow_replace('\\', r"\\");
...
fn replace_html_string_to_literal(html: &str) -> String {
    ...
    let escaped_html = html.cow_replace('\\', r"\\");
    let escaped_html = escaped_html.cow_replace('"', r#"\""#);
    format!("{{\"{escaped_html}\"}}")
}

fn replace_string_literal_to_html(string: &str) -> String {
    let escaped_html = string.cow_replace('\\', r"\\");
    let escaped_html = escaped_html.cow_replace('"', r#"\""#);
    format!("\"{escaped_html}\"")
}
