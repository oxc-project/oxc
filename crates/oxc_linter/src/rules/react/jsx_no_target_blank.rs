...
/// Split `rel` on spaces and test each token. A value with a lone surrogate
/// is not checked and is accepted.
fn any_rel_token(value: JSStr<'_>, is_match: impl FnMut(&str) -> bool) -> bool {
    value.as_str().is_none_or(|value| value.split(' ').any(is_match))
}
...
fn test() {
    ...
    let pass = vec![
        ...
        (r#"<a href={"foobar"} target={"_blank"} rel={"noopener noreferrer"}></a>"#, None, None),
        (r#"<a href={"foobar"} target={"_blank"} rel={"noreferrer"}></a>"#, None, None),
        // A `rel` value with a lone surrogate is not checked.
        (
