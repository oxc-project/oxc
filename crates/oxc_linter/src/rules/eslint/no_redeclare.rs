...
fn no_redeclare_diagnostic(name: &str, span: Span, is_builtin: bool) -> OxcDiagnostic {
    let message = if is_builtin {
        format!("'{name}' is already defined as a built-in global variable.")
    } else {
...
impl Rule for NoRedeclare {
    ...
    fn run_on_symbol(&self, symbol_id: SymbolId, ctx: &LintContext) {
        ...
        let is_builtin = self.built_in_globals && ctx.is_env_or_global_var(name);

        if is_builtin {
            let span = ctx.scoping().symbol_span(symbol_id);
            ctx.diagnostic(no_redeclare_diagnostic(name, span, is_builtin));
        }
