...
impl NoSelfAssign {
        ...
        match left {
            AssignmentTarget::SimpleAssignmentTarget(simple_assignment_target) => {
                if let Expression::Identifier(id2) = right.without_parenthesized() {
                    ...
                    if let Some(Expression::Identifier(id1)) =
                        simple_assignment_target.get_expression()
                    {
                        if id1.name == id2.name {
                            self_assign = true;
                        }
                    } else if let SimpleAssignmentTarget::AssignmentTargetIdentifier(id1) =
    ...
    fn is_member_expression_same_reference<'a>(
        ...
        if let Some(name1) = member1.static_property_name() {
            if name1 == member2.static_property_name().unwrap_or_default() {
