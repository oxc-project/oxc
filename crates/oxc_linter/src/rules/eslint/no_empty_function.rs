...
struct ViolationInfo<'a>(pub Option<(&'static str, Option<oxc_ast::StaticName<'a>>)>);
impl<'a> From<(&'static str, Option<oxc_ast::StaticName<'a>>)> for ViolationInfo<'a> {
    fn from(value: (&'static str, Option<oxc_ast::StaticName<'a>>)) -> Self {
        ...
        while let Some(parent) = ancestors.next() {
            match parent {
                ...
                | AstKind::IdentifierReference(IdentifierReference { name, .. }) => {
                    return ("function", Some(oxc_ast::StaticName::from(*name))).into();
                }
