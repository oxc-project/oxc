...
impl Rule for NoUnusedVars {
    ...
    fn run_once(&self, ctx: &LintContext) {
        // Precompute exported local names once (avoids O(exports) scan per symbol).
        let exported_names = Symbol::collect_exported_local_names(ctx.module_record());
        ...
        for symbol in ctx.scoping().symbol_ids() {
            ...
            self.run_on_symbol_internal(&symbol, ctx, &exported_names);
        }
    }
