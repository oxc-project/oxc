use oxc_ast::{
    ...
    ast::{
        ...
        ExpressionTag, MemberExpressionKind, SimpleAssignmentTarget, UnaryOperator, UpdateOperator,
    },
};
...
fn verify(expr: &AssignmentExpression, mode: &AlwaysNever, ctx: &LintContext) {
    ...
    let left = &expr.left;
    let ExpressionKind::BinaryExpression(binary_expr) = expr.right.without_parentheses().kind()
    else {
        return;
    };
...
fn can_be_fixed(target: &AssignmentTarget) -> bool {
    ...
    match expr.kind() {
        MemberExpressionKind::ComputedMemberExpression(computed_expr) => {
            matches!(
                computed_expr.object.tag(),
...
fn check_is_same_reference(left: &AssignmentTarget, right: &Expression, ctx: &LintContext) -> bool {
    ...
    // x.y vs x['y']
    if (left_member_expr.is_computed_member_expression()
        && !right_member_expr.is_computed_member_expression())
        || (!left_member_expr.is_computed_member_expression()
            && right_member_expr.is_computed_member_expression())
    {
        return false;
    }
