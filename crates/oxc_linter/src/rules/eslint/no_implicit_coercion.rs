        ...
        match node.kind() {
            AstKind::UnaryExpression(unary_expr) => {
                ...
                if self.boolean
                    ...
                    && let ExpressionKind::UnaryExpression(inner) = unary_expr.argument.kind() && inner.operator == UnaryOperator::LogicalNot
                {
                ...
                if self.number
                    ...
                    && let ExpressionKind::UnaryExpression(inner) = unary_expr.argument.without_parentheses().kind() && inner.operator == UnaryOperator::UnaryNegation
...
fn is_indexof_call(expr: &Expression) -> bool {
    match expr.without_parentheses().kind() {
        ExpressionKind::CallExpression(call) => {
            ...
            if let ExpressionKind::StaticMemberExpression(member) = callee.kind() {
                return matches!(member.property.name.as_str(), "indexOf" | "lastIndexOf");
            }
            if let ExpressionKind::ChainExpression(chain) = callee.kind()
                && let oxc_ast::ast::ChainElement::MemberExpression(member) = &chain.expression
        ...
        ExpressionKind::ChainExpression(chain) => {
            if let oxc_ast::ast::ChainElement::CallExpression(call) = &chain.expression
                && let ExpressionKind::StaticMemberExpression(member) =
                    call.callee.without_parentheses().kind()
            {
                return matches!(member.property.name.as_str(), "indexOf" | "lastIndexOf");
            }
            false
