...
impl Rule for OneVar {
    ...
    fn run<'a>(&self, node: &AstNode<'a>, ctx: &LintContext<'a>) {
        // `never` and `consecutive` only depend on a declaration and its previous
        // statement-list sibling, so they run through the bucketed per-node-type
        // dispatch. `always` needs cross-declaration scope state and stays in
        // `run_once`.
        let AstKind::VariableDeclaration(decl) = node.kind() else { return };

        let policy = self.0.policy(decl.kind);
        let policy_needs_never =
        ...
        if !policy_needs_never && !policy_needs_consecutive {
            return;
        }

        let summary = summarize_declaration(decl, self.0.separate_requires);

        if policy_needs_consecutive
            && let Some(previous) = previous_sibling_declaration(ctx, node.id(), decl.span().start)
            && previous.kind == decl.kind
        {
            let previous_summary = summarize_declaration(previous, self.0.separate_requires);
            self.check_consecutive(decl, summary, previous_summary, policy, ctx);
        }

        if policy_needs_never {
            let context = classify_var_decl_context(ctx, node.id(), decl);
            Self::check_never(decl, summary, policy, context, ctx);
        }
    }

    fn run_once(&self, ctx: &LintContext<'_>) {
        // Only the `always` family (including `splitRequires`) needs
        // cross-declaration state per effective scope, covering binding-less
        // declarators like `var {} = obj` that scoping APIs cannot surface.
        // TODO: replace this node-table scan with a per-node-type iteration API
        // once one exists, so only `VariableDeclaration` nodes are visited.
        if !self.0.needs_always() {
            return;
        }
...
// Mirrors ESLint's `parent.body[indexOf(node) - 1]` lookup, extended to
// `SwitchCase.consequent` (intentional oxlint divergence). Only direct
// statement-list children participate in `consecutive`. Sibling statements are
// source-ordered, so the declaration is located by binary search on span start.
fn previous_sibling_declaration<'a>(
    ctx: &LintContext<'a>,
    node_id: NodeId,
    span_start: u32,
) -> Option<&'a VariableDeclaration<'a>> {
    let statements: &[Statement<'a>] = match ctx.nodes().parent_kind(node_id) {
    ...
    let index =
        statements.binary_search_by(|statement| statement.span().start.cmp(&span_start)).ok()?;

    match statements.get(index.checked_sub(1)?)? {
        Statement::VariableDeclaration(previous) => Some(previous),
        _ => None,
    }
}
