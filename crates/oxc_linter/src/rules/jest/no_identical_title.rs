...
fn describe_repeat(span0: Span) -> OxcDiagnostic {
    OxcDiagnostic::warning("eslint-plugin-jest(no-identical-title): Describe block title is used multiple times in the same describe block.")
        .with_help("Change the title of describe block.")
        .with_labels([span0.into()])
}

fn test_repeat(span0: Span) -> OxcDiagnostic {
    OxcDiagnostic::warning("eslint-plugin-jest(no-identical-title): Test title is used multiple times in the same describe block.").with_help("Change the title of test.").with_labels([span0.into()])
}
