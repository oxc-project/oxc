...
impl Rule for BadArrayMethodOnArguments {
    fn run<'a>(&self, node: &AstNode<'a>, ctx: &LintContext<'a>) {
        if let AstKind::IdentifierReference(reference) = node.kind() {
            if reference.name == "argument" {
                if let Some(parent_node_id) = ctx.nodes().parent_id(node.id()) {
                    if let AstKind::MemberExpression(member_expr) = ctx.nodes().kind(parent_node_id)
                    {
                        if let Some(parent_node_id) = ctx.nodes().parent_id(parent_node_id) {
                            if let AstKind::CallExpression(_) = ctx.nodes().kind(parent_node_id) {
                                match member_expr {
                                    MemberExpression::StaticMemberExpression(expr) => {
                                        if ARRAY_METHODS
                                            .binary_search(&expr.property.name.as_str())
                                            .is_ok()
                                        {
                                    ...
                                    MemberExpression::ComputedMemberExpression(expr) => {
                                        match &expr.expression {
                                            Expression::StringLiteral(name) => {
                                                if ARRAY_METHODS
                                                    .binary_search(&name.value.as_str())
                                                    .is_ok()
                                                {
                                                    ctx.diagnostic(
                                                        BadArrayMethodOnArgumentsDiagnostic(
                                                            name.value.clone(),
                                                            expr.span,
                                                        ),
                                                    );
                                            ...
                                            Expression::TemplateLiteral(template) => {
                                                ...
                                                {
                                                    if let Some(template_element) =
                                                        template.quasis.get(0)
                                                    {
                                                        if let Some(name) =
                                                            template_element.value.cooked.as_deref()
                                                        {
                                                            if ARRAY_METHODS
                                                                ...
                                                                .is_ok()
                                                            {
                                                                ctx.diagnostic(BadArrayMethodOnArgumentsDiagnostic(Atom::from(name),expr.span));
                                                            }
                                                        }
                                                    }
                                                }
