...
#[derive(Debug, Clone)]
pub struct NoHooks<F: TestFramework + std::fmt::Debug>(PhantomData<F>, Box<NoHooksConfig>);

impl<F: TestFramework + std::fmt::Debug> Default for NoHooks<F> {
    fn default() -> Self {
...
impl<F: TestFramework + std::fmt::Debug> std::ops::Deref for NoHooks<F> {
...
impl<F: TestFramework + std::fmt::Debug> Rule for NoHooks<F> {
