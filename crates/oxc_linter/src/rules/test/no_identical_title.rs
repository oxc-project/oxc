...
#[derive(Debug, Clone)]
pub struct NoIdenticalTitle<F: TestFramework + std::fmt::Debug>(PhantomData<F>);

impl<F: TestFramework + std::fmt::Debug> Default for NoIdenticalTitle<F> {
    fn default() -> Self {
        Self(PhantomData)
    }
}
...
impl<F: TestFramework + std::fmt::Debug> Rule for NoIdenticalTitle<F> {
