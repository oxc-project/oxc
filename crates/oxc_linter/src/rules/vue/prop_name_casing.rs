...
impl PropNameCasing {
    ...
    fn report_if_invalid(&self, name: JSStr<'_>, span: Span, ctx: &LintContext<'_>) {
        ...
        // A name with a lone surrogate is not matched against `ignoreProps` and is ignored.
        if !options.ignore_props.is_empty()
            && name.as_str().is_none_or(|name| is_ignored(name, &options.ignore_props))
        {
            return;
        }
        ...
        let display = diagnostic_text(name);
...
fn test() {
    ...
    let pass = vec![
        // A name with a lone surrogate is not matched against `ignoreProps`.
        (
            r#"
                  <script>
                  export default {
                ...
                "#,
            Some(serde_json::json!([ "camelCase", { "ignoreProps": ["_text$"] } ])),
            None,
