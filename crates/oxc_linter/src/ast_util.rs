...
pub fn is_static_boolean<'a>(expr: &Expression<'a>, semantic: &Semantic<'a>) -> bool {
    match expr.kind() {
        ...
        ExpressionKind::CallExpression(call_expr) => call_expr.is_constant(true, semantic),
...
fn is_logical_identity(op: LogicalOperator, expr: &Expression) -> bool {
    match expr.kind() {
        ...
        ExpressionKind::ParenthesizedExpression(expr) => is_logical_identity(op, &expr.expression),
        _ => false,
    }
}
...
impl<'a> IsConstant<'a, '_> for CallExpression<'a> {
    fn is_constant(&self, _in_boolean_position: bool, semantic: &Semantic<'a>) -> bool {
        if let ExpressionKind::Identifier(ident) = self.callee.kind()
            && ident.name == "Boolean"
...
impl<'a> IsConstant<'a, '_> for Argument<'a> {
    fn is_constant(&self, in_boolean_position: bool, semantic: &Semantic<'a>) -> bool {
        match self {
            ...
            Self::Expression(expr) => expr.is_constant(in_boolean_position, semantic),
        }
    }
}
...
pub fn extract_regex_flags<'a>(args: &'a ArenaVec<'a, Argument<'a>>) -> Option<RegExpFlags> {
    ...
    let Argument::Expression(arg_expr) = &args[1] else {
        return None;
    };
    let flag_arg = match arg_expr.kind() {
        ExpressionKind::StringLiteral(flag_arg) => flag_arg.value,
        ExpressionKind::TemplateLiteral(template) => template.single_quasi()?,
        _ => return None,
    };
...
pub fn is_method_call<'a>(
    ...
    if let Some(objects) = objects {
        let ExpressionKind::Identifier(ident) = member_expr.object().get_inner_expression().kind()
        else {
            return false;
        };
...
pub fn is_new_expression<'a>(
    ...
    let ExpressionKind::Identifier(ident) = new_expr.callee.without_parentheses().kind() else {
        return false;
    };
...
pub fn leftmost_identifier_reference<'a, 'b: 'a>(
    ...
    match expr.kind() {
        ExpressionKind::Identifier(ident) => Ok(ident),
        ExpressionKind::StaticMemberExpression(mem) => leftmost_identifier_reference(&mem.object),
        ExpressionKind::ComputedMemberExpression(mem) => leftmost_identifier_reference(&mem.object),
        ExpressionKind::PrivateFieldExpression(mem) => leftmost_identifier_reference(&mem.object),
        _ => Err(expr),
    }
}
...
fn is_array_from_family_method(callee: &Expression) -> bool {
    ...
    match member_expr.static_property_name() {
        Some("from") => {
            matches!(object.get_inner_expression().kind(), ExpressionKind::Identifier(ident) if ident.name.ends_with("Array"))
        }
...
pub fn get_static_property_name<'a>(parent_node: &AstNode<'a>) -> Option<Cow<'a, str>> {
    ...
    if key.as_expression().is_some_and(Expression::is_null_literal) {
        return Some("null".into());
    }

    match key.as_expression().map(Expression::kind) {
        Some(ExpressionKind::RegExpLiteral(regex)) => Some(Cow::Owned(regex.regex.to_string())),
        Some(ExpressionKind::BigIntLiteral(bigint)) => Some(Cow::Borrowed(bigint.value.as_str())),
        Some(ExpressionKind::TemplateLiteral(template)) => {
...
pub fn get_outer_member_expression<'a, 'b>(
    ...
    match assignment {
        SimpleAssignmentTarget::MemberExpression(member) => {
            let mut node = member.as_static_member_expression()?;
            loop {
                if node.object.is_null() {
                    return Some(node);
                }

                if let Some(object) = node.object.as_static_member_expression()
