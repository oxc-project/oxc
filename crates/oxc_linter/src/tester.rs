...
use crate::{rules::RULES, LintOptions, Linter, Runner};
...
impl Tester {
    pub fn new<S: Into<String>>(
        ...
        Self { rule_path, rule_name, expect_pass, expect_fail, snapshot: String::new() }
    }
    ...
    fn run(&mut self, source_text: &str, config: Option<Value>) -> bool {
        let source_type = SourceType::from_path(&self.rule_path).unwrap();
        let rule = RULES
            .iter()
        ...
        let linter = Linter::from_rules(vec![rule]).with_fix(false);
        let path = PathBuf::from("./tests/fixtures/index.js").canonicalize().unwrap();
        Runner::new(LintOptions::default()).with_linter(linter).run_source(
        ...
        drop(tx_error);
        let result = rx_error.iter().flat_map(|(_, diagnostic)| diagnostic).collect::<Vec<_>>();
        if result.is_empty() {
            return true;
        }
