...
impl Linter {
    ...
    fn run_rules<'a, const TIMINGS: bool>(
        ...
        // Skipped for files parsed by an external JS parser, which run their external
        // rules through `run_external_rules_with_js_parser` instead.
    ...
    pub fn run_with_js_parser(
        ...
        #[expect(clippy::cast_possible_truncation)]
        let source_text_utf16_len = {
            let mut len = original_source_text.len() as u32;
            if let Some(mut converter) = span_converter.converter() {
                converter.convert_offset(&mut len);
            }
            len
        };
