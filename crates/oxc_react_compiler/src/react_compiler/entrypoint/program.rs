...
// oxc splice (Stage 2)
//
// Replaces the Babel splicing machinery above. Builds the final oxc `Program` by
// substituting each compiled oxc function (from codegen) for its original — matched
// by `span.start == fn_node_id` — applying gating, inserting outlined functions, and
// adding the memo-cache / gating imports. The HIR-driven decisions mirror the Babel
// reference (`apply_compiled_functions` & friends); only node construction differs.
// =============================================================================
