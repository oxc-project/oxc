...
/// by source start offset. Codegen (`set_raw_type_renames`) uses these (filtered by
/// `reference_node_ids`) to apply binding renames when it re-parses the type from
/// source (`typeof x` -> `typeof x_0`). Over-collected idents (type labels,
/// object-type property keys) are harmless — they are dropped by the
/// `reference_node_ids` filter in codegen.
fn collect_type_idents(ty: &oxc::TSType) -> Vec<crate::react_compiler_ast::common::RawIdent> {
...
fn lower_statement(
    ...
    match stmt {
        ...
        oxc::Statement::TSEnumDeclaration(e) => {
            ...
            // `UnsupportedNode` carrying a Babel enum node whose span lets the
            // back-end re-parse it verbatim from source (matching the original
            // Babel front-end, which wrapped the enum the same way).
            let loc = builder.source_location(e.span);
            // The back-end re-parses the enum from `start`/`end`, so the Babel
            // `loc` (a distinct `SourceLocation` type) is unused here.
            let base = crate::react_compiler_ast::common::BaseNode {
                node_type: Some("TSEnumDeclaration".to_string()),
                start: Some(e.span.start),
                end: Some(e.span.end),
                loc: None,
                node_id: Some(e.span.start),
                ..Default::default()
            };
            let babel_enum = crate::react_compiler_ast::declarations::TSEnumDeclaration {
                base,
                id: crate::react_compiler_ast::expressions::Identifier {
                    base: crate::react_compiler_ast::common::BaseNode::default(),
                ...
                members: Vec::new(),
                declare: if e.declare { Some(true) } else { None },
                is_const: if e.r#const { Some(true) } else { None },
            };
            let original_node = Some(crate::react_compiler_ast::OriginalNode::Statement(Box::new(
                crate::react_compiler_ast::statements::Statement::TSEnumDeclaration(babel_enum),
            )));
            lower_value_to_temporary(
                builder,
