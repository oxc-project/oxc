...
impl Program<'_> {
    ...
    pub fn to_estree_ts_json(&self, ranges: bool) -> String {
        ...
        let config = ConfigTS::new(ranges);
        let mut serializer = CompactTSSerializer::with_capacity(capacity, config);
    ...
    pub fn to_estree_js_json(&self, ranges: bool) -> String {
        let capacity = self.source_text.len() * JSON_CAPACITY_RATIO_COMPACT;
        let config = ConfigJS::new(ranges);
        let mut serializer = CompactJSSerializer::with_capacity(capacity, config);
        self.serialize(&mut serializer);
        serializer.into_string()
    }
    ...
    pub fn to_pretty_estree_ts_json(&self, ranges: bool) -> String {
        ...
        let config = ConfigTS::new(ranges);
        let mut serializer = PrettyTSSerializer::with_capacity(capacity, config);
        self.serialize(&mut serializer);
        serializer.into_string()
    }
    ...
    pub fn to_pretty_estree_js_json(&self, ranges: bool) -> String {
        let capacity = self.source_text.len() * JSON_CAPACITY_RATIO_PRETTY;
        let config = ConfigJS::new(ranges);
        let mut serializer = PrettyJSSerializer::with_capacity(capacity, config);
        self.serialize(&mut serializer);
        serializer.into_string()
    }
    ...
    pub fn to_estree_ts_json_with_fixes(&self, ranges: bool) -> String {
        let capacity = self.source_text.len() * JSON_CAPACITY_RATIO_COMPACT;
        let config = ConfigFixesTS::new(ranges);
        let serializer = CompactFixesTSSerializer::with_capacity(capacity, config);
    ...
    pub fn to_estree_js_json_with_fixes(&self, ranges: bool) -> String {
        let capacity = self.source_text.len() * JSON_CAPACITY_RATIO_COMPACT;
        let config = ConfigFixesJS::new(ranges);
        let serializer = CompactFixesJSSerializer::with_capacity(capacity, config);
        serializer.serialize_with_fixes(self)
    }
    ...
    pub fn to_pretty_estree_ts_json_with_fixes(&self, ranges: bool) -> String {
        let capacity = self.source_text.len() * JSON_CAPACITY_RATIO_PRETTY;
        let config = ConfigFixesTS::new(ranges);
        let serializer = PrettyFixesTSSerializer::with_capacity(capacity, config);
        serializer.serialize_with_fixes(self)
    }
    ...
    pub fn to_pretty_estree_js_json_with_fixes(&self, ranges: bool) -> String {
        let capacity = self.source_text.len() * JSON_CAPACITY_RATIO_PRETTY;
        let config = ConfigFixesJS::new(ranges);
        let serializer = PrettyFixesJSSerializer::with_capacity(capacity, config);
        serializer.serialize_with_fixes(self)
    }
}
