...
impl<'a> AstKind<'a> {
    ...
    pub fn from_expression(e: &'a Expression<'a>) -> Self {
        match e.kind() {
            ...
            ExpressionKind::ImportMeta(e) => Self::ImportMeta(e),
            ExpressionKind::NewTarget(e) => Self::NewTarget(e),
    ...
    pub fn debug_name(&self) -> std::borrow::Cow<'_, str> {
        ...
        match self {
            ...
            Self::NewExpression(n) => {
                let callee = if let Some(id) = n.callee.as_identifier() {
                    Some(id.name.as_str())
                } else if let Some(member) = n.callee.as_member_expression() {
