...
const _: () = if cfg!(target_family = "wasm") || align_of::<u64>() == 8 {
    ...
    // Padding: 2 bytes
    assert!(size_of::<TSMappedType>() == 68);
    assert!(align_of::<TSMappedType>() == 4);
    assert!(offset_of!(TSMappedType, node_id) == 56);
    assert!(offset_of!(TSMappedType, span) == 0);
    assert!(offset_of!(TSMappedType, key) == 8);
    ...
    assert!(offset_of!(TSMappedType, optional) == 64);
    assert!(offset_of!(TSMappedType, readonly) == 65);
    assert!(offset_of!(TSMappedType, scope_id) == 60);
