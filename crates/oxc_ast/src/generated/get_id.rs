...
impl Expression<'_> {
    ...
    pub fn node_id(&self) -> NodeId {
        match self.kind() {
            ExpressionKind::BooleanLiteral(it) => it.node_id(),
            ExpressionKind::NullLiteral(it) => it.node_id(),
            ExpressionKind::NumericLiteral(it) => it.node_id(),
            ExpressionKind::BigIntLiteral(it) => it.node_id(),
            ExpressionKind::RegExpLiteral(it) => it.node_id(),
            ExpressionKind::StringLiteral(it) => it.node_id(),
            ExpressionKind::TemplateLiteral(it) => it.node_id(),
            ExpressionKind::Identifier(it) => it.node_id(),
            ExpressionKind::Super(it) => it.node_id(),
            ExpressionKind::ArrayExpression(it) => it.node_id(),
            ExpressionKind::ArrowFunctionExpression(it) => it.node_id(),
            ExpressionKind::AssignmentExpression(it) => it.node_id(),
            ExpressionKind::AwaitExpression(it) => it.node_id(),
            ExpressionKind::BinaryExpression(it) => it.node_id(),
            ExpressionKind::CallExpression(it) => it.node_id(),
            ExpressionKind::ChainExpression(it) => it.node_id(),
            ExpressionKind::ClassExpression(it) => it.node_id(),
            ExpressionKind::ConditionalExpression(it) => it.node_id(),
            ExpressionKind::FunctionExpression(it) => it.node_id(),
            ExpressionKind::ImportExpression(it) => it.node_id(),
            ExpressionKind::LogicalExpression(it) => it.node_id(),
            ExpressionKind::NewExpression(it) => it.node_id(),
            ExpressionKind::ObjectExpression(it) => it.node_id(),
            ExpressionKind::ParenthesizedExpression(it) => it.node_id(),
            ExpressionKind::SequenceExpression(it) => it.node_id(),
            ExpressionKind::TaggedTemplateExpression(it) => it.node_id(),
            ExpressionKind::ThisExpression(it) => it.node_id(),
            ExpressionKind::UnaryExpression(it) => it.node_id(),
            ExpressionKind::UpdateExpression(it) => it.node_id(),
            ExpressionKind::YieldExpression(it) => it.node_id(),
            ExpressionKind::PrivateInExpression(it) => it.node_id(),
            ExpressionKind::ImportMeta(it) => it.node_id(),
            ExpressionKind::NewTarget(it) => it.node_id(),
            ExpressionKind::JSXElement(it) => it.node_id(),
            ExpressionKind::JSXFragment(it) => it.node_id(),
            ExpressionKind::TSAsExpression(it) => it.node_id(),
            ExpressionKind::TSSatisfiesExpression(it) => it.node_id(),
            ExpressionKind::TSTypeAssertion(it) => it.node_id(),
            ExpressionKind::TSNonNullExpression(it) => it.node_id(),
            ExpressionKind::TSInstantiationExpression(it) => it.node_id(),
            ExpressionKind::V8IntrinsicExpression(it) => it.node_id(),
            ExpressionKind::ComputedMemberExpression(it) => it.node_id(),
            ExpressionKind::StaticMemberExpression(it) => it.node_id(),
            ExpressionKind::PrivateFieldExpression(it) => it.node_id(),
        }
    }
}
...
impl MemberExpression<'_> {
    ...
    pub fn node_id(&self) -> NodeId {
        match self.kind() {
            MemberExpressionKind::ComputedMemberExpression(it) => it.node_id(),
            MemberExpressionKind::StaticMemberExpression(it) => it.node_id(),
            MemberExpressionKind::PrivateFieldExpression(it) => it.node_id(),
        }
    }
}
