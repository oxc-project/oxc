...
impl<'a> ModuleExportName<'a> {
    ...
    /// names to be well-formed Unicode, and the parser rejects a string form
    ...
    /// ## Panics
    ///
    /// Panics if a string form contains a lone surrogate. The parser rejects
    /// such a name, so this only happens for an AST built by other means.
