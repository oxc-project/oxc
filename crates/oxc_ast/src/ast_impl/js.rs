...
impl<'a> BindingPattern<'a> {
    ...
    ///
    /// This method is more efficient than [`get_binding_identifiers`] followed by [`Iterator::all`]
    /// when you only need to check a condition, as it does not allocate a `Vec` and can
    /// short-circuit on the first `false` result.
    ///
    /// If the pattern contains no binding identifiers, returns `true` (vacuous truth).
