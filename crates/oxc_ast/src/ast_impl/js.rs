...
impl<'a> Expression<'a> {
    ...
    pub fn is_typescript_syntax(&self) -> bool {
        self.is_ts_as_expression()
            || self.is_ts_satisfies_expression()
            || self.is_ts_type_assertion()
            || self.is_ts_non_null_expression()
            || self.is_ts_instantiation_expression()
    }
    ...
    pub fn is_literal(&self) -> bool {
        ...
        self.is_boolean_literal()
            ...
            || self.is_big_int_literal()
            || self.is_reg_exp_literal()
            || self.is_string_literal()
    }

    /// Returns `true` for [string](StringLiteral) and [template](TemplateLiteral) literals.
    pub fn is_string_or_template_literal(&self) -> bool {
        self.is_string_literal() || self.is_template_literal()
    }

    /// Return `true` if the expression is a plain template.
    pub fn is_no_substitution_template(&self) -> bool {
        self.as_template_literal().is_some_and(|e| e.is_no_substitution_template())
    }

    /// Returns `true` for [numeric](NumericLiteral) and [big int](BigIntLiteral) literals.
    pub fn is_number_literal(&self) -> bool {
        self.is_numeric_literal() || self.is_big_int_literal()
    }

    // is_big_int_literal() is now generated by define_expression_variants! macro
    ...
    pub fn is_void_0(&self) -> bool {
        self.as_unary_expression().is_some_and(|expr| {
            expr.operator == UnaryOperator::Void
                && expr.argument.as_numeric_literal().is_some_and(|lit| lit.value == 0.0)
        })
    }
    ...
    pub fn without_parentheses_mut(&mut self) -> &mut Self {
        let mut expr: *mut Self = self;
        // SAFETY: We follow the chain of ParenthesizedExpression.expression references.
        // Each step replaces `expr` with an inner pointer, so there is no aliasing.
        unsafe {
            while let Some(paran_expr) = (*expr).as_parenthesized_expression_mut() {
    ...
    pub fn is_specific_member_access(&self, object: &str, property: &str) -> bool {
        ...
        if inner.is_member_expression() {
            return inner.to_member_expression().is_specific_member_access(object, property);
        }
        match inner.kind() {
    ...
    pub fn into_inner_expression(self) -> Expression<'a> {
        ...
        loop {
            if expr.is_parenthesized_expression() {
                expr = expr.into_parenthesized_expression().unbox().expression;
            } else if expr.is_ts_as_expression() {
                expr = expr.into_ts_as_expression().unbox().expression;
            } else if expr.is_ts_satisfies_expression() {
                expr = expr.into_ts_satisfies_expression().unbox().expression;
            } else if expr.is_ts_instantiation_expression() {
                expr = expr.into_ts_instantiation_expression().unbox().expression;
            } else if expr.is_ts_non_null_expression() {
                expr = expr.into_ts_non_null_expression().unbox().expression;
            } else if expr.is_ts_type_assertion() {
                expr = expr.into_ts_type_assertion().unbox().expression;
            } else {
                break;
            }
        }
    ...
    pub fn get_inner_expression_mut(&mut self) -> &mut Expression<'a> {
        let mut expr: *mut Self = self;
        // SAFETY: We walk a chain of wrapper expressions (Parenthesized, TS type wrappers).
        // Each step replaces `expr` with the inner `expression` field, so no aliasing occurs.
        unsafe {
            loop {
                expr = match (*expr).kind_mut() {
                    ExpressionKindMut::ParenthesizedExpression(e) => &mut e.expression,
                    ExpressionKindMut::TSAsExpression(e) => &mut e.expression,
                    ExpressionKindMut::TSSatisfiesExpression(e) => &mut e.expression,
                    ExpressionKindMut::TSInstantiationExpression(e) => &mut e.expression,
                    ExpressionKindMut::TSNonNullExpression(e) => &mut e.expression,
                    ExpressionKindMut::TSTypeAssertion(e) => &mut e.expression,
                    _ => break,
                };
            }
            &mut *expr
    ...
    pub fn into_chain_element(self) -> Option<ChainElement<'a>> {
        if self.is_static_member_expression() {
            Some(ChainElement::StaticMemberExpression(self.into_static_member_expression()))
        } else if self.is_computed_member_expression() {
            Some(ChainElement::ComputedMemberExpression(self.into_computed_member_expression()))
        } else if self.is_private_field_expression() {
            Some(ChainElement::PrivateFieldExpression(self.into_private_field_expression()))
        } else if self.is_call_expression() {
            Some(ChainElement::CallExpression(self.into_call_expression()))
        } else if self.is_ts_non_null_expression() {
            Some(ChainElement::TSNonNullExpression(self.into_ts_non_null_expression()))
        } else {
            None
        }
    }
    ...
    pub fn is_function(&self) -> bool {
        self.is_function_expression() || self.is_arrow_function_expression()
    }
    ...
    // is_call_expression() and is_super() are now generated by define_expression_variants! macro
    ...
    pub fn is_call_like_expression(&self) -> bool {
        self.is_call_expression() || self.is_new_expression() || self.is_import_expression()
    }

    /// Returns `true` if this [`Expression`] is a [`BinaryExpression`] or [`LogicalExpression`].
    pub fn is_binaryish(&self) -> bool {
        self.is_binary_expression() || self.is_logical_expression()
    }
    ...
    pub fn get_member_expr(&self) -> Option<MemberExpression<'a>> {
        let inner = self.get_inner_expression();
        if let Some(chain_expr) = inner.as_chain_expression() {
            // ChainElement is still an enum — reconstruct by value from the ref
            chain_expr.expression.as_member_expression().map(|me| match me {
                MemberExpression::ComputedMemberExpression(b) => {
                    MemberExpression::ComputedMemberExpression(unsafe {
                        oxc_allocator::Box::from_non_null(oxc_allocator::Box::as_non_null(b))
                    })
                }
                MemberExpression::StaticMemberExpression(b) => {
                    MemberExpression::StaticMemberExpression(unsafe {
                        oxc_allocator::Box::from_non_null(oxc_allocator::Box::as_non_null(b))
                    })
                }
                MemberExpression::PrivateFieldExpression(b) => {
                    MemberExpression::PrivateFieldExpression(unsafe {
                        oxc_allocator::Box::from_non_null(oxc_allocator::Box::as_non_null(b))
                    })
                }
            })
        } else {
            inner.as_member_expression()
        }
    }
    ...
    pub fn is_require_call(&self) -> bool {
        self.as_call_expression().is_some_and(|call_expr| call_expr.is_require_call())
    }
    ...
    pub fn is_entity_name_expression(&self) -> bool {
        ...
        let inner = self.without_parentheses();
        inner.is_identifier()
            || inner.is_this_expression()
    ...
    pub fn is_jsx(&self) -> bool {
        self.is_jsx_element() || self.is_jsx_fragment()
    }
}
...
impl<'a> MemberExpression<'a> {
    ...
    pub fn through_optional_is_specific_member_access(&self, object: &str, property: &str) -> bool {
        let obj_expr = self.object().without_parentheses();
        let object_matches = if let Some(chain) = obj_expr.as_chain_expression() {
            match chain.expression.member_expression() {
                None => false,
                Some(member_expr) => {
            ...
            obj_expr.is_specific_id(object)
        };
...
impl<'a> ChainElement<'a> {
    /// Returns a reference to the member expression if this chain element contains one.
    pub fn member_expression(&self) -> Option<&MemberExpression<'a>> {
        match self {
            ChainElement::TSNonNullExpression(e) => {
                // TSNonNullExpression wraps an Expression; if it's a member expression,
...
impl<'a> From<ChainElement<'a>> for Expression<'a> {
    fn from(value: ChainElement<'a>) -> Self {
        match value {
            ChainElement::CallExpression(e) => Expression::call_expression(e),
            ChainElement::TSNonNullExpression(e) => Expression::ts_non_null_expression(e),
...
impl<'a> CallExpression<'a> {
    ...
    pub fn callee_name(&self) -> Option<&str> {
        if let Some(ident) = self.callee.as_identifier() {
            ...
            self.callee.as_member_expression().and_then(|me| me.static_property_name())
        }
    }
