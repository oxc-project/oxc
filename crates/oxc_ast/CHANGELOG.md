...
- Add `oxc/no-const-enum` rule (#3435)

### Bug Fixes

- UsingDeclaration is not a typescript syntax (#3482)

### Refactor

- Update scope attrs (#3494)
- Move scope from `TSModuleBlock` to `TSModuleDeclaration` (#3488)
- Rename function params (#3487)
- Remove defunct hashing of `Span` (#3486)
...
- Add `callee_name` method to the `CallExpression`. (#3076)

### Bug Fixes

- Set `ScopeFlags::Function` bit for class methods (#3277)
- Create scopes for functions (#3273)
- Create scope for function nested in class method (#3234)
- Correctly parse cls.fn<C> = x (#3208)
- Do not include `trailing_comma` in JSON AST (#3073)

### Documentation

- Document enum inheritance (#3192)

### Refactor

- Order AST type fields in visitation order (#3228)
- Add `is_strict` methods (#3227)
- Fix doc comments for methods generated by `inherit_variants!` macro (#3195)
- Avoid an `String::from_utf8` over head during serialization (#3145)
