...
pub mod walk_mut {
    ...
    pub fn walk_expression<'a, V: VisitMut<'a>>(visitor: &mut V, it: &mut Expression<'a>) {
        ...
        match it.kind_mut() {
            ...
            ExpressionKindMut::ImportMeta(it) => visitor.visit_import_meta(it),
            ExpressionKindMut::NewTarget(it) => visitor.visit_new_target(it),
    ...
    pub fn walk_member_expression<'a, V: VisitMut<'a>>(
        ...
        // No `AstType` for this type
        match it.kind_mut() {
            MemberExpressionKindMut::ComputedMemberExpression(it) => {
                visitor.visit_computed_member_expression(it)
            }
            MemberExpressionKindMut::StaticMemberExpression(it) => {
                visitor.visit_static_member_expression(it)
            }
            MemberExpressionKindMut::PrivateFieldExpression(it) => {
    ...
    pub fn walk_arguments<'a, V: VisitMut<'a>>(
        ...
        for el in it {
            match el {
                ...
                oxc_ast::ast::Argument::Expression(expr) => {
