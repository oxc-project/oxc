...
impl<'a> VisitMut<'a> for Utf8ToUtf16Converter<'_> {
    ...
    fn visit_identifier_name(&mut self, it: &mut IdentifierName<'a>) {
        let mut start = it.span.start(); self.convert_offset(&mut start); it.span.set_start(start);
        walk_mut::walk_identifier_name(self, it);
        let mut end = it.span.end(); self.convert_offset(&mut end); it.span.set_end(end);
    }
    ...
    fn visit_object_property(&mut self, prop: &mut ObjectProperty<'a>) {
        let mut start = prop.span.start(); self.convert_offset(&mut start); prop.span.set_start(start);
        ...
        let mut end = prop.span.end(); self.convert_offset(&mut end); prop.span.set_end(end);
    }
    ...
    fn visit_binding_rest_element(&mut self, rest_element: &mut BindingRestElement<'a>) {
        let mut start = rest_element.span.start(); self.convert_offset(&mut start); rest_element.span.set_start(start);
        ...
        let mut end = rest_element.span.end(); self.convert_offset(&mut end); rest_element.span.set_end(end);
    }
    ...
    fn visit_export_named_declaration(&mut self, decl: &mut ExportNamedDeclaration<'a>) {
        ...
        if let Some(Declaration::ClassDeclaration(class)) = &mut decl.declaration {
            ...
            let mut start = decl.span.start(); self.convert_offset(&mut start); decl.span.set_start(start);
            walk_mut::walk_export_named_declaration(self, decl);
            let mut end = decl.span.end(); self.convert_offset(&mut end); decl.span.set_end(end);
        }
    }
    ...
    fn visit_export_specifier(&mut self, specifier: &mut ExportSpecifier<'a>) {
        let mut start = specifier.span.start(); self.convert_offset(&mut start); specifier.span.set_start(start);
        ...
        let mut end = specifier.span.end(); self.convert_offset(&mut end); specifier.span.set_end(end);
    }
    ...
    fn visit_template_literal(&mut self, lit: &mut TemplateLiteral<'a>) {
        let mut start = lit.span.start(); self.convert_offset(&mut start); lit.span.set_start(start);
        ...
        let mut end = lit.span.end(); self.convert_offset(&mut end); lit.span.set_end(end);
    }
