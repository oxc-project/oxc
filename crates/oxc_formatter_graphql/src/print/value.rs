//! Value printers, including string re-encoding.
//!
...
//! cooking (GraphQL spec string/block-string value algorithms) and Prettier's
//! re-encoding.
...
use oxc_formatter_core::{
    ...
    builders::{block_indent, group, hard_line_break, soft_block_indent_with_maybe_space, text},
    write,
};
...
use super::{
    GraphqlFormatter, SeparatorKind, closing_token_start, common, format_with, node_text,
    write_sequence,
};
...
pub fn write_value(value: &cst::Value, f: &mut GraphqlFormatter<'_, '_>) {
    match value {
        ...
        cst::Value::StringValue(sv) => write_string_value(sv, f),
...
fn write_list_value<'a>(list: &cst::ListValue, f: &mut GraphqlFormatter<'_, 'a>) {
    ...
    write!(f, "[");
    if values.is_empty() {
        let dangling = f.context().comments().take_before(r_brack);
