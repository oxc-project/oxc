...
/// ```
///
/// Prefix the trait with `!` to ask the opposite question:
///
/// ```
/// # use oxc_data_structures::types::implements;
...
/// ```
///
/// `implements!(T: !Trait)` is exactly `!implements!(T: Trait)`, but reads better.
///
...
///
/// Note that `!` here is the ordinary boolean negation, applied to the answer. It is unrelated to
/// negative impls (`impl !Send for Foo`), and asserts nothing about *why* the trait is absent.
///
...
///
/// The `!Trait` form is a thin wrapper - it delegates to the positive form and negates the result.
///
/// [`DoesNotImpl`]: __private::DoesNotImpl
#[macro_export]
macro_rules! implements {
    ($ty:ty: !$trait_:path) => {
        !$crate::types::implements!($ty: $trait_)
    };
...
mod tests {
    ...
    fn negated() {
        ...
        // Negating a trait which *is* implemented answers `false`
        assert!(!implements!(u32: !Copy));
    }
