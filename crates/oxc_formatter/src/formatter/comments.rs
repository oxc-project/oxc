...
impl<'a> Comments<'a> {
    ...
    /// A suppression line comment ending the operator's line (`const a = // prettier-ignore` + newline)
    /// is the left side's trailing run for placement (it keeps its line) and still targets the right-hand side:
    /// `AssignmentLike::write_right` marks the node it targets, whose own `fmt` then takes the suppressed path
    /// (a union re-keys the mark to its first member, see `TSUnionType::write`).
