...
impl<K: std::hash::Hash + Eq, V> CommentsMap<K, V> {
    pub fn new() -> Self {
        Self { index: FxHashMap::default(), parts: Vec::new(), out_of_order: Vec::new() }
    }
        ...
        V: Clone,
    {
        match self.index.get_mut(&key) {
            None => {
                ...
                self.index
                    .insert(key, Entry::InOrder(InOrderEntry::leading(start..self.parts.len())));
            }
        ...
        V: Clone,
    {
        match self.index.get_mut(&key) {
            None => {
                ...
                self.index
                    .insert(key, Entry::InOrder(InOrderEntry::dangling(start..self.parts.len())));
            }
        ...
        V: Clone,
    {
        match self.index.get_mut(&key) {
            None => {
                ...
                self.index
                    .insert(key, Entry::InOrder(InOrderEntry::trailing(start..self.parts.len())));
            }
    ...
    pub fn all_parts(&self) -> impl Iterator<Item = &V> {
        self.index.values().flat_map(|entry| PartsIterator::from_entry(entry, self))
    }
}
...
pub(super) enum PartsIterator<'a, V> {
    ...
    Leading { leading: std::slice::Iter<'a, V>, dangling: &'a [V], trailing: &'a [V] },
    ...
    Dangling { dangling: std::slice::Iter<'a, V>, trailing: &'a [V] },
}
...
impl<'a, V> Iterator for PartsIterator<'a, V> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self {
            ...
            PartsIterator::Leading { leading, dangling, trailing } => match leading.next() {
                Some(next) => Some(next),
                None if !dangling.is_empty() => {
                    let mut dangling_iterator = dangling.iter();
                    let next = dangling_iterator.next().unwrap();
                    *self = PartsIterator::Dangling { dangling: dangling_iterator, trailing };
                    Some(next)
                }
                None => {
            ...
            PartsIterator::Dangling { dangling, trailing } => {
                if let Some(next) = dangling.next() {
                    Some(next)
                } else {
                    let mut trailing_iterator = trailing.iter();
                    let next = trailing_iterator.next();
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        match self {
            ...
            PartsIterator::Leading { leading, dangling, trailing } => {
        ...
        Self: Sized,
    {
        match self {
            ...
            PartsIterator::Leading { leading, dangling, trailing } => {
                trailing.last().or_else(|| dangling.last()).or_else(|| leading.last())
            }
            PartsIterator::Dangling { dangling, trailing } => {
...
impl InOrderEntry {
    ...
    fn range(&self) -> Range<usize> {
        self.leading_start.value()
            ..self.trailing_end.or(self.trailing_start).unwrap_or(self.dangling_start).value()
    }
}
...
mod tests {
    ...
    fn leading_dangling_trailing() {
        ...
        assert_eq!(map.parts(&"a").copied().collect::<Vec<_>>(), vec![1, 2, 3, 4]);
    }
    ...
    fn empty() {
        ...
        assert_eq!(map.parts(&"a").copied().collect::<Vec<_>>(), Vec::<i32>::new());
    }
    ...
    fn dangling_leading() {
        ...
        assert_eq!(map.parts(&"a").copied().collect::<Vec<_>>(), vec![2, 1, 3, 4]);
    ...
    fn trailing_leading() {
        ...
        assert_eq!(map.parts(&"a").copied().collect::<Vec<_>>(), vec![2, 3, 1, 4]);
