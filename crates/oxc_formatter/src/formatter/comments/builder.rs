// use super::super::source_map::{DeletedRangeEntry, DeletedRanges};
// use super::super::{TextRange, TextSize};
...
pub(super) struct CommentsBuilderVisitor {}
...
// ) -> (CommentsMap<SyntaxElementKey, SourceComment<Style::Language>>, FxHashSet<SyntaxElementKey>)
// {
...
// } else {
// (self.pending_comments.len(), 0, CommentTextPosition::SameLine, None)
// };
...
// comment.lines_after =
// comments.peek().map_or(lines_before, |(_, next)| next.lines_before);
// }
// }
...
// if let Some(parens_source_range) =
// self.parentheses.r_paren_source_range(piece.text_range().end())
// {
...
// self.parentheses.outer_most_parenthesized_node(last_token, parens_source_range);
...
// fn finish(
// self,
// ) -> (CommentsMap<SyntaxElementKey, SourceComment<L>>, FxHashSet<SyntaxElementKey>) {
...
// fn default() -> Self {
// Self { comments: CommentsMap::new(), skipped: FxHashSet::default() }
// }
// }
...
mod tests {
  ...
  b
        ...
        assert_eq!(
            comment.preceding_node().map(|node| node.text_trimmed().to_string()).as_deref(),
            Some("a: 'a'")
        );
        assert_eq!(
            comment.following_node().map(|node| node.text_trimmed().to_string()).as_deref(),
            Some("b")
        );
        assert_eq!(comment.enclosing_node().kind(), JsSyntaxKind::JS_OBJECT_EXPRESSION);

        let b = root.descendants().find_map(JsShorthandPropertyObjectMember::cast).unwrap();
  ...
  b
        ...
        let a = root.descendants().find_map(JsPropertyObjectMember::cast).unwrap();
  ...
  b
    ...
    fn dangling_arrow() {
        ...
        assert_eq!(decorated.enclosing_node().kind(), JsSyntaxKind::JS_PARAMETERS);
...
b;"#;
        ...
        let parenthesized = root.descendants().find_map(JsParenthesizedExpression::cast).unwrap();

        let reference_identifier =
            root.descendants().find_map(JsReferenceIdentifier::cast).unwrap();
        ...
        assert_eq!(argument_trailing.text_position(), CommentTextPosition::OwnLine);
        assert_eq!(argument_trailing.lines_before(), 1);
        assert_eq!(argument_trailing.lines_after(), 1);
        assert_eq!(
            argument_trailing.preceding_node().map(|preceding| preceding.kind()),
            Some(JsSyntaxKind::JS_IDENTIFIER_EXPRESSION)
        );
        assert_eq!(argument_trailing.following_node(), None);
        assert_eq!(argument_trailing.enclosing_node().kind(), JsSyntaxKind::JS_UNARY_EXPRESSION);

        let identifier_leading = &decorated_comments[1];
        assert_eq!(identifier_leading.text_position(), CommentTextPosition::OwnLine);
        ...
        assert_eq!(identifier_leading.enclosing_node().kind(), JsSyntaxKind::JS_MODULE);

        let unary = transformed.descendants().find_map(JsUnaryExpression::cast).unwrap();
        assert!(!comments.trailing(&unary.argument().unwrap().syntax().key()).is_empty());
    }
...
/* test */"#,
        ...
        assert_eq!(first.following_node().map(SyntaxNode::kind), Some(JsSyntaxKind::JS_MODULE));
    ...
    fn r_paren_inside_list() {
        ...
        assert_eq!(comment.lines_before(), 0);
        assert_eq!(comment.lines_after(), 0);
        assert_eq!(comment.preceding_node().kind().unwrap(), JsSyntaxKind::JS_SEQUENCE_EXPRESSION);
        assert_eq!(comment.following_node(), None);
        assert_eq!(comment.enclosing_node().kind(), JsSyntaxKind::JS_PARENTHESIZED_EXPRESSION);

        let sequence = root.descendants().find_map(JsSequenceExpression::cast).unwrap();
