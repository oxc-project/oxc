...
impl<'a> Printer<'a> {
    ...
    fn flush_line_suffixes(
        ...
        if suffixes.len() > 0 {
            ...
            // Simulate extending the indent stack with suffix indents
            // Original: indent_stack.extend(suffix_indents.drain().rev())
            // This makes suffix indents available in reverse order as we process suffixes
            let saved_suffix_indents: Vec<Indention> = suffix_indents.drain(..).rev().collect();
            let mut suffix_iter = saved_suffix_indents.iter();
