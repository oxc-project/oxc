...
use super::{
    ...
    serialize::{
        format_default_value, is_named_generic_tag, join_words, push_indented_desc,
        strip_default_is_suffix, wrap_type_expression,
    },
...
pub(super) fn format_type_name_comment_tag(
    ...
    let remaining_desc = if let Some(rest) = rest_of_desc {
        super::serialize::join_iter(rest.lines().map(str::trim), "\n")
    } else {
        String::new()
    };
