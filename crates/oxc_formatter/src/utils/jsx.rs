...
pub fn get_wrap_state(parent: &AstNodes<'_>) -> WrapState {
    ...
    match parent {
        ...
        | AstNodes::JSXElement(_)
        | AstNodes::JSXFragment(_)
...
impl<'a> Format<'a> for JsxSpace {
    fn fmt(&self, f: &mut Formatter<'_, 'a>) -> FormatResult<()> {
        write!(f, [if_group_breaks(&JsxRawSpace), if_group_fits_on_line(&space())])
    }
}
...
impl<'a> Format<'a> for JsxRawSpace {
    fn fmt(&self, f: &mut Formatter<'_, 'a>) -> FormatResult<()> {
        // Use double quotes for now, could be configurable
        // TODO:
        write!(f, [text(r#"{" "}"#)])
    }
}
...
pub fn is_whitespace_jsx_expression<'a>(
    ...
    _comments: &crate::formatter::comments::Comments<'a>,
) -> bool {
    match &child.expression {
        JSXExpression::StringLiteral(literal) => {
            // Check if it's a single space string
            matches!(literal.value.as_str(), " ")
        }
        _ => false,
    }
}
                ...
                while let Some(chunk) = chunks.next() {
                    match chunk {
                        ...
                        (relative_start, JsxTextChunk::Word(word)) => {
                            let span = Span::new(
                                text.span.start + relative_start as u32,
                                text.span.start + relative_start as u32 + word.len() as u32,
                            );
                            // TODO: correct it

                            builder.entry(JsxChild::Word(JsxWord::new(word, span)));
                        }
                    }
                }
            }
