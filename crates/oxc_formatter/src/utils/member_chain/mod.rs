...
use crate::{
    ...
    best_fitting,
    formatter::{Buffer, Comments, Format, prelude::*},
    utils::{
...
impl<'a> Format<'a, JsFormatContext<'a>> for MemberChain<'a, '_> {
    fn fmt(&self, f: &mut JsFormatter<'_, 'a>) {
        ...
        let format_one_line = js_format_with(|f| {
        ...
        let format_tail = js_format_with(|f| {
        ...
        let format_expanded = js_format_with(|f| write!(f, [self.head, indent(&format_tail)]));
