...
impl<'a> AssignmentLike<'a, '_> {
    ...
    fn layout(
        ...
        if let Some(Expression::CallExpression(call_expression)) =
            &right_expression.map(|e| e.as_ref())
        {
            if call_expression
                .callee
        ...
        if matches!(right_expression.map(|e| e.as_ref()), Some(Expression::StringLiteral(_))) {
            return AssignmentLikeLayout::BreakAfterOperator;
        }
...
fn should_break_after_operator<'a, 'b>(
    right: &'b AstNode<'a, Expression<'a>>,
    f: &Formatter<'_, 'a>,
) -> bool {
...
fn is_poorly_breakable_member_or_call_chain<'a, 'b>(
    expression: &'b AstNode<'a, Expression<'a>>,
    f: &Formatter<'_, 'a>,
) -> bool {
