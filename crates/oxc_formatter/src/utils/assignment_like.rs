...
///
/// <https://github.com/prettier/prettier/blob/a043ac0d733c4d53f980aa73807a63fc914f23bd/src/language-js/print/assignment.js#L278-L310>
fn is_complex_type_arguments<'a>(
    ...
    // NOTE: `f.intern()` would normally advance the comment cursor, but the caller
    // (`is_poorly_breakable_member_or_call_chain`) already guards with
    // `has_comment_in_span(call_expressions[0].span)`, so no comments exist here.
    f.intern(type_arguments).is_some_and(|e| e.will_break())
}
