...
impl<'a> FormatWrite<'a> for AstNode<'a, FormalParameter<'a>> {
    fn write(&self, f: &mut Formatter<'_, 'a>) {
        // Handle own-line comments between the pattern and the initializer value
        // These comments should be printed BEFORE the pattern (Prettier moves them)
        // We must exclude comments that are inside the pattern itself
        let leading_comments = if let Some(initializer) = self.initializer() {
            ...
            // Filter out comments that are inside the pattern (comments whose start is before pattern end)
            let start_index =
        ...
        let content = format_with(|f| {
            // Print leading comments before the pattern if there are any
...
pub fn should_hug_function_parameters<'a>(
    ...
    match &only_parameter.pattern {
        ...
        BindingPattern::ArrayPattern(_) | BindingPattern::ObjectPattern(_) => {
            // If there's an initializer, only hug if it's an empty object/array or identifier
