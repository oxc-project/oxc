...
use crate::{
    format_args,
    formatter::{Buffer, Format, FormatResult, Formatter, prelude::*, write},
    write,
};

use self::{semicolon::OptionalSemicolon, utils::FormatStatementBody};
...
impl<'a> FormatWrite<'a> for Class<'a> {
    fn write(&self, f: &mut Formatter<'_, 'a>) -> FormatResult<()> {
        write!(f, ["class ", self.id, space(), self.body])
    }
}
