...
impl<'a> FormatWrite<'a> for AstNode<'a, JSXClosingElement<'a>> {
    fn write(&self, f: &mut Formatter<'_, 'a>) -> FormatResult<()> {
        ...
        write!(f, ["</", &format_name, line_suffix_boundary(), ">",])
    }
}
    ...
    fn write(&self, f: &mut Formatter<'_, 'a>) -> FormatResult<()> {
        ...
        if matches!(self.parent, AstNodes::JSXElement(_) | AstNodes::JSXFragment(_)) {
            if let JSXExpression::EmptyExpression(_) = self.expression {
                ...
                let should_inline =
                // !comments.has_comments(expression.syntax()) &&
                 (is_conditional_or_binary
                        || should_inline_jsx_expression(&self.expression, comments));
            ...
            let should_inline =
                should_inline_jsx_expression(&self.expression, f.context().comments());
