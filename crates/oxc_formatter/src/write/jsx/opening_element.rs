...
impl<'a, 'b> FormatOpeningElement<'a, 'b> {
    ...
    fn compute_layout(&self, f: &mut Formatter<'_, 'a>) -> OpeningElementLayout {
        ...
        // TODO: Check for comments on name and last attribute
        let name_has_comments = false;
        let last_attribute_has_comments = false;

        if self.is_self_closing && attributes.is_empty() && !name_has_comments {
            OpeningElementLayout::Inline
        } else if attributes.len() == 1
            && attributes.iter().all(|attribute| is_single_line_string_literal_attribute(attribute))
            && !name_has_comments
        {
