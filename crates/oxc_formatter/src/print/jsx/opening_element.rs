...
fn is_multiline_string_literal_attribute(attribute: &JSXAttributeItem<'_>) -> bool {
    ...
    attr.value.as_ref().is_some_and(|value| matches!(value, JSXAttributeValue::StringLiteral(string) if string.value.as_str().is_some_and(|value| value.contains('\n'))))
}
...
fn is_single_line_string_literal_attribute(attribute: &JSXAttributeItem) -> bool {
    as_string_literal_attribute_value(attribute)
        .is_some_and(|string| !string.value.as_str().is_some_and(|value| value.contains('\n')))
}
