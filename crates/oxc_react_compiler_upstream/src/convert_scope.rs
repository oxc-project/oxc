...
fn get_binding_kind(flags: SymbolFlags, semantic: &Semantic, symbol_id: SymbolId) -> BindingKind {
    ...
    match decl_node.kind() {
        ...
        AstKind::TSExternalModuleDeclaration(_) | AstKind::TSNamespaceDeclaration(_) => {
...
fn find_binding_identifier_start(kind: AstKind, name: &str) -> Option<u32> {
    match kind {
        ...
        AstKind::TSExternalModuleDeclaration(_) => None,
        AstKind::TSNamespaceDeclaration(decl) => {
            (decl.id.name.as_str() == name).then_some(decl.id.span.start)
        }
        _ => None,
    }
}
