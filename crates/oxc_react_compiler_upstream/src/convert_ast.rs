...
impl<'a> ConvertCtx<'a> {
    ...
    fn convert_statement(&self, stmt: &oxc::Statement) -> Statement {
        match stmt {
            ...
            oxc::Statement::TSExternalModuleDeclaration(t) => Statement::TSModuleDeclaration(
                self.convert_ts_declaration_passthrough(t.span, false),
            ),
            oxc::Statement::TSNamespaceDeclaration(t) => Statement::TSModuleDeclaration(
                self.convert_ts_declaration_passthrough(t.span, false),
            ),
            ...
            oxc::Statement::ExportDeclaration(e) => {
                Statement::ExportNamedDeclaration(self.convert_export_declaration(e))
            }
            oxc::Statement::ExportNamedDeclaration(e) => {
                Statement::ExportNamedDeclaration(self.convert_export_named_declaration(e))
            }
            oxc::Statement::ExportFromDeclaration(e) => {
                Statement::ExportNamedDeclaration(self.convert_export_from_declaration(e))
            }
    ...
    fn convert_class_declaration(&self, class: &oxc::Class) -> ClassDeclaration {
        ClassDeclaration {
            ...
            super_class: class
                .heritage
                .as_ref()
                .map(|heritage| Box::new(self.convert_expression(&heritage.expression))),
            body: ClassBody {
    ...
    fn convert_export_declaration(
        &self,
        export: &oxc::ExportDeclaration,
    ) -> ExportNamedDeclaration {
        ExportNamedDeclaration {
            base: self.make_base_node(export.span),
            declaration: Some(Box::new(self.convert_declaration(&export.declaration))),
            specifiers: Vec::new(),
            source: None,
            export_kind: None,
            assertions: None,
    ...
    fn convert_export_from_declaration(
        &self,
        export: &oxc::ExportFromDeclaration,
    ) -> ExportNamedDeclaration {
        ExportNamedDeclaration {
            base: self.make_base_node(export.span),
            declaration: None,
            ...
            assertions: None,
            attributes: export.with_clause.as_ref().map(|with_clause| {
                with_clause
                    .with_entries
                    .iter()
                    .map(|entry| self.convert_import_attribute(entry))
                    .collect()
            }),
        }
    }

    fn convert_declaration(&self, declaration: &oxc::Declaration) -> Declaration {
        match declaration {
            oxc::Declaration::VariableDeclaration(variable) => {
                Declaration::VariableDeclaration(self.convert_variable_declaration(variable))
            }
            oxc::Declaration::FunctionDeclaration(function) => {
                if function.body.is_none() {
                    Declaration::TSDeclareFunction(self.convert_ts_declare_function(function))
                } else {
                    Declaration::FunctionDeclaration(self.convert_function_declaration(function))
                }
            }
            oxc::Declaration::ClassDeclaration(class) => {
                Declaration::ClassDeclaration(self.convert_class_declaration(class))
            }
            oxc::Declaration::TSTypeAliasDeclaration(type_alias) => {
                Declaration::TSTypeAliasDeclaration(
                    self.convert_ts_type_alias_declaration(type_alias),
                )
            }
            oxc::Declaration::TSInterfaceDeclaration(interface) => {
                Declaration::TSInterfaceDeclaration(
                    self.convert_ts_interface_declaration(interface),
                )
            }
            oxc::Declaration::TSEnumDeclaration(ts_enum) => {
                Declaration::TSEnumDeclaration(self.convert_ts_enum_declaration(ts_enum))
            }
            oxc::Declaration::TSExternalModuleDeclaration(module) => {
                Declaration::TSModuleDeclaration(
                    self.convert_ts_declaration_passthrough(module.span, false),
                )
            }
            oxc::Declaration::TSNamespaceDeclaration(module) => Declaration::TSModuleDeclaration(
                self.convert_ts_declaration_passthrough(module.span, false),
            ),
            oxc::Declaration::TSImportEqualsDeclaration(import) => {
                Declaration::TSModuleDeclaration(
                    self.convert_ts_declaration_passthrough(import.span, false),
                )
            }
            oxc::Declaration::TSGlobalDeclaration(global) => Declaration::TSModuleDeclaration(
                self.convert_ts_declaration_passthrough(global.span, true),
            ),
        }
    }
    ...
    fn convert_expression(&self, expr: &oxc::Expression) -> Expression {
        match expr {
            ...
            oxc::Expression::ImportMeta(meta) => {
                Expression::MetaProperty(self.convert_meta_property(meta.span, "import", "meta"))
            }
            oxc::Expression::NewTarget(meta) => {
                Expression::MetaProperty(self.convert_meta_property(meta.span, "new", "target"))
            }
    ...
    fn convert_meta_property(&self, span: Span, meta: &str, property: &str) -> MetaProperty {
        MetaProperty {
            base: self.make_base_node(span),
            meta: Identifier {
                base: self.make_base_node(span),
    ...
    fn convert_arrow_function_expression(
        ...
        let (body, expression) = match &arrow.body {
            oxc::ArrowFunctionBody::FunctionBody(body) => {
                (ArrowFunctionBody::BlockStatement(self.convert_function_body(body)), false)
            }
            body => {
                (ArrowFunctionBody::Expression(Box::new(self.convert_expression_like(body))), true)
            }
        };
        ...
        ArrowFunctionExpression {
            ...
            expression: if expression { Some(true) } else { None },
            return_type: arrow
                .return_type
                .as_ref()
        ...
        impl<'a> ExpressionLike for $enum_ty {
            fn convert_with(&self, ctx: &ConvertCtx) -> Expression {
                match self {
                    ...
                    Self::ImportMeta(e) => Expression::MetaProperty(ctx.convert_meta_property(e.span, "import", "meta")),
                    Self::NewTarget(e) => Expression::MetaProperty(ctx.convert_meta_property(e.span, "new", "target")),
...
// ArrowFunctionBody: FunctionBody + INHERIT(Expression<'a>)
impl_expression_like!(oxc::ArrowFunctionBody<'a>, [
    Self::FunctionBody(_) => unreachable!("handled separately")
]);
