...
fn set_statement_span(stmt: &mut oxc::Statement<'_>, span: Span) {
    match stmt {
        ...
        oxc::Statement::ExportDeclaration(d) => *d.span_mut() = span,
        oxc::Statement::ExportNamedDeclaration(d) => *d.span_mut() = span,
        oxc::Statement::ExportFromDeclaration(d) => *d.span_mut() = span,
...
impl<'a> ReverseCtx<'a> {
    ...
    fn convert_statement(&self, stmt: &Statement) -> oxc::Statement<'a> {
        match stmt {
            ...
            Statement::TryStatement(s) => {
                let block = ArenaBox::new_in(self.convert_block_statement(&s.block), self);
                let handler = s
                    .handler
                    .as_ref()
                    .map(|h| ArenaBox::new_in(self.convert_catch_clause(h), self));
                let finalizer = s
                    .finalizer
                    .as_ref()
                    .map(|f| ArenaBox::new_in(self.convert_block_statement(f), self));
            ...
            Statement::ExportNamedDeclaration(d) => {
                ...
                self.convert_export_named_declaration(d)
            }
    ...
    fn convert_catch_clause(&self, clause: &CatchClause) -> oxc::CatchClause<'a> {
        ...
        let body = ArenaBox::new_in(self.convert_block_statement(&clause.body), self);
        oxc::CatchClause::new(SPAN, param, body, self)
    }
    ...
    fn convert_variable_declarator(
        ...
        oxc::VariableDeclarator::new(SPAN, id, type_annotation, init, definite, self)
    }
        ...
        let converted = match expr {
            ...
            Expression::MetaProperty(mp) => {
                if mp.meta.name == "import" && mp.property.name == "meta" {
                    oxc::Expression::new_import_meta(SPAN, self)
                } else {
                    oxc::Expression::new_new_target(SPAN, self)
                }
            }
    ...
    fn convert_function_decl(
        ...
        let mut func = oxc::Function::new(
            ...
            ArenaBox::new_in(params, self),
            None::<ArenaBox<'a, oxc::TSTypeAnnotation<'a>>>,
            Some(ArenaBox::new_in(body, self)),
            self,
        );
    ...
    fn convert_class_declaration(&self, c: &ClassDeclaration) -> oxc::Class<'a> {
        ...
        let heritage = c.super_class.as_ref().map(|super_class| {
            oxc::ClassHeritage::new(self.convert_expression(super_class), None, self)
        });
        let body = oxc::ClassBody::boxed(SPAN, ArenaVec::new_in(self), self);
        oxc::Class::new(
            SPAN,
    ...
    fn convert_arrow_function(&self, arrow: &ArrowFunctionExpression) -> oxc::Expression<'a> {
        ...
        let body = match &*arrow.body {
            ArrowFunctionBody::BlockStatement(block) => oxc::ArrowFunctionBody::FunctionBody(
                ArenaBox::new_in(self.convert_block_to_function_body(block), self),
            ),
            ArrowFunctionBody::Expression(expr) => {
                oxc::ArrowFunctionBody::from(self.convert_expression(expr))
            }
        };
    ...
    fn convert_params_to_formal_parameters(
        ...
        oxc::FormalParameters::new(
            ...
            items_vec,
            rest.map(|rest| ArenaBox::new_in(rest, self)),
            self,
        )
    }
    ...
    fn convert_pattern_to_binding_pattern(&self, pattern: &PatternLike) -> oxc::BindingPattern<'a> {
        match pattern {
            ...
            PatternLike::ObjectPattern(obj) => {
                ...
                oxc::BindingPattern::new_object_pattern(
                    SPAN,
                    props_vec,
                    rest.map(|rest| ArenaBox::new_in(rest, self)),
                    self,
            ...
            PatternLike::ArrayPattern(arr) => {
                ...
                oxc::BindingPattern::new_array_pattern(
                    SPAN,
                    elems_vec,
                    rest.map(|rest| ArenaBox::new_in(rest, self)),
                    self,
    ...
    fn convert_pattern_to_assignment_target(
        ...
        match pattern {
            ...
            PatternLike::ObjectPattern(obj) => {
                ...
                let props_vec = ArenaVec::from_iter_in(properties, self);
                oxc::AssignmentTarget::new_object_assignment_target(
                    SPAN,
                    props_vec,
                    rest.map(|rest| ArenaBox::new_in(rest, self)),
                    self,
            ...
            PatternLike::ArrayPattern(arr) => {
                ...
                let elems_vec = ArenaVec::from_iter_in(elements, self);
                oxc::AssignmentTarget::new_array_assignment_target(
                    SPAN,
                    elems_vec,
                    rest.map(|rest| ArenaBox::new_in(rest, self)),
                    self,
    ...
    fn convert_jsx_element(&self, el: &JSXElement) -> oxc::JSXElement<'a> {
        ...
        oxc::JSXElement::new(
            SPAN,
            ArenaBox::new_in(opening, self),
            children,
            closing.map(|closing| ArenaBox::new_in(closing, self)),
            self,
        )
    }
    ...
    fn convert_export_named_declaration(
        ...
        if let Some(declaration) = declaration {
            oxc::Statement::new_export_declaration(SPAN, declaration, self)
        } else if let Some(source) = source {
            oxc::Statement::new_export_from_declaration(
                SPAN,
                specifiers,
                source,
                export_kind,
            ...
            oxc::Statement::new_export_named_declaration(SPAN, specifiers, export_kind, self)
        }
    }
