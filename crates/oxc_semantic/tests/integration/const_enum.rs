...
use oxc_semantic::{
    NormalizedConstEnumInfo, NormalizedConstEnumMemberInfo, NormalizedConstantValue, Semantic,
    SemanticBuilder,
};
...
fn find_member_by_name<'a>(
    ...
    name: &str,
) -> Option<&'a NormalizedConstEnumMemberInfo> {
    enum_info.members.values().find(|member| member.name == name)
}
...
fn test_const_enum_constant_propagation() {
    ...
    assert_const_enum_value(&f_member.value, "Computed");
}
...
fn test_const_enum_member_access_propagation() {
    ...
    let a_member = find_member_by_name(enum_info, "A").unwrap();
    assert_const_enum_value(&a_member.value, "Computed");
    ...
    let b_member = find_member_by_name(enum_info, "B").unwrap();
    assert_const_enum_value(&b_member.value, "Computed");
    ...
    let c_member = find_member_by_name(enum_info, "C").unwrap();
    assert_const_enum_value(&c_member.value, "Computed");
    ...
    let d_member = find_member_by_name(enum_info, "D").unwrap();
    assert_const_enum_value(&d_member.value, "Computed");
}
