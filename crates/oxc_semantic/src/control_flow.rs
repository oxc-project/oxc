...
pub enum AssignmentValue {
    Constant(AstNodeId),
    // todo: instead of references, after the first reference,
    // every other reference should just refer to the previous
    // reference's register
    Reference(AstNodeId),
    Register(Register),
    Function(AstNodeId),
    ArrowExpression(AstNodeId),
    UnaryExpression(Box<UnaryExpressioneAssignmentValue>),
    Collection(Box<CollectionAssignmentValue>),
    CalleeWithArguments(Box<CalleeWithArgumentsAssignmentValue>),
    ObjectPropertyAccess(Box<ObjectPropertyAccessAssignmentValue>),
    Binary(Box<BinaryAssignmentValue>),
    Update(Box<UpdateAssignmentValue>),
    ImplicitUndefined,
    This,
    Super,
}
...
pub fn print_basic_block(
    basic_block_elements: &Vec<BasicBlockElement>,
    nodes: &AstNodes,
    source_text: &str,
) -> String {
        ...
        match basic_block {
            ...
            BasicBlockElement::Assignment(to, with) => {
                ...
                match with {
                    ...
                    AssignmentValue::Constant(constant) => {
                        let x = nodes.get_node(*constant);
                    ...
                    AssignmentValue::Reference(constant) => {
                        let x = nodes.get_node(*constant);
                        let span = x.kind().span();
                        output.push_str(&format!(
                            "Reference(<{}>)",
                    ...
                    AssignmentValue::Register(register) => {
                        output.push_str(&print_register(*register).to_string());
                    }
                    AssignmentValue::Collection(b) => {
                        let CollectionAssignmentValue { id: _, elements, spreads, collection_type } =
                            &**b;
                        match collection_type {
                            CollectionType::Array => output.push_str("ArrayMadeWithExpressions["),
                            CollectionType::Object => output.push_str("ObjectMadeWithExpressions{"),
                            CollectionType::JSXElement => {
                                output.push_str("JSXElementMadeWithExpressions{");
                            }
                            CollectionType::JSXFragment => {
                                output.push_str("JSXFragmentMadeWithExpressions{");
                            }
                            CollectionType::Class => {
                                output.push_str("ClassMadeWithExpressions{");
                            }
                            CollectionType::TemplateLiteral => {
                                output.push_str("TaggedTemplateMadeWithExpressions{");
                            }
                        }
                        output.push_str(
                            &elements
                        ...
                        match collection_type {
                            CollectionType::Array => output.push(']'),
                            CollectionType::TemplateLiteral
                            | CollectionType::Class
                            | CollectionType::JSXFragment
                            | CollectionType::JSXElement
                            | CollectionType::Object => output.push('}'),
                        }
                    }
                    AssignmentValue::CalleeWithArguments(b) => {
                        let CalleeWithArgumentsAssignmentValue {
                            id: _,
                            callee,
                            arguments,
                            call_type,
                        ...
                        match call_type {
                            CallType::New => output.push_str("new "),
                            CallType::Import
                            | CallType::TaggedTemplate
                            | CallType::CallExpression => {}
                            CallType::Yield => output.push_str("yield "),
                        }
                        match call_type {
                            CallType::TaggedTemplate => {
                                output.push_str("TaggedTemplate(tag=");
                                output.push_str(&print_register(*callee).to_string());
                                output.push_str(", template=");
                                debug_assert!(arguments.len() == 1);
                                let spreads = spreads.clone().into_iter().collect::<HashSet<_>>();
                                output.push_str(
                                    &arguments
                                        .iter()
                                        .enumerate()
                    ...
                    AssignmentValue::ObjectPropertyAccess(b) => {
                        ...
                        output.push_str(&print_register(*access_on).to_string());
                        match access_by {
                            ObjectPropertyAccessBy::PrivateProperty(priv_property) => {
                                if *optional {
                                    output.push('?');
                                }
                                output.push('.');
                                output.push_str(&format!("{priv_property}"));
                            }
                            ObjectPropertyAccessBy::Property(property) => {
                                if *optional {
                                    output.push('?');
                                }
                                output.push('.');
                            ...
                            ObjectPropertyAccessBy::Expression(reg) => {
                                if *optional {
                                    output.push('?');
                                    output.push('.');
                                }
                                output.push_str(&format!("[{}]", print_register(*reg)));
                            }
                        }
                    }
                    ...
                    AssignmentValue::Function(_) => {
                        output.push_str("Function{}");
                    }
                    AssignmentValue::Binary(b) => {
                        let BinaryAssignmentValue { id: _, a, b, operator } = &**b;
                        output.push_str(&format!(
                            "{} {} {}",
                            print_register(*a),
                            match operator {
                                BinaryOp::BinaryOperator(binop) => binop.as_str(),
                                BinaryOp::LogicalOperator(logop) => logop.as_str(),
                                BinaryOp::AssignmentOperator(asmtop) => asmtop.as_str(),
                            },
                            print_register(*b)
                        ));
                    }
                    AssignmentValue::ArrowExpression(_) => {
                        output.push_str("ArrowExpression{}");
                    }
                    AssignmentValue::UnaryExpression(b) => {
                        let UnaryExpressioneAssignmentValue(_, op, a) = &**b;
                        output.push_str(&format!("{} {}", op.as_str(), print_register(*a)));
                    }
                    AssignmentValue::Update(b) => {
                        let UpdateAssignmentValue { id: _, expr, op, prefix } = &**b;
                        if *prefix {
                            output.push_str(&format!("{}{}", op.as_str(), print_register(*expr)));
                        } else {
                            output.push_str(&format!("{}{}", print_register(*expr), op.as_str()));
                        }
                    }
                }
