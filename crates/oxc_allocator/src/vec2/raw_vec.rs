...
impl<'a, T> RawVec<'a, T> {
    ...
    pub fn cap(&self) -> usize {
        if mem::size_of::<T>() == 0 { !0 } else { u32_to_usize(self.cap) }
    }
    ...
    */
    ...
    pub fn grow_one(&mut self) {
        if let Err(err) = self.grow_amortized(u32_to_usize(self.cap), 1) {
            handle_error(err);
        }
    }
