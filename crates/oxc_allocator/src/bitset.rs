...
impl<'alloc> BitSet<'alloc> {
    ...
    pub fn remove(&mut self, bit: usize) {
        if let Some(word) = self.entries.get_mut(bit / USIZE_BITS) {
            *word &= !(1 << (bit % USIZE_BITS));
        }
    }

    /// Perform a union with another bitset.
    pub fn union(&mut self, other: &Self) {
        for (my_word, other_word) in self.entries.iter_mut().zip(other.entries.iter()) {
            *my_word |= *other_word;
        }
    }
}
