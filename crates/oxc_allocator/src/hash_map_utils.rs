...
impl HashMapUtils {
    ...
    pub fn for_diagnostic_grouping<'alloc, K, V>(
        ...
        let capacity = estimated_files.clamp(CapacityHints::SMALL_COLLECTION, CapacityHints::DIAGNOSTIC_FILES);
    ...
    pub fn for_config_paths<'alloc, K, V>(
        ...
        let capacity = (path_count / 2).clamp(CapacityHints::SMALL_COLLECTION, CapacityHints::CONFIG_PATHS);
        HashMap::with_capacity_in(capacity, allocator)
    }
    ...
    pub fn for_symbol_table<'alloc, K, V>(
        ...
        let capacity = estimated_symbols.clamp(CapacityHints::SMALL_COLLECTION, CapacityHints::SYMBOLS_PER_SCOPE);
        HashMap::with_capacity_in(capacity, allocator)
    }
    ...
    pub fn for_references<'alloc, K, V>(
        ...
        let capacity = estimated_refs.clamp(CapacityHints::SMALL_COLLECTION, CapacityHints::REFERENCES_PER_SYMBOL);
        HashMap::with_capacity_in(capacity, allocator)
    }
...
impl<K, V> FxHashMapExt<K, V> for rustc_hash::FxHashMap<K, V> {
    ...
    fn for_config_storage(config_count: usize) -> Self {
        let capacity = (config_count / 2).clamp(CapacityHints::SMALL_COLLECTION, CapacityHints::CONFIG_PATHS);
        Self::with_capacity_and_hasher(capacity, Default::default())
    }
