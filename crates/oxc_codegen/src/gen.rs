...
impl<'a> Gen for Program<'a> {
    fn gen(&self, p: &mut Codegen, ctx: Context) {
        ...
        p.print_leading_comments(self.span.end);
        // if p.comments.len() != 0 {
        // while let Some(comment) = p.comments.next() {
        // println!("*****{}*****", comment.real_span().source_text(p.source_text.unwrap()));
        // }
        // panic!("{:?}", p.source_text);
        // }
    }
