...
use crate::{
    util::{
        accessibilty_to_string, jsx_attribute_name_to_string, jsx_attribute_to_constant_string,
...
pub(super) fn resolve_class_property<'a, 'b: 'a>(
    ...
    match property_name {
        "extended_class_name" => resolve_property_with(contexts, |v| {
            let Some(Expression::Identifier(ref ident)) = v.as_class().unwrap().class.super_class else {return FieldValue::Null};
            ident.name.to_string().into()
        }),
