...
pub use parser_impl::{parse_regexp_literal, StringLiteralParser};

#[cfg(test)]
mod test {
    use crate::parser::reader::string_literal_parser::{
        ast, parse_regexp_literal, ParserOptions, StringLiteralParser,
    };
...
\r\n""#,
            ...
            if let Err(err) =
                StringLiteralParser::new(&allocator, source_text, ParserOptions::default()).parse()
            {
...
without continuation""#,
    ...
    fn should_fail_early_errors() {
        ...
        for source_text in [r#""invalid octal \777""#, r#""invalid non-octal decimal \9""#] {
            ...
            let result = StringLiteralParser::new(
                &allocator,
                source_text,
                ParserOptions { strict_mode: false, ..ParserOptions::default() },
            )
            .parse();
            ...
            let result = StringLiteralParser::new(
                &allocator,
                source_text,
                ParserOptions { strict_mode: true, ..ParserOptions::default() },
            )
            .parse();
    ...
    fn parse_quotes() {
        ...
        let ast = StringLiteralParser::new(&allocator, r#""double""#, options).parse().unwrap();
        assert_eq!(ast.kind, ast::StringLiteralKind::Double);

        let ast = StringLiteralParser::new(&allocator, r"'single'", options).parse().unwrap();
    ...
    fn should_combine_surrogate_pair() {
        ...
        let ast = StringLiteralParser::new(
            &allocator,
            source_text,
            ParserOptions { combine_surrogate_pair: false, ..ParserOptions::default() },
        )
        .parse()
        .unwrap();
        ...
        let ast = StringLiteralParser::new(
            &allocator,
            source_text,
            ParserOptions { combine_surrogate_pair: true, ..ParserOptions::default() },
        )
        .parse()
        .unwrap();
    ...
    fn span_offset() {
        ...
        let ret1 = StringLiteralParser::new(
            &allocator,
            source_text,
        ...
        .parse()
        .unwrap();
        let ret2 = StringLiteralParser::new(
            &allocator,
            source_text,
    ...
    fn restore_span() {
        ...
        let ast = StringLiteralParser::new(
            &allocator,
            source_text,
            ParserOptions { span_offset: 0, combine_surrogate_pair: false, strict_mode: false },
        )
        .parse()
        .unwrap();
        ...
        let ast = StringLiteralParser::new(
            &allocator,
            source_text,
            ParserOptions { span_offset: 0, combine_surrogate_pair: true, strict_mode: false },
        )
        .parse()
        .unwrap();
        ...
        let ast = StringLiteralParser::new(
            &allocator,
            &source_text[3..13],
            ParserOptions { span_offset: 3, ..ParserOptions::default() },
        )
        .parse()
        .unwrap();
