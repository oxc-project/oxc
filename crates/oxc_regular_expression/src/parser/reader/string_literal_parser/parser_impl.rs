...
impl<'a> StringLiteralParser<'a> {
    ...
    fn handle_code_point(
        ...
        // If the code point is in the BMP, or if forced
        if combine_surrogate_pair || (0..=0xffff).contains(&cp) {
            body.push(ast::CodePoint { span, value: cp });
        } else {
            // Otherwise, split the code point into a surrogate pair, the same span is shared
            let (lead, trail) =
    ...
    // ```
    pub fn parse(mut self) -> Result<ast::StringLiteral<'a>> {
    ...
    fn consume_octal_digit(&mut self) -> Option<u32> {
        if let Some(ch) = self.peek().filter(char::is_ascii_digit).filter(|&ch| ch < '8') {
            self.advance();
