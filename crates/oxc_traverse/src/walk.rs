...
pub(crate) unsafe fn walk_program<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_program(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::ProgramDirectives(ancestor::ProgramWithoutDirectives(node)));
    for item in (*((node as *mut u8).add(ancestor::OFFSET_PROGRAM_DIRECTIVES)
        as *mut Vec<Directive>))
        .iter_mut()
    {
        walk_directive(traverser, item as *mut _);
    }
    if let Some(field) =
        &mut *((node as *mut u8).add(ancestor::OFFSET_PROGRAM_HASHBANG) as *mut Option<Hashbang>)
    {
        traverser.ctx().retag_stack(AncestorType::ProgramHashbang);
        walk_hashbang(traverser, field as *mut _);
    }
    traverser.ctx().retag_stack(AncestorType::ProgramBody);
    walk_statements(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_PROGRAM_BODY) as *mut Vec<Statement>,
    );
    traverser.ctx().pop_stack();
    traverser.exit_program(&mut *node);
}
...
pub(crate) unsafe fn walk_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_expression(&mut *node);
    match &mut *node {
        Expression::BooleanLiteral(node) => {
            walk_boolean_literal(traverser, (&mut **node) as *mut _)
        }
        Expression::NullLiteral(node) => walk_null_literal(traverser, (&mut **node) as *mut _),
        Expression::NumericLiteral(node) => {
            walk_numeric_literal(traverser, (&mut **node) as *mut _)
        }
        Expression::BigintLiteral(node) => walk_big_int_literal(traverser, (&mut **node) as *mut _),
        Expression::RegExpLiteral(node) => walk_reg_exp_literal(traverser, (&mut **node) as *mut _),
        Expression::StringLiteral(node) => walk_string_literal(traverser, (&mut **node) as *mut _),
        Expression::TemplateLiteral(node) => {
            walk_template_literal(traverser, (&mut **node) as *mut _)
        }
        Expression::Identifier(node) => {
            walk_identifier_reference(traverser, (&mut **node) as *mut _)
        }
        Expression::MetaProperty(node) => walk_meta_property(traverser, (&mut **node) as *mut _),
        Expression::Super(node) => walk_super(traverser, (&mut **node) as *mut _),
        Expression::ArrayExpression(node) => {
            walk_array_expression(traverser, (&mut **node) as *mut _)
        }
        Expression::ArrowFunctionExpression(node) => {
            walk_arrow_function_expression(traverser, (&mut **node) as *mut _)
        }
        Expression::AssignmentExpression(node) => {
            walk_assignment_expression(traverser, (&mut **node) as *mut _)
        }
        Expression::AwaitExpression(node) => {
            walk_await_expression(traverser, (&mut **node) as *mut _)
        }
        Expression::BinaryExpression(node) => {
            walk_binary_expression(traverser, (&mut **node) as *mut _)
        }
        Expression::CallExpression(node) => {
            walk_call_expression(traverser, (&mut **node) as *mut _)
        }
        Expression::ChainExpression(node) => {
            walk_chain_expression(traverser, (&mut **node) as *mut _)
        }
        Expression::ClassExpression(node) => walk_class(traverser, (&mut **node) as *mut _),
        Expression::ConditionalExpression(node) => {
            walk_conditional_expression(traverser, (&mut **node) as *mut _)
        }
        Expression::FunctionExpression(node) => walk_function(traverser, (&mut **node) as *mut _),
        Expression::ImportExpression(node) => {
            walk_import_expression(traverser, (&mut **node) as *mut _)
        }
        Expression::LogicalExpression(node) => {
            walk_logical_expression(traverser, (&mut **node) as *mut _)
        }
        Expression::NewExpression(node) => walk_new_expression(traverser, (&mut **node) as *mut _),
        Expression::ObjectExpression(node) => {
            walk_object_expression(traverser, (&mut **node) as *mut _)
        }
        Expression::ParenthesizedExpression(node) => {
            walk_parenthesized_expression(traverser, (&mut **node) as *mut _)
        }
        Expression::SequenceExpression(node) => {
            walk_sequence_expression(traverser, (&mut **node) as *mut _)
        }
        Expression::TaggedTemplateExpression(node) => {
            walk_tagged_template_expression(traverser, (&mut **node) as *mut _)
        }
        Expression::ThisExpression(node) => {
            walk_this_expression(traverser, (&mut **node) as *mut _)
        }
        Expression::UnaryExpression(node) => {
            walk_unary_expression(traverser, (&mut **node) as *mut _)
        }
        Expression::UpdateExpression(node) => {
            walk_update_expression(traverser, (&mut **node) as *mut _)
        }
        Expression::YieldExpression(node) => {
            walk_yield_expression(traverser, (&mut **node) as *mut _)
        }
        Expression::PrivateInExpression(node) => {
            walk_private_in_expression(traverser, (&mut **node) as *mut _)
        }
        Expression::JSXElement(node) => walk_jsx_element(traverser, (&mut **node) as *mut _),
        Expression::JSXFragment(node) => walk_jsx_fragment(traverser, (&mut **node) as *mut _),
        Expression::TSAsExpression(node) => {
            walk_ts_as_expression(traverser, (&mut **node) as *mut _)
        }
        Expression::TSSatisfiesExpression(node) => {
            walk_ts_satisfies_expression(traverser, (&mut **node) as *mut _)
        }
        Expression::TSTypeAssertion(node) => {
            walk_ts_type_assertion(traverser, (&mut **node) as *mut _)
        }
        Expression::TSNonNullExpression(node) => {
            walk_ts_non_null_expression(traverser, (&mut **node) as *mut _)
        }
        Expression::TSInstantiationExpression(node) => {
            walk_ts_instantiation_expression(traverser, (&mut **node) as *mut _)
        }
        Expression::ComputedMemberExpression(_)
        | Expression::StaticMemberExpression(_)
        | Expression::PrivateFieldExpression(_) => {
            walk_member_expression(traverser, node as *mut _)
        }
    }
    traverser.exit_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_identifier_name<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_identifier_name(&mut *node);
    traverser.exit_identifier_name(&mut *node);
}
...
pub(crate) unsafe fn walk_identifier_reference<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_identifier_reference(&mut *node);
    traverser.exit_identifier_reference(&mut *node);
}
...
pub(crate) unsafe fn walk_binding_identifier<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_binding_identifier(&mut *node);
    traverser.exit_binding_identifier(&mut *node);
}
...
pub(crate) unsafe fn walk_label_identifier<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_label_identifier(&mut *node);
    traverser.exit_label_identifier(&mut *node);
}
...
pub(crate) unsafe fn walk_this_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_this_expression(&mut *node);
    traverser.exit_this_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_array_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_array_expression(&mut *node);
    traverser.ctx().push_stack(Ancestor::ArrayExpressionElements(
        ancestor::ArrayExpressionWithoutElements(node),
    ));
    for item in (*((node as *mut u8).add(ancestor::OFFSET_ARRAY_EXPRESSION_ELEMENTS)
        as *mut Vec<ArrayExpressionElement>))
        .iter_mut()
    {
        walk_array_expression_element(traverser, item as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_array_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_array_expression_element<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_array_expression_element(&mut *node);
    match &mut *node {
        ArrayExpressionElement::SpreadElement(node) => {
            walk_spread_element(traverser, (&mut **node) as *mut _)
        }
        ArrayExpressionElement::Elision(node) => walk_elision(traverser, node as *mut _),
        ...
        | ArrayExpressionElement::PrivateFieldExpression(_) => {
            walk_expression(traverser, node as *mut _)
        }
    }
    traverser.exit_array_expression_element(&mut *node);
}

pub(crate) unsafe fn walk_elision<'a, Tr: Traverse<'a>>(traverser: &mut Tr, node: *mut Elision) {
    traverser.enter_elision(&mut *node);
    traverser.exit_elision(&mut *node);
}
...
pub(crate) unsafe fn walk_object_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_object_expression(&mut *node);
    traverser.ctx().push_stack(Ancestor::ObjectExpressionProperties(
    ...
    for item in (*((node as *mut u8).add(ancestor::OFFSET_OBJECT_EXPRESSION_PROPERTIES)
        ...
        .iter_mut()
    {
        walk_object_property_kind(traverser, item as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_object_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_object_property_kind<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_object_property_kind(&mut *node);
    match &mut *node {
        ObjectPropertyKind::ObjectProperty(node) => {
            walk_object_property(traverser, (&mut **node) as *mut _)
        }
        ObjectPropertyKind::SpreadProperty(node) => {
            walk_spread_element(traverser, (&mut **node) as *mut _)
        }
    }
    traverser.exit_object_property_kind(&mut *node);
}
...
pub(crate) unsafe fn walk_object_property<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_object_property(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::ObjectPropertyKey(ancestor::ObjectPropertyWithoutKey(node)));
    walk_property_key(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_OBJECT_PROPERTY_KEY) as *mut PropertyKey,
    );
    traverser.ctx().retag_stack(AncestorType::ObjectPropertyValue);
    walk_expression(
        traverser,
    ...
    {
        traverser.ctx().retag_stack(AncestorType::ObjectPropertyInit);
        walk_expression(traverser, field as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_object_property(&mut *node);
}
...
pub(crate) unsafe fn walk_property_key<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_property_key(&mut *node);
    match &mut *node {
        PropertyKey::StaticIdentifier(node) => {
            walk_identifier_name(traverser, (&mut **node) as *mut _)
        }
        PropertyKey::PrivateIdentifier(node) => {
            walk_private_identifier(traverser, (&mut **node) as *mut _)
        }
        ...
        | PropertyKey::PrivateFieldExpression(_) => walk_expression(traverser, node as *mut _),
    }
    traverser.exit_property_key(&mut *node);
}
...
pub(crate) unsafe fn walk_template_literal<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_template_literal(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::TemplateLiteralQuasis(ancestor::TemplateLiteralWithoutQuasis(node)));
    for item in (*((node as *mut u8).add(ancestor::OFFSET_TEMPLATE_LITERAL_QUASIS)
        as *mut Vec<TemplateElement>))
        .iter_mut()
    {
        walk_template_element(traverser, item as *mut _);
    }
    traverser.ctx().retag_stack(AncestorType::TemplateLiteralExpressions);
    for item in (*((node as *mut u8).add(ancestor::OFFSET_TEMPLATE_LITERAL_EXPRESSIONS)
        as *mut Vec<Expression>))
        .iter_mut()
    {
        walk_expression(traverser, item as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_template_literal(&mut *node);
}
...
pub(crate) unsafe fn walk_tagged_template_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_tagged_template_expression(&mut *node);
    traverser.ctx().push_stack(Ancestor::TaggedTemplateExpressionTag(
    ...
    traverser.ctx().retag_stack(AncestorType::TaggedTemplateExpressionQuasi);
    ...
    {
        traverser.ctx().retag_stack(AncestorType::TaggedTemplateExpressionTypeParameters);
        walk_ts_type_parameter_instantiation(traverser, (&mut **field) as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_tagged_template_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_template_element<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_template_element(&mut *node);
    traverser.exit_template_element(&mut *node);
}
...
pub(crate) unsafe fn walk_member_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_member_expression(&mut *node);
    match &mut *node {
        MemberExpression::ComputedMemberExpression(node) => {
            walk_computed_member_expression(traverser, (&mut **node) as *mut _)
        }
        MemberExpression::StaticMemberExpression(node) => {
            walk_static_member_expression(traverser, (&mut **node) as *mut _)
        }
        MemberExpression::PrivateFieldExpression(node) => {
            walk_private_field_expression(traverser, (&mut **node) as *mut _)
        }
    }
    traverser.exit_member_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_computed_member_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_computed_member_expression(&mut *node);
    traverser.ctx().push_stack(Ancestor::ComputedMemberExpressionObject(
    ...
    traverser.ctx().retag_stack(AncestorType::ComputedMemberExpressionExpression);
    walk_expression(
        traverser,
    ...
    traverser.ctx().pop_stack();
    traverser.exit_computed_member_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_static_member_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_static_member_expression(&mut *node);
    traverser.ctx().push_stack(Ancestor::StaticMemberExpressionObject(
    ...
    traverser.ctx().retag_stack(AncestorType::StaticMemberExpressionProperty);
    ...
    traverser.ctx().pop_stack();
    traverser.exit_static_member_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_private_field_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_private_field_expression(&mut *node);
    traverser.ctx().push_stack(Ancestor::PrivateFieldExpressionObject(
    ...
    traverser.ctx().retag_stack(AncestorType::PrivateFieldExpressionField);
    ...
    traverser.ctx().pop_stack();
    traverser.exit_private_field_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_call_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_call_expression(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::CallExpressionCallee(ancestor::CallExpressionWithoutCallee(node)));
    walk_expression(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_CALL_EXPRESSION_CALLEE) as *mut Expression,
    );
    traverser.ctx().retag_stack(AncestorType::CallExpressionArguments);
    for item in (*((node as *mut u8).add(ancestor::OFFSET_CALL_EXPRESSION_ARGUMENTS)
        as *mut Vec<Argument>))
        .iter_mut()
    {
        walk_argument(traverser, item as *mut _);
    }
    if let Some(field) = &mut *((node as *mut u8)
        .add(ancestor::OFFSET_CALL_EXPRESSION_TYPE_PARAMETERS)
        as *mut Option<Box<TSTypeParameterInstantiation>>)
    {
        traverser.ctx().retag_stack(AncestorType::CallExpressionTypeParameters);
        walk_ts_type_parameter_instantiation(traverser, (&mut **field) as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_call_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_new_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_new_expression(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::NewExpressionCallee(ancestor::NewExpressionWithoutCallee(node)));
    walk_expression(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_NEW_EXPRESSION_CALLEE) as *mut Expression,
    );
    traverser.ctx().retag_stack(AncestorType::NewExpressionArguments);
    ...
    if let Some(field) = &mut *((node as *mut u8)
        ...
        as *mut Option<Box<TSTypeParameterInstantiation>>)
    {
        traverser.ctx().retag_stack(AncestorType::NewExpressionTypeParameters);
        walk_ts_type_parameter_instantiation(traverser, (&mut **field) as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_new_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_meta_property<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_meta_property(&mut *node);
    traverser.ctx().push_stack(Ancestor::MetaPropertyMeta(ancestor::MetaPropertyWithoutMeta(node)));
    walk_identifier_name(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_META_PROPERTY_META) as *mut IdentifierName,
    );
    traverser.ctx().retag_stack(AncestorType::MetaPropertyProperty);
    walk_identifier_name(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_META_PROPERTY_PROPERTY) as *mut IdentifierName,
    );
    traverser.ctx().pop_stack();
    traverser.exit_meta_property(&mut *node);
}
...
pub(crate) unsafe fn walk_spread_element<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_spread_element(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::SpreadElementArgument(ancestor::SpreadElementWithoutArgument(node)));
    walk_expression(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_SPREAD_ELEMENT_ARGUMENT) as *mut Expression,
    );
    traverser.ctx().pop_stack();
    traverser.exit_spread_element(&mut *node);
}
...
pub(crate) unsafe fn walk_argument<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_argument(&mut *node);
    match &mut *node {
        Argument::SpreadElement(node) => walk_spread_element(traverser, (&mut **node) as *mut _),
        ...
        | Argument::PrivateFieldExpression(_) => walk_expression(traverser, node as *mut _),
    }
    traverser.exit_argument(&mut *node);
}
...
pub(crate) unsafe fn walk_update_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_update_expression(&mut *node);
    traverser.ctx().push_stack(Ancestor::UpdateExpressionArgument(
        ancestor::UpdateExpressionWithoutArgument(node),
    ));
    ...
    traverser.ctx().pop_stack();
    traverser.exit_update_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_unary_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_unary_expression(&mut *node);
    traverser.ctx().push_stack(Ancestor::UnaryExpressionArgument(
        ancestor::UnaryExpressionWithoutArgument(node),
    ));
    walk_expression(
        traverser,
    ...
    traverser.ctx().pop_stack();
    traverser.exit_unary_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_binary_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_binary_expression(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::BinaryExpressionLeft(ancestor::BinaryExpressionWithoutLeft(node)));
    walk_expression(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_BINARY_EXPRESSION_LEFT) as *mut Expression,
    );
    traverser.ctx().retag_stack(AncestorType::BinaryExpressionRight);
    walk_expression(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_BINARY_EXPRESSION_RIGHT) as *mut Expression,
    );
    traverser.ctx().pop_stack();
    traverser.exit_binary_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_private_in_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_private_in_expression(&mut *node);
    traverser.ctx().push_stack(Ancestor::PrivateInExpressionLeft(
        ancestor::PrivateInExpressionWithoutLeft(node),
    ));
    walk_private_identifier(
        traverser,
    ...
    traverser.ctx().retag_stack(AncestorType::PrivateInExpressionRight);
    walk_expression(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_PRIVATE_IN_EXPRESSION_RIGHT) as *mut Expression,
    );
    traverser.ctx().pop_stack();
    traverser.exit_private_in_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_logical_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_logical_expression(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::LogicalExpressionLeft(ancestor::LogicalExpressionWithoutLeft(node)));
    walk_expression(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_LOGICAL_EXPRESSION_LEFT) as *mut Expression,
    );
    traverser.ctx().retag_stack(AncestorType::LogicalExpressionRight);
    walk_expression(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_LOGICAL_EXPRESSION_RIGHT) as *mut Expression,
    );
    traverser.ctx().pop_stack();
    traverser.exit_logical_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_conditional_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_conditional_expression(&mut *node);
    traverser.ctx().push_stack(Ancestor::ConditionalExpressionTest(
    ...
    traverser.ctx().retag_stack(AncestorType::ConditionalExpressionConsequent);
    walk_expression(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_CONDITIONAL_EXPRESSION_CONSEQUENT)
            as *mut Expression,
    );
    traverser.ctx().retag_stack(AncestorType::ConditionalExpressionAlternate);
    walk_expression(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_CONDITIONAL_EXPRESSION_ALTERNATE) as *mut Expression,
    );
    traverser.ctx().pop_stack();
    traverser.exit_conditional_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_assignment_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_assignment_expression(&mut *node);
    traverser.ctx().push_stack(Ancestor::AssignmentExpressionLeft(
        ancestor::AssignmentExpressionWithoutLeft(node),
    ));
    walk_assignment_target(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_ASSIGNMENT_EXPRESSION_LEFT) as *mut AssignmentTarget,
    );
    traverser.ctx().retag_stack(AncestorType::AssignmentExpressionRight);
    walk_expression(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_ASSIGNMENT_EXPRESSION_RIGHT) as *mut Expression,
    );
    traverser.ctx().pop_stack();
    traverser.exit_assignment_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_assignment_target<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_assignment_target(&mut *node);
    match &mut *node {
        ...
        | AssignmentTarget::PrivateFieldExpression(_) => {
            walk_simple_assignment_target(traverser, node as *mut _)
        }
        AssignmentTarget::ArrayAssignmentTarget(_)
        | AssignmentTarget::ObjectAssignmentTarget(_) => {
            walk_assignment_target_pattern(traverser, node as *mut _)
        }
    }
    traverser.exit_assignment_target(&mut *node);
}
...
pub(crate) unsafe fn walk_simple_assignment_target<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_simple_assignment_target(&mut *node);
    match &mut *node {
    ...
    traverser.exit_simple_assignment_target(&mut *node);
}
...
pub(crate) unsafe fn walk_assignment_target_pattern<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_assignment_target_pattern(&mut *node);
    match &mut *node {
        AssignmentTargetPattern::ArrayAssignmentTarget(node) => {
            walk_array_assignment_target(traverser, (&mut **node) as *mut _)
        }
        AssignmentTargetPattern::ObjectAssignmentTarget(node) => {
            walk_object_assignment_target(traverser, (&mut **node) as *mut _)
        }
    }
    traverser.exit_assignment_target_pattern(&mut *node);
}
...
pub(crate) unsafe fn walk_array_assignment_target<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_array_assignment_target(&mut *node);
    traverser.ctx().push_stack(Ancestor::ArrayAssignmentTargetElements(
    ...
    for item in (*((node as *mut u8).add(ancestor::OFFSET_ARRAY_ASSIGNMENT_TARGET_ELEMENTS)
        ...
        .flatten()
    {
        walk_assignment_target_maybe_default(traverser, item as *mut _);
    }
    if let Some(field) = &mut *((node as *mut u8).add(ancestor::OFFSET_ARRAY_ASSIGNMENT_TARGET_REST)
        as *mut Option<AssignmentTargetRest>)
    {
        traverser.ctx().retag_stack(AncestorType::ArrayAssignmentTargetRest);
        walk_assignment_target_rest(traverser, field as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_array_assignment_target(&mut *node);
}
...
pub(crate) unsafe fn walk_object_assignment_target<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_object_assignment_target(&mut *node);
    traverser.ctx().push_stack(Ancestor::ObjectAssignmentTargetProperties(
    ...
    for item in (*((node as *mut u8).add(ancestor::OFFSET_OBJECT_ASSIGNMENT_TARGET_PROPERTIES)
        ...
        .iter_mut()
    {
        walk_assignment_target_property(traverser, item as *mut _);
    }
    if let Some(field) = &mut *((node as *mut u8)
        .add(ancestor::OFFSET_OBJECT_ASSIGNMENT_TARGET_REST)
        as *mut Option<AssignmentTargetRest>)
    {
        traverser.ctx().retag_stack(AncestorType::ObjectAssignmentTargetRest);
        walk_assignment_target_rest(traverser, field as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_object_assignment_target(&mut *node);
}
...
pub(crate) unsafe fn walk_assignment_target_rest<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_assignment_target_rest(&mut *node);
    traverser.ctx().push_stack(Ancestor::AssignmentTargetRestTarget(
    ...
    traverser.ctx().pop_stack();
    traverser.exit_assignment_target_rest(&mut *node);
}
...
pub(crate) unsafe fn walk_assignment_target_maybe_default<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_assignment_target_maybe_default(&mut *node);
    match &mut *node {
        AssignmentTargetMaybeDefault::AssignmentTargetWithDefault(node) => {
            walk_assignment_target_with_default(traverser, (&mut **node) as *mut _)
        }
        ...
        | AssignmentTargetMaybeDefault::PrivateFieldExpression(_) => {
            walk_assignment_target(traverser, node as *mut _)
        }
    }
    traverser.exit_assignment_target_maybe_default(&mut *node);
}
...
pub(crate) unsafe fn walk_assignment_target_with_default<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_assignment_target_with_default(&mut *node);
    traverser.ctx().push_stack(Ancestor::AssignmentTargetWithDefaultBinding(
    ...
    traverser.ctx().retag_stack(AncestorType::AssignmentTargetWithDefaultInit);
    walk_expression(
        traverser,
    ...
    traverser.ctx().pop_stack();
    traverser.exit_assignment_target_with_default(&mut *node);
}
...
pub(crate) unsafe fn walk_assignment_target_property<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_assignment_target_property(&mut *node);
    match &mut *node {
        AssignmentTargetProperty::AssignmentTargetPropertyIdentifier(node) => {
            walk_assignment_target_property_identifier(traverser, (&mut **node) as *mut _)
        }
        AssignmentTargetProperty::AssignmentTargetPropertyProperty(node) => {
            walk_assignment_target_property_property(traverser, (&mut **node) as *mut _)
        }
    }
    traverser.exit_assignment_target_property(&mut *node);
}
...
pub(crate) unsafe fn walk_assignment_target_property_identifier<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_assignment_target_property_identifier(&mut *node);
    traverser.ctx().push_stack(Ancestor::AssignmentTargetPropertyIdentifierBinding(
    ...
    if let Some(field) = &mut *((node as *mut u8)
        ...
        as *mut Option<Expression>)
    {
        traverser.ctx().retag_stack(AncestorType::AssignmentTargetPropertyIdentifierInit);
        walk_expression(traverser, field as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_assignment_target_property_identifier(&mut *node);
}
...
pub(crate) unsafe fn walk_assignment_target_property_property<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_assignment_target_property_property(&mut *node);
    traverser.ctx().push_stack(Ancestor::AssignmentTargetPropertyPropertyName(
    ...
    traverser.ctx().retag_stack(AncestorType::AssignmentTargetPropertyPropertyBinding);
    ...
    traverser.ctx().pop_stack();
    traverser.exit_assignment_target_property_property(&mut *node);
}
...
pub(crate) unsafe fn walk_sequence_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_sequence_expression(&mut *node);
    traverser.ctx().push_stack(Ancestor::SequenceExpressionExpressions(
    ...
    traverser.ctx().pop_stack();
    traverser.exit_sequence_expression(&mut *node);
}

pub(crate) unsafe fn walk_super<'a, Tr: Traverse<'a>>(traverser: &mut Tr, node: *mut Super) {
    traverser.enter_super(&mut *node);
    traverser.exit_super(&mut *node);
}
...
pub(crate) unsafe fn walk_await_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_await_expression(&mut *node);
    traverser.ctx().push_stack(Ancestor::AwaitExpressionArgument(
        ancestor::AwaitExpressionWithoutArgument(node),
    ));
    walk_expression(
        traverser,
    ...
    traverser.ctx().pop_stack();
    traverser.exit_await_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_chain_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_chain_expression(&mut *node);
    traverser.ctx().push_stack(Ancestor::ChainExpressionExpression(
    ...
    traverser.ctx().pop_stack();
    traverser.exit_chain_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_chain_element<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_chain_element(&mut *node);
    match &mut *node {
    ...
    traverser.exit_chain_element(&mut *node);
}
...
pub(crate) unsafe fn walk_parenthesized_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_parenthesized_expression(&mut *node);
    traverser.ctx().push_stack(Ancestor::ParenthesizedExpressionExpression(
    ...
    traverser.ctx().pop_stack();
    traverser.exit_parenthesized_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_statement<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_statement(&mut *node);
    match &mut *node {
        Statement::BlockStatement(node) => walk_block_statement(traverser, (&mut **node) as *mut _),
        Statement::BreakStatement(node) => walk_break_statement(traverser, (&mut **node) as *mut _),
        Statement::ContinueStatement(node) => {
            walk_continue_statement(traverser, (&mut **node) as *mut _)
        }
        Statement::DebuggerStatement(node) => {
            walk_debugger_statement(traverser, (&mut **node) as *mut _)
        }
        Statement::DoWhileStatement(node) => {
            walk_do_while_statement(traverser, (&mut **node) as *mut _)
        }
        Statement::EmptyStatement(node) => walk_empty_statement(traverser, (&mut **node) as *mut _),
        Statement::ExpressionStatement(node) => {
            walk_expression_statement(traverser, (&mut **node) as *mut _)
        }
        Statement::ForInStatement(node) => {
            walk_for_in_statement(traverser, (&mut **node) as *mut _)
        }
        Statement::ForOfStatement(node) => {
            walk_for_of_statement(traverser, (&mut **node) as *mut _)
        }
        Statement::ForStatement(node) => walk_for_statement(traverser, (&mut **node) as *mut _),
        Statement::IfStatement(node) => walk_if_statement(traverser, (&mut **node) as *mut _),
        Statement::LabeledStatement(node) => {
            walk_labeled_statement(traverser, (&mut **node) as *mut _)
        }
        Statement::ReturnStatement(node) => {
            walk_return_statement(traverser, (&mut **node) as *mut _)
        }
        Statement::SwitchStatement(node) => {
            walk_switch_statement(traverser, (&mut **node) as *mut _)
        }
        Statement::ThrowStatement(node) => walk_throw_statement(traverser, (&mut **node) as *mut _),
        Statement::TryStatement(node) => walk_try_statement(traverser, (&mut **node) as *mut _),
        Statement::WhileStatement(node) => walk_while_statement(traverser, (&mut **node) as *mut _),
        Statement::WithStatement(node) => walk_with_statement(traverser, (&mut **node) as *mut _),
        ...
        | Statement::TSImportEqualsDeclaration(_) => walk_declaration(traverser, node as *mut _),
        ...
        | Statement::TSNamespaceExportDeclaration(_) => {
            walk_module_declaration(traverser, node as *mut _)
        }
    }
    traverser.exit_statement(&mut *node);
}
...
pub(crate) unsafe fn walk_directive<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_directive(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::DirectiveExpression(ancestor::DirectiveWithoutExpression(node)));
    walk_string_literal(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_DIRECTIVE_EXPRESSION) as *mut StringLiteral,
    );
    traverser.ctx().pop_stack();
    traverser.exit_directive(&mut *node);
}
...
pub(crate) unsafe fn walk_hashbang<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_hashbang(&mut *node);
    traverser.exit_hashbang(&mut *node);
}
...
pub(crate) unsafe fn walk_block_statement<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_block_statement(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::BlockStatementBody(ancestor::BlockStatementWithoutBody(node)));
    walk_statements(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_BLOCK_STATEMENT_BODY) as *mut Vec<Statement>,
    );
    traverser.ctx().pop_stack();
    traverser.exit_block_statement(&mut *node);
}
...
pub(crate) unsafe fn walk_declaration<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_declaration(&mut *node);
    match &mut *node {
        Declaration::VariableDeclaration(node) => {
            walk_variable_declaration(traverser, (&mut **node) as *mut _)
        }
        Declaration::FunctionDeclaration(node) => walk_function(traverser, (&mut **node) as *mut _),
        Declaration::ClassDeclaration(node) => walk_class(traverser, (&mut **node) as *mut _),
        Declaration::UsingDeclaration(node) => {
            walk_using_declaration(traverser, (&mut **node) as *mut _)
        }
        Declaration::TSTypeAliasDeclaration(node) => {
            walk_ts_type_alias_declaration(traverser, (&mut **node) as *mut _)
        }
        Declaration::TSInterfaceDeclaration(node) => {
            walk_ts_interface_declaration(traverser, (&mut **node) as *mut _)
        }
        Declaration::TSEnumDeclaration(node) => {
            walk_ts_enum_declaration(traverser, (&mut **node) as *mut _)
        }
        Declaration::TSModuleDeclaration(node) => {
            walk_ts_module_declaration(traverser, (&mut **node) as *mut _)
        }
        Declaration::TSImportEqualsDeclaration(node) => {
            walk_ts_import_equals_declaration(traverser, (&mut **node) as *mut _)
        }
    }
    traverser.exit_declaration(&mut *node);
}
...
pub(crate) unsafe fn walk_variable_declaration<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_variable_declaration(&mut *node);
    traverser.ctx().push_stack(Ancestor::VariableDeclarationDeclarations(
    ...
    for item in (*((node as *mut u8).add(ancestor::OFFSET_VARIABLE_DECLARATION_DECLARATIONS)
        ...
        .iter_mut()
    {
        walk_variable_declarator(traverser, item as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_variable_declaration(&mut *node);
}
...
pub(crate) unsafe fn walk_variable_declarator<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_variable_declarator(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::VariableDeclaratorId(ancestor::VariableDeclaratorWithoutId(node)));
    ...
    if let Some(field) = &mut *((node as *mut u8).add(ancestor::OFFSET_VARIABLE_DECLARATOR_INIT)
        as *mut Option<Expression>)
    {
        traverser.ctx().retag_stack(AncestorType::VariableDeclaratorInit);
        walk_expression(traverser, field as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_variable_declarator(&mut *node);
}
...
pub(crate) unsafe fn walk_using_declaration<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_using_declaration(&mut *node);
    traverser.ctx().push_stack(Ancestor::UsingDeclarationDeclarations(
    ...
    traverser.ctx().pop_stack();
    traverser.exit_using_declaration(&mut *node);
}
...
pub(crate) unsafe fn walk_empty_statement<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_empty_statement(&mut *node);
    traverser.exit_empty_statement(&mut *node);
}
...
pub(crate) unsafe fn walk_expression_statement<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_expression_statement(&mut *node);
    traverser.ctx().push_stack(Ancestor::ExpressionStatementExpression(
    ...
    traverser.ctx().pop_stack();
    traverser.exit_expression_statement(&mut *node);
}
...
pub(crate) unsafe fn walk_if_statement<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_if_statement(&mut *node);
    traverser.ctx().push_stack(Ancestor::IfStatementTest(ancestor::IfStatementWithoutTest(node)));
    walk_expression(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_IF_STATEMENT_TEST) as *mut Expression,
    );
    traverser.ctx().retag_stack(AncestorType::IfStatementConsequent);
    walk_statement(
        traverser,
    ...
    {
        traverser.ctx().retag_stack(AncestorType::IfStatementAlternate);
        walk_statement(traverser, field as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_if_statement(&mut *node);
}
...
pub(crate) unsafe fn walk_do_while_statement<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_do_while_statement(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::DoWhileStatementBody(ancestor::DoWhileStatementWithoutBody(node)));
    walk_statement(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_DO_WHILE_STATEMENT_BODY) as *mut Statement,
    );
    traverser.ctx().retag_stack(AncestorType::DoWhileStatementTest);
    walk_expression(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_DO_WHILE_STATEMENT_TEST) as *mut Expression,
    );
    traverser.ctx().pop_stack();
    traverser.exit_do_while_statement(&mut *node);
}
...
pub(crate) unsafe fn walk_while_statement<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_while_statement(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::WhileStatementTest(ancestor::WhileStatementWithoutTest(node)));
    walk_expression(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_WHILE_STATEMENT_TEST) as *mut Expression,
    );
    traverser.ctx().retag_stack(AncestorType::WhileStatementBody);
    walk_statement(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_WHILE_STATEMENT_BODY) as *mut Statement,
    );
    traverser.ctx().pop_stack();
    traverser.exit_while_statement(&mut *node);
}
...
pub(crate) unsafe fn walk_for_statement<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_for_statement(&mut *node);
    traverser.ctx().push_stack(Ancestor::ForStatementInit(ancestor::ForStatementWithoutInit(node)));
    if let Some(field) = &mut *((node as *mut u8).add(ancestor::OFFSET_FOR_STATEMENT_INIT)
        as *mut Option<ForStatementInit>)
    {
        walk_for_statement_init(traverser, field as *mut _);
    }
    if let Some(field) = &mut *((node as *mut u8).add(ancestor::OFFSET_FOR_STATEMENT_TEST)
        as *mut Option<Expression>)
    {
        traverser.ctx().retag_stack(AncestorType::ForStatementTest);
        walk_expression(traverser, field as *mut _);
    }
    if let Some(field) = &mut *((node as *mut u8).add(ancestor::OFFSET_FOR_STATEMENT_UPDATE)
        as *mut Option<Expression>)
    {
        traverser.ctx().retag_stack(AncestorType::ForStatementUpdate);
        walk_expression(traverser, field as *mut _);
    }
    traverser.ctx().retag_stack(AncestorType::ForStatementBody);
    walk_statement(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_FOR_STATEMENT_BODY) as *mut Statement,
    );
    traverser.ctx().pop_stack();
    traverser.exit_for_statement(&mut *node);
}
...
pub(crate) unsafe fn walk_for_statement_init<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_for_statement_init(&mut *node);
    match &mut *node {
        ...
        | ForStatementInit::PrivateFieldExpression(_) => walk_expression(traverser, node as *mut _),
    }
    traverser.exit_for_statement_init(&mut *node);
}
...
pub(crate) unsafe fn walk_for_in_statement<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_for_in_statement(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::ForInStatementLeft(ancestor::ForInStatementWithoutLeft(node)));
    walk_for_statement_left(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_FOR_IN_STATEMENT_LEFT) as *mut ForStatementLeft,
    );
    traverser.ctx().retag_stack(AncestorType::ForInStatementRight);
    walk_expression(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_FOR_IN_STATEMENT_RIGHT) as *mut Expression,
    );
    traverser.ctx().retag_stack(AncestorType::ForInStatementBody);
    walk_statement(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_FOR_IN_STATEMENT_BODY) as *mut Statement,
    );
    traverser.ctx().pop_stack();
    traverser.exit_for_in_statement(&mut *node);
}
...
pub(crate) unsafe fn walk_for_of_statement<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_for_of_statement(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::ForOfStatementLeft(ancestor::ForOfStatementWithoutLeft(node)));
    walk_for_statement_left(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_FOR_OF_STATEMENT_LEFT) as *mut ForStatementLeft,
    );
    traverser.ctx().retag_stack(AncestorType::ForOfStatementRight);
    walk_expression(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_FOR_OF_STATEMENT_RIGHT) as *mut Expression,
    );
    traverser.ctx().retag_stack(AncestorType::ForOfStatementBody);
    walk_statement(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_FOR_OF_STATEMENT_BODY) as *mut Statement,
    );
    traverser.ctx().pop_stack();
    traverser.exit_for_of_statement(&mut *node);
}
...
pub(crate) unsafe fn walk_for_statement_left<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_for_statement_left(&mut *node);
    match &mut *node {
    ...
    traverser.exit_for_statement_left(&mut *node);
}
...
pub(crate) unsafe fn walk_continue_statement<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_continue_statement(&mut *node);
    traverser.ctx().push_stack(Ancestor::ContinueStatementLabel(
        ancestor::ContinueStatementWithoutLabel(node),
    ));
    if let Some(field) = &mut *((node as *mut u8).add(ancestor::OFFSET_CONTINUE_STATEMENT_LABEL)
        as *mut Option<LabelIdentifier>)
    {
        walk_label_identifier(traverser, field as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_continue_statement(&mut *node);
}
...
pub(crate) unsafe fn walk_break_statement<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_break_statement(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::BreakStatementLabel(ancestor::BreakStatementWithoutLabel(node)));
    ...
    traverser.ctx().pop_stack();
    traverser.exit_break_statement(&mut *node);
}
...
pub(crate) unsafe fn walk_return_statement<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_return_statement(&mut *node);
    traverser.ctx().push_stack(Ancestor::ReturnStatementArgument(
        ancestor::ReturnStatementWithoutArgument(node),
    ));
    ...
    traverser.ctx().pop_stack();
    traverser.exit_return_statement(&mut *node);
}
...
pub(crate) unsafe fn walk_with_statement<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_with_statement(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::WithStatementObject(ancestor::WithStatementWithoutObject(node)));
    walk_expression(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_WITH_STATEMENT_OBJECT) as *mut Expression,
    );
    traverser.ctx().retag_stack(AncestorType::WithStatementBody);
    walk_statement(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_WITH_STATEMENT_BODY) as *mut Statement,
    );
    traverser.ctx().pop_stack();
    traverser.exit_with_statement(&mut *node);
}
...
pub(crate) unsafe fn walk_switch_statement<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_switch_statement(&mut *node);
    traverser.ctx().push_stack(Ancestor::SwitchStatementDiscriminant(
    ...
    traverser.ctx().retag_stack(AncestorType::SwitchStatementCases);
    for item in (*((node as *mut u8).add(ancestor::OFFSET_SWITCH_STATEMENT_CASES)
        as *mut Vec<SwitchCase>))
        .iter_mut()
    {
        walk_switch_case(traverser, item as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_switch_statement(&mut *node);
}
...
pub(crate) unsafe fn walk_switch_case<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_switch_case(&mut *node);
    traverser.ctx().push_stack(Ancestor::SwitchCaseTest(ancestor::SwitchCaseWithoutTest(node)));
    if let Some(field) =
        &mut *((node as *mut u8).add(ancestor::OFFSET_SWITCH_CASE_TEST) as *mut Option<Expression>)
    {
        walk_expression(traverser, field as *mut _);
    }
    traverser.ctx().retag_stack(AncestorType::SwitchCaseConsequent);
    walk_statements(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_SWITCH_CASE_CONSEQUENT) as *mut Vec<Statement>,
    );
    traverser.ctx().pop_stack();
    traverser.exit_switch_case(&mut *node);
}
...
pub(crate) unsafe fn walk_labeled_statement<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_labeled_statement(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::LabeledStatementLabel(ancestor::LabeledStatementWithoutLabel(node)));
    walk_label_identifier(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_LABELED_STATEMENT_LABEL) as *mut LabelIdentifier,
    );
    traverser.ctx().retag_stack(AncestorType::LabeledStatementBody);
    walk_statement(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_LABELED_STATEMENT_BODY) as *mut Statement,
    );
    traverser.ctx().pop_stack();
    traverser.exit_labeled_statement(&mut *node);
}
...
pub(crate) unsafe fn walk_throw_statement<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_throw_statement(&mut *node);
    traverser.ctx().push_stack(Ancestor::ThrowStatementArgument(
        ancestor::ThrowStatementWithoutArgument(node),
    ));
    walk_expression(
        traverser,
    ...
    traverser.ctx().pop_stack();
    traverser.exit_throw_statement(&mut *node);
}
...
pub(crate) unsafe fn walk_try_statement<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_try_statement(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::TryStatementBlock(ancestor::TryStatementWithoutBlock(node)));
    ...
    {
        traverser.ctx().retag_stack(AncestorType::TryStatementHandler);
        walk_catch_clause(traverser, (&mut **field) as *mut _);
    }
    if let Some(field) = &mut *((node as *mut u8).add(ancestor::OFFSET_TRY_STATEMENT_FINALIZER)
        as *mut Option<Box<BlockStatement>>)
    {
        traverser.ctx().retag_stack(AncestorType::TryStatementFinalizer);
        walk_block_statement(traverser, (&mut **field) as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_try_statement(&mut *node);
}
...
pub(crate) unsafe fn walk_catch_clause<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_catch_clause(&mut *node);
    traverser.ctx().push_stack(Ancestor::CatchClauseParam(ancestor::CatchClauseWithoutParam(node)));
    if let Some(field) = &mut *((node as *mut u8).add(ancestor::OFFSET_CATCH_CLAUSE_PARAM)
        as *mut Option<CatchParameter>)
    {
        walk_catch_parameter(traverser, field as *mut _);
    }
    traverser.ctx().retag_stack(AncestorType::CatchClauseBody);
    walk_block_statement(
        traverser,
    ...
    traverser.ctx().pop_stack();
    traverser.exit_catch_clause(&mut *node);
}
...
pub(crate) unsafe fn walk_catch_parameter<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_catch_parameter(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::CatchParameterPattern(ancestor::CatchParameterWithoutPattern(node)));
    walk_binding_pattern(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_CATCH_PARAMETER_PATTERN) as *mut BindingPattern,
    );
    traverser.ctx().pop_stack();
    traverser.exit_catch_parameter(&mut *node);
}
...
pub(crate) unsafe fn walk_debugger_statement<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_debugger_statement(&mut *node);
    traverser.exit_debugger_statement(&mut *node);
}
...
pub(crate) unsafe fn walk_binding_pattern<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_binding_pattern(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::BindingPatternKind(ancestor::BindingPatternWithoutKind(node)));
    ...
    {
        traverser.ctx().retag_stack(AncestorType::BindingPatternTypeAnnotation);
        walk_ts_type_annotation(traverser, (&mut **field) as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_binding_pattern(&mut *node);
}
...
pub(crate) unsafe fn walk_binding_pattern_kind<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_binding_pattern_kind(&mut *node);
    match &mut *node {
        BindingPatternKind::BindingIdentifier(node) => {
            walk_binding_identifier(traverser, (&mut **node) as *mut _)
        }
        BindingPatternKind::ObjectPattern(node) => {
            walk_object_pattern(traverser, (&mut **node) as *mut _)
        }
        BindingPatternKind::ArrayPattern(node) => {
            walk_array_pattern(traverser, (&mut **node) as *mut _)
        }
        BindingPatternKind::AssignmentPattern(node) => {
            walk_assignment_pattern(traverser, (&mut **node) as *mut _)
        }
    }
    traverser.exit_binding_pattern_kind(&mut *node);
}
...
pub(crate) unsafe fn walk_assignment_pattern<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_assignment_pattern(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::AssignmentPatternLeft(ancestor::AssignmentPatternWithoutLeft(node)));
    walk_binding_pattern(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_ASSIGNMENT_PATTERN_LEFT) as *mut BindingPattern,
    );
    traverser.ctx().retag_stack(AncestorType::AssignmentPatternRight);
    walk_expression(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_ASSIGNMENT_PATTERN_RIGHT) as *mut Expression,
    );
    traverser.ctx().pop_stack();
    traverser.exit_assignment_pattern(&mut *node);
}
...
pub(crate) unsafe fn walk_object_pattern<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_object_pattern(&mut *node);
    traverser.ctx().push_stack(Ancestor::ObjectPatternProperties(
        ancestor::ObjectPatternWithoutProperties(node),
    ));
    for item in (*((node as *mut u8).add(ancestor::OFFSET_OBJECT_PATTERN_PROPERTIES)
        as *mut Vec<BindingProperty>))
        .iter_mut()
    {
        walk_binding_property(traverser, item as *mut _);
    }
    if let Some(field) = &mut *((node as *mut u8).add(ancestor::OFFSET_OBJECT_PATTERN_REST)
        as *mut Option<Box<BindingRestElement>>)
    {
        traverser.ctx().retag_stack(AncestorType::ObjectPatternRest);
        walk_binding_rest_element(traverser, (&mut **field) as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_object_pattern(&mut *node);
}
...
pub(crate) unsafe fn walk_binding_property<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_binding_property(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::BindingPropertyKey(ancestor::BindingPropertyWithoutKey(node)));
    walk_property_key(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_BINDING_PROPERTY_KEY) as *mut PropertyKey,
    );
    traverser.ctx().retag_stack(AncestorType::BindingPropertyValue);
    walk_binding_pattern(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_BINDING_PROPERTY_VALUE) as *mut BindingPattern,
    );
    traverser.ctx().pop_stack();
    traverser.exit_binding_property(&mut *node);
}
...
pub(crate) unsafe fn walk_array_pattern<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_array_pattern(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::ArrayPatternElements(ancestor::ArrayPatternWithoutElements(node)));
    for item in (*((node as *mut u8).add(ancestor::OFFSET_ARRAY_PATTERN_ELEMENTS)
        as *mut Vec<Option<BindingPattern>>))
        .iter_mut()
        .flatten()
    {
        walk_binding_pattern(traverser, item as *mut _);
    }
    if let Some(field) = &mut *((node as *mut u8).add(ancestor::OFFSET_ARRAY_PATTERN_REST)
        as *mut Option<Box<BindingRestElement>>)
    {
        traverser.ctx().retag_stack(AncestorType::ArrayPatternRest);
        walk_binding_rest_element(traverser, (&mut **field) as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_array_pattern(&mut *node);
}
...
pub(crate) unsafe fn walk_binding_rest_element<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_binding_rest_element(&mut *node);
    traverser.ctx().push_stack(Ancestor::BindingRestElementArgument(
    ...
    traverser.ctx().pop_stack();
    traverser.exit_binding_rest_element(&mut *node);
}
...
pub(crate) unsafe fn walk_function<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_function(&mut *node);
    traverser.ctx().push_stack(Ancestor::FunctionId(ancestor::FunctionWithoutId(node)));
    if let Some(field) = &mut *((node as *mut u8).add(ancestor::OFFSET_FUNCTION_ID)
        as *mut Option<BindingIdentifier>)
    {
        walk_binding_identifier(traverser, field as *mut _);
    }
    if let Some(field) = &mut *((node as *mut u8).add(ancestor::OFFSET_FUNCTION_THIS_PARAM)
        as *mut Option<TSThisParameter>)
    {
        traverser.ctx().retag_stack(AncestorType::FunctionThisParam);
        walk_ts_this_parameter(traverser, field as *mut _);
    }
    traverser.ctx().retag_stack(AncestorType::FunctionParams);
    ...
    {
        traverser.ctx().retag_stack(AncestorType::FunctionBody);
        walk_function_body(traverser, (&mut **field) as *mut _);
    }
    ...
    {
        traverser.ctx().retag_stack(AncestorType::FunctionTypeParameters);
        walk_ts_type_parameter_declaration(traverser, (&mut **field) as *mut _);
    }
    if let Some(field) = &mut *((node as *mut u8).add(ancestor::OFFSET_FUNCTION_RETURN_TYPE)
        as *mut Option<Box<TSTypeAnnotation>>)
    {
        traverser.ctx().retag_stack(AncestorType::FunctionReturnType);
        walk_ts_type_annotation(traverser, (&mut **field) as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_function(&mut *node);
}
...
pub(crate) unsafe fn walk_formal_parameters<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_formal_parameters(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::FormalParametersItems(ancestor::FormalParametersWithoutItems(node)));
    for item in (*((node as *mut u8).add(ancestor::OFFSET_FORMAL_PARAMETERS_ITEMS)
        as *mut Vec<FormalParameter>))
        .iter_mut()
    {
        walk_formal_parameter(traverser, item as *mut _);
    }
    if let Some(field) = &mut *((node as *mut u8).add(ancestor::OFFSET_FORMAL_PARAMETERS_REST)
        as *mut Option<Box<BindingRestElement>>)
    {
        traverser.ctx().retag_stack(AncestorType::FormalParametersRest);
        walk_binding_rest_element(traverser, (&mut **field) as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_formal_parameters(&mut *node);
}
...
pub(crate) unsafe fn walk_formal_parameter<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_formal_parameter(&mut *node);
    traverser.ctx().push_stack(Ancestor::FormalParameterPattern(
        ancestor::FormalParameterWithoutPattern(node),
    ));
    walk_binding_pattern(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_FORMAL_PARAMETER_PATTERN) as *mut BindingPattern,
    );
    traverser.ctx().retag_stack(AncestorType::FormalParameterDecorators);
    for item in (*((node as *mut u8).add(ancestor::OFFSET_FORMAL_PARAMETER_DECORATORS)
        as *mut Vec<Decorator>))
        .iter_mut()
    {
        walk_decorator(traverser, item as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_formal_parameter(&mut *node);
}
...
pub(crate) unsafe fn walk_function_body<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_function_body(&mut *node);
    traverser.ctx().push_stack(Ancestor::FunctionBodyDirectives(
        ancestor::FunctionBodyWithoutDirectives(node),
    ));
    ...
    traverser.ctx().retag_stack(AncestorType::FunctionBodyStatements);
    walk_statements(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_FUNCTION_BODY_STATEMENTS) as *mut Vec<Statement>,
    );
    traverser.ctx().pop_stack();
    traverser.exit_function_body(&mut *node);
}
...
pub(crate) unsafe fn walk_arrow_function_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_arrow_function_expression(&mut *node);
    traverser.ctx().push_stack(Ancestor::ArrowFunctionExpressionParams(
    ...
    traverser.ctx().retag_stack(AncestorType::ArrowFunctionExpressionBody);
    walk_function_body(
        traverser,
    ...
    if let Some(field) = &mut *((node as *mut u8)
        ...
        as *mut Option<Box<TSTypeParameterDeclaration>>)
    {
        traverser.ctx().retag_stack(AncestorType::ArrowFunctionExpressionTypeParameters);
        walk_ts_type_parameter_declaration(traverser, (&mut **field) as *mut _);
    }
    if let Some(field) = &mut *((node as *mut u8)
        ...
        as *mut Option<Box<TSTypeAnnotation>>)
    {
        traverser.ctx().retag_stack(AncestorType::ArrowFunctionExpressionReturnType);
        walk_ts_type_annotation(traverser, (&mut **field) as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_arrow_function_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_yield_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_yield_expression(&mut *node);
    traverser.ctx().push_stack(Ancestor::YieldExpressionArgument(
        ancestor::YieldExpressionWithoutArgument(node),
    ));
    ...
    traverser.ctx().pop_stack();
    traverser.exit_yield_expression(&mut *node);
}

pub(crate) unsafe fn walk_class<'a, Tr: Traverse<'a>>(traverser: &mut Tr, node: *mut Class<'a>) {
    traverser.enter_class(&mut *node);
    traverser.ctx().push_stack(Ancestor::ClassId(ancestor::ClassWithoutId(node)));
    if let Some(field) =
    ...
    {
        traverser.ctx().retag_stack(AncestorType::ClassSuperClass);
        walk_expression(traverser, field as *mut _);
    }
    traverser.ctx().retag_stack(AncestorType::ClassBody);
    walk_class_body(
        traverser,
    ...
    if let Some(field) = &mut *((node as *mut u8).add(ancestor::OFFSET_CLASS_TYPE_PARAMETERS)
        as *mut Option<Box<TSTypeParameterDeclaration>>)
    {
        traverser.ctx().retag_stack(AncestorType::ClassTypeParameters);
        walk_ts_type_parameter_declaration(traverser, (&mut **field) as *mut _);
    }
    if let Some(field) = &mut *((node as *mut u8).add(ancestor::OFFSET_CLASS_SUPER_TYPE_PARAMETERS)
        as *mut Option<Box<TSTypeParameterInstantiation>>)
    {
        traverser.ctx().retag_stack(AncestorType::ClassSuperTypeParameters);
        walk_ts_type_parameter_instantiation(traverser, (&mut **field) as *mut _);
    }
    if let Some(field) = &mut *((node as *mut u8).add(ancestor::OFFSET_CLASS_IMPLEMENTS)
        as *mut Option<Vec<TSClassImplements>>)
    {
        traverser.ctx().retag_stack(AncestorType::ClassImplements);
        for item in field.iter_mut() {
            walk_ts_class_implements(traverser, item as *mut _);
        }
    }
    traverser.ctx().retag_stack(AncestorType::ClassDecorators);
...
pub(crate) unsafe fn walk_class_body<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_class_body(&mut *node);
    traverser.ctx().push_stack(Ancestor::ClassBodyBody(ancestor::ClassBodyWithoutBody(node)));
    for item in (*((node as *mut u8).add(ancestor::OFFSET_CLASS_BODY_BODY)
        as *mut Vec<ClassElement>))
        .iter_mut()
    {
        walk_class_element(traverser, item as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_class_body(&mut *node);
}
...
pub(crate) unsafe fn walk_class_element<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_class_element(&mut *node);
    match &mut *node {
        ClassElement::StaticBlock(node) => walk_static_block(traverser, (&mut **node) as *mut _),
        ClassElement::MethodDefinition(node) => {
            walk_method_definition(traverser, (&mut **node) as *mut _)
        }
        ClassElement::PropertyDefinition(node) => {
            walk_property_definition(traverser, (&mut **node) as *mut _)
        }
        ClassElement::AccessorProperty(node) => {
            walk_accessor_property(traverser, (&mut **node) as *mut _)
        }
        ClassElement::TSIndexSignature(node) => {
            walk_ts_index_signature(traverser, (&mut **node) as *mut _)
        }
    }
    traverser.exit_class_element(&mut *node);
}
...
pub(crate) unsafe fn walk_method_definition<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_method_definition(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::MethodDefinitionKey(ancestor::MethodDefinitionWithoutKey(node)));
    walk_property_key(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_METHOD_DEFINITION_KEY) as *mut PropertyKey,
    );
    traverser.ctx().retag_stack(AncestorType::MethodDefinitionValue);
    walk_function(
        traverser,
        (&mut **((node as *mut u8).add(ancestor::OFFSET_METHOD_DEFINITION_VALUE)
            as *mut Box<Function>)) as *mut _,
    );
    traverser.ctx().retag_stack(AncestorType::MethodDefinitionDecorators);
    ...
    traverser.ctx().pop_stack();
    traverser.exit_method_definition(&mut *node);
}
...
pub(crate) unsafe fn walk_property_definition<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_property_definition(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::PropertyDefinitionKey(ancestor::PropertyDefinitionWithoutKey(node)));
    walk_property_key(
        traverser,
    ...
    if let Some(field) = &mut *((node as *mut u8).add(ancestor::OFFSET_PROPERTY_DEFINITION_VALUE)
        as *mut Option<Expression>)
    {
        traverser.ctx().retag_stack(AncestorType::PropertyDefinitionValue);
        walk_expression(traverser, field as *mut _);
    }
    if let Some(field) = &mut *((node as *mut u8)
        ...
        as *mut Option<Box<TSTypeAnnotation>>)
    {
        traverser.ctx().retag_stack(AncestorType::PropertyDefinitionTypeAnnotation);
        walk_ts_type_annotation(traverser, (&mut **field) as *mut _);
    }
    traverser.ctx().retag_stack(AncestorType::PropertyDefinitionDecorators);
    ...
    traverser.ctx().pop_stack();
    traverser.exit_property_definition(&mut *node);
}
...
pub(crate) unsafe fn walk_private_identifier<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_private_identifier(&mut *node);
    traverser.exit_private_identifier(&mut *node);
}
...
pub(crate) unsafe fn walk_static_block<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_static_block(&mut *node);
    traverser.ctx().push_stack(Ancestor::StaticBlockBody(ancestor::StaticBlockWithoutBody(node)));
    walk_statements(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_STATIC_BLOCK_BODY) as *mut Vec<Statement>,
    );
    traverser.ctx().pop_stack();
    traverser.exit_static_block(&mut *node);
}
...
pub(crate) unsafe fn walk_module_declaration<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_module_declaration(&mut *node);
    match &mut *node {
        ModuleDeclaration::ImportDeclaration(node) => {
            walk_import_declaration(traverser, (&mut **node) as *mut _)
        }
        ModuleDeclaration::ExportAllDeclaration(node) => {
            walk_export_all_declaration(traverser, (&mut **node) as *mut _)
        }
        ModuleDeclaration::ExportDefaultDeclaration(node) => {
            walk_export_default_declaration(traverser, (&mut **node) as *mut _)
        }
        ModuleDeclaration::ExportNamedDeclaration(node) => {
            walk_export_named_declaration(traverser, (&mut **node) as *mut _)
        }
        ModuleDeclaration::TSExportAssignment(node) => {
            walk_ts_export_assignment(traverser, (&mut **node) as *mut _)
        }
        ModuleDeclaration::TSNamespaceExportDeclaration(node) => {
            walk_ts_namespace_export_declaration(traverser, (&mut **node) as *mut _)
        }
    }
    traverser.exit_module_declaration(&mut *node);
}
...
pub(crate) unsafe fn walk_accessor_property<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_accessor_property(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::AccessorPropertyKey(ancestor::AccessorPropertyWithoutKey(node)));
    walk_property_key(
        traverser,
    ...
    if let Some(field) = &mut *((node as *mut u8).add(ancestor::OFFSET_ACCESSOR_PROPERTY_VALUE)
        as *mut Option<Expression>)
    {
        traverser.ctx().retag_stack(AncestorType::AccessorPropertyValue);
        walk_expression(traverser, field as *mut _);
    }
    traverser.ctx().retag_stack(AncestorType::AccessorPropertyDecorators);
    ...
    traverser.ctx().pop_stack();
    traverser.exit_accessor_property(&mut *node);
}
...
pub(crate) unsafe fn walk_import_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_import_expression(&mut *node);
    traverser.ctx().push_stack(Ancestor::ImportExpressionSource(
        ancestor::ImportExpressionWithoutSource(node),
    ));
    walk_expression(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_IMPORT_EXPRESSION_SOURCE) as *mut Expression,
    );
    traverser.ctx().retag_stack(AncestorType::ImportExpressionArguments);
    ...
    traverser.ctx().pop_stack();
    traverser.exit_import_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_import_declaration<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_import_declaration(&mut *node);
    traverser.ctx().push_stack(Ancestor::ImportDeclarationSpecifiers(
    ...
    {
        for item in field.iter_mut() {
            walk_import_declaration_specifier(traverser, item as *mut _);
        }
    }
    traverser.ctx().retag_stack(AncestorType::ImportDeclarationSource);
    walk_string_literal(
        traverser,
    ...
    {
        traverser.ctx().retag_stack(AncestorType::ImportDeclarationWithClause);
        walk_with_clause(traverser, field as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_import_declaration(&mut *node);
}
...
pub(crate) unsafe fn walk_import_declaration_specifier<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_import_declaration_specifier(&mut *node);
    match &mut *node {
        ImportDeclarationSpecifier::ImportSpecifier(node) => {
            walk_import_specifier(traverser, (&mut **node) as *mut _)
        }
        ImportDeclarationSpecifier::ImportDefaultSpecifier(node) => {
            walk_import_default_specifier(traverser, (&mut **node) as *mut _)
        }
        ImportDeclarationSpecifier::ImportNamespaceSpecifier(node) => {
            walk_import_namespace_specifier(traverser, (&mut **node) as *mut _)
        }
    }
    traverser.exit_import_declaration_specifier(&mut *node);
}
...
pub(crate) unsafe fn walk_import_specifier<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_import_specifier(&mut *node);
    traverser.ctx().push_stack(Ancestor::ImportSpecifierImported(
        ancestor::ImportSpecifierWithoutImported(node),
    ));
    walk_module_export_name(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_IMPORT_SPECIFIER_IMPORTED) as *mut ModuleExportName,
    );
    traverser.ctx().retag_stack(AncestorType::ImportSpecifierLocal);
    walk_binding_identifier(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_IMPORT_SPECIFIER_LOCAL) as *mut BindingIdentifier,
    );
    traverser.ctx().pop_stack();
    traverser.exit_import_specifier(&mut *node);
}
...
pub(crate) unsafe fn walk_import_default_specifier<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_import_default_specifier(&mut *node);
    traverser.ctx().push_stack(Ancestor::ImportDefaultSpecifierLocal(
    ...
    traverser.ctx().pop_stack();
    traverser.exit_import_default_specifier(&mut *node);
}
...
pub(crate) unsafe fn walk_import_namespace_specifier<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_import_namespace_specifier(&mut *node);
    traverser.ctx().push_stack(Ancestor::ImportNamespaceSpecifierLocal(
    ...
    traverser.ctx().pop_stack();
    traverser.exit_import_namespace_specifier(&mut *node);
}
...
pub(crate) unsafe fn walk_with_clause<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_with_clause(&mut *node);
    traverser.ctx().push_stack(Ancestor::WithClauseAttributesKeyword(
    ...
    traverser.ctx().retag_stack(AncestorType::WithClauseWithEntries);
    for item in (*((node as *mut u8).add(ancestor::OFFSET_WITH_CLAUSE_WITH_ENTRIES)
        as *mut Vec<ImportAttribute>))
        .iter_mut()
    {
        walk_import_attribute(traverser, item as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_with_clause(&mut *node);
}
...
pub(crate) unsafe fn walk_import_attribute<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_import_attribute(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::ImportAttributeKey(ancestor::ImportAttributeWithoutKey(node)));
    walk_import_attribute_key(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_IMPORT_ATTRIBUTE_KEY) as *mut ImportAttributeKey,
    );
    traverser.ctx().retag_stack(AncestorType::ImportAttributeValue);
    walk_string_literal(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_IMPORT_ATTRIBUTE_VALUE) as *mut StringLiteral,
    );
    traverser.ctx().pop_stack();
    traverser.exit_import_attribute(&mut *node);
}
...
pub(crate) unsafe fn walk_import_attribute_key<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_import_attribute_key(&mut *node);
    match &mut *node {
        ImportAttributeKey::Identifier(node) => walk_identifier_name(traverser, node as *mut _),
        ImportAttributeKey::StringLiteral(node) => walk_string_literal(traverser, node as *mut _),
    }
    traverser.exit_import_attribute_key(&mut *node);
}
...
pub(crate) unsafe fn walk_export_named_declaration<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_export_named_declaration(&mut *node);
    traverser.ctx().push_stack(Ancestor::ExportNamedDeclarationDeclaration(
    ...
    {
        walk_declaration(traverser, field as *mut _);
    }
    traverser.ctx().retag_stack(AncestorType::ExportNamedDeclarationSpecifiers);
    for item in (*((node as *mut u8).add(ancestor::OFFSET_EXPORT_NAMED_DECLARATION_SPECIFIERS)
        as *mut Vec<ExportSpecifier>))
        .iter_mut()
    {
        walk_export_specifier(traverser, item as *mut _);
    }
    if let Some(field) = &mut *((node as *mut u8)
        .add(ancestor::OFFSET_EXPORT_NAMED_DECLARATION_SOURCE)
        as *mut Option<StringLiteral>)
    {
        traverser.ctx().retag_stack(AncestorType::ExportNamedDeclarationSource);
        walk_string_literal(traverser, field as *mut _);
    }
    if let Some(field) = &mut *((node as *mut u8)
        .add(ancestor::OFFSET_EXPORT_NAMED_DECLARATION_WITH_CLAUSE)
        as *mut Option<WithClause>)
    {
        traverser.ctx().retag_stack(AncestorType::ExportNamedDeclarationWithClause);
        walk_with_clause(traverser, field as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_export_named_declaration(&mut *node);
}
...
pub(crate) unsafe fn walk_export_default_declaration<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_export_default_declaration(&mut *node);
    traverser.ctx().push_stack(Ancestor::ExportDefaultDeclarationDeclaration(
    ...
    traverser.ctx().retag_stack(AncestorType::ExportDefaultDeclarationExported);
    walk_module_export_name(
        traverser,
    ...
    traverser.ctx().pop_stack();
    traverser.exit_export_default_declaration(&mut *node);
}
...
pub(crate) unsafe fn walk_export_all_declaration<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_export_all_declaration(&mut *node);
    traverser.ctx().push_stack(Ancestor::ExportAllDeclarationExported(
    ...
    {
        walk_module_export_name(traverser, field as *mut _);
    }
    traverser.ctx().retag_stack(AncestorType::ExportAllDeclarationSource);
    walk_string_literal(
        traverser,
    ...
    if let Some(field) = &mut *((node as *mut u8)
        ...
        as *mut Option<WithClause>)
    {
        traverser.ctx().retag_stack(AncestorType::ExportAllDeclarationWithClause);
        walk_with_clause(traverser, field as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_export_all_declaration(&mut *node);
}
...
pub(crate) unsafe fn walk_export_specifier<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_export_specifier(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::ExportSpecifierLocal(ancestor::ExportSpecifierWithoutLocal(node)));
    walk_module_export_name(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_EXPORT_SPECIFIER_LOCAL) as *mut ModuleExportName,
    );
    traverser.ctx().retag_stack(AncestorType::ExportSpecifierExported);
    walk_module_export_name(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_EXPORT_SPECIFIER_EXPORTED) as *mut ModuleExportName,
    );
    traverser.ctx().pop_stack();
    traverser.exit_export_specifier(&mut *node);
}
...
pub(crate) unsafe fn walk_export_default_declaration_kind<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_export_default_declaration_kind(&mut *node);
    match &mut *node {
        ExportDefaultDeclarationKind::FunctionDeclaration(node) => {
            walk_function(traverser, (&mut **node) as *mut _)
        }
        ExportDefaultDeclarationKind::ClassDeclaration(node) => {
            walk_class(traverser, (&mut **node) as *mut _)
        }
    ...
    traverser.exit_export_default_declaration_kind(&mut *node);
}
...
pub(crate) unsafe fn walk_module_export_name<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_module_export_name(&mut *node);
    match &mut *node {
        ModuleExportName::Identifier(node) => walk_identifier_name(traverser, node as *mut _),
        ModuleExportName::StringLiteral(node) => walk_string_literal(traverser, node as *mut _),
    }
    traverser.exit_module_export_name(&mut *node);
}
...
pub(crate) unsafe fn walk_jsx_element<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_jsx_element(&mut *node);
    traverser.ctx().push_stack(Ancestor::JSXElementOpeningElement(
        ancestor::JSXElementWithoutOpeningElement(node),
    ));
    ...
    {
        traverser.ctx().retag_stack(AncestorType::JSXElementClosingElement);
        walk_jsx_closing_element(traverser, (&mut **field) as *mut _);
    }
    traverser.ctx().retag_stack(AncestorType::JSXElementChildren);
    for item in (*((node as *mut u8).add(ancestor::OFFSET_JSX_ELEMENT_CHILDREN)
        as *mut Vec<JSXChild>))
        .iter_mut()
    {
        walk_jsx_child(traverser, item as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_jsx_element(&mut *node);
}
...
pub(crate) unsafe fn walk_jsx_opening_element<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_jsx_opening_element(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::JSXOpeningElementName(ancestor::JSXOpeningElementWithoutName(node)));
    walk_jsx_element_name(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_JSX_OPENING_ELEMENT_NAME) as *mut JSXElementName,
    );
    traverser.ctx().retag_stack(AncestorType::JSXOpeningElementAttributes);
    for item in (*((node as *mut u8).add(ancestor::OFFSET_JSX_OPENING_ELEMENT_ATTRIBUTES)
        as *mut Vec<JSXAttributeItem>))
        .iter_mut()
    {
        walk_jsx_attribute_item(traverser, item as *mut _);
    }
    if let Some(field) = &mut *((node as *mut u8)
        .add(ancestor::OFFSET_JSX_OPENING_ELEMENT_TYPE_PARAMETERS)
        as *mut Option<Box<TSTypeParameterInstantiation>>)
    {
        traverser.ctx().retag_stack(AncestorType::JSXOpeningElementTypeParameters);
        walk_ts_type_parameter_instantiation(traverser, (&mut **field) as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_jsx_opening_element(&mut *node);
}
...
pub(crate) unsafe fn walk_jsx_closing_element<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_jsx_closing_element(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::JSXClosingElementName(ancestor::JSXClosingElementWithoutName(node)));
    walk_jsx_element_name(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_JSX_CLOSING_ELEMENT_NAME) as *mut JSXElementName,
    );
    traverser.ctx().pop_stack();
    traverser.exit_jsx_closing_element(&mut *node);
}
...
pub(crate) unsafe fn walk_jsx_fragment<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_jsx_fragment(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::JSXFragmentChildren(ancestor::JSXFragmentWithoutChildren(node)));
    ...
    traverser.ctx().pop_stack();
    traverser.exit_jsx_fragment(&mut *node);
}
...
pub(crate) unsafe fn walk_jsx_element_name<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_jsx_element_name(&mut *node);
    match &mut *node {
        JSXElementName::Identifier(node) => walk_jsx_identifier(traverser, (&mut **node) as *mut _),
        JSXElementName::NamespacedName(node) => {
            walk_jsx_namespaced_name(traverser, (&mut **node) as *mut _)
        }
        JSXElementName::MemberExpression(node) => {
            walk_jsx_member_expression(traverser, (&mut **node) as *mut _)
        }
    }
    traverser.exit_jsx_element_name(&mut *node);
}
...
pub(crate) unsafe fn walk_jsx_namespaced_name<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_jsx_namespaced_name(&mut *node);
    traverser.ctx().push_stack(Ancestor::JSXNamespacedNameNamespace(
    ...
    traverser.ctx().retag_stack(AncestorType::JSXNamespacedNameProperty);
    walk_jsx_identifier(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_JSX_NAMESPACED_NAME_PROPERTY) as *mut JSXIdentifier,
    );
    traverser.ctx().pop_stack();
    traverser.exit_jsx_namespaced_name(&mut *node);
}
...
pub(crate) unsafe fn walk_jsx_member_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_jsx_member_expression(&mut *node);
    traverser.ctx().push_stack(Ancestor::JSXMemberExpressionObject(
    ...
    traverser.ctx().retag_stack(AncestorType::JSXMemberExpressionProperty);
    walk_jsx_identifier(
        traverser,
    ...
    traverser.ctx().pop_stack();
    traverser.exit_jsx_member_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_jsx_member_expression_object<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_jsx_member_expression_object(&mut *node);
    match &mut *node {
        JSXMemberExpressionObject::Identifier(node) => {
            walk_jsx_identifier(traverser, (&mut **node) as *mut _)
        }
        JSXMemberExpressionObject::MemberExpression(node) => {
            walk_jsx_member_expression(traverser, (&mut **node) as *mut _)
        }
    }
    traverser.exit_jsx_member_expression_object(&mut *node);
}
...
pub(crate) unsafe fn walk_jsx_expression_container<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_jsx_expression_container(&mut *node);
    traverser.ctx().push_stack(Ancestor::JSXExpressionContainerExpression(
    ...
    traverser.ctx().pop_stack();
    traverser.exit_jsx_expression_container(&mut *node);
}
...
pub(crate) unsafe fn walk_jsx_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_jsx_expression(&mut *node);
    match &mut *node {
        JSXExpression::EmptyExpression(node) => {
            walk_jsx_empty_expression(traverser, node as *mut _)
        }
        ...
        | JSXExpression::PrivateFieldExpression(_) => walk_expression(traverser, node as *mut _),
    }
    traverser.exit_jsx_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_jsx_empty_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_jsx_empty_expression(&mut *node);
    traverser.exit_jsx_empty_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_jsx_attribute_item<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_jsx_attribute_item(&mut *node);
    match &mut *node {
        JSXAttributeItem::Attribute(node) => walk_jsx_attribute(traverser, (&mut **node) as *mut _),
        JSXAttributeItem::SpreadAttribute(node) => {
            walk_jsx_spread_attribute(traverser, (&mut **node) as *mut _)
        }
    }
    traverser.exit_jsx_attribute_item(&mut *node);
}
...
pub(crate) unsafe fn walk_jsx_attribute<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_jsx_attribute(&mut *node);
    traverser.ctx().push_stack(Ancestor::JSXAttributeName(ancestor::JSXAttributeWithoutName(node)));
    ...
    {
        traverser.ctx().retag_stack(AncestorType::JSXAttributeValue);
        walk_jsx_attribute_value(traverser, field as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_jsx_attribute(&mut *node);
}
...
pub(crate) unsafe fn walk_jsx_spread_attribute<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_jsx_spread_attribute(&mut *node);
    traverser.ctx().push_stack(Ancestor::JSXSpreadAttributeArgument(
    ...
    traverser.ctx().pop_stack();
    traverser.exit_jsx_spread_attribute(&mut *node);
}
...
pub(crate) unsafe fn walk_jsx_attribute_name<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_jsx_attribute_name(&mut *node);
    match &mut *node {
    ...
    traverser.exit_jsx_attribute_name(&mut *node);
}
...
pub(crate) unsafe fn walk_jsx_attribute_value<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_jsx_attribute_value(&mut *node);
    match &mut *node {
        JSXAttributeValue::StringLiteral(node) => {
            walk_string_literal(traverser, (&mut **node) as *mut _)
        }
        JSXAttributeValue::ExpressionContainer(node) => {
            walk_jsx_expression_container(traverser, (&mut **node) as *mut _)
        }
        JSXAttributeValue::Element(node) => walk_jsx_element(traverser, (&mut **node) as *mut _),
        JSXAttributeValue::Fragment(node) => walk_jsx_fragment(traverser, (&mut **node) as *mut _),
    }
    traverser.exit_jsx_attribute_value(&mut *node);
}
...
pub(crate) unsafe fn walk_jsx_identifier<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_jsx_identifier(&mut *node);
    traverser.exit_jsx_identifier(&mut *node);
}
...
pub(crate) unsafe fn walk_jsx_child<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_jsx_child(&mut *node);
    match &mut *node {
        JSXChild::Text(node) => walk_jsx_text(traverser, (&mut **node) as *mut _),
        JSXChild::Element(node) => walk_jsx_element(traverser, (&mut **node) as *mut _),
        JSXChild::Fragment(node) => walk_jsx_fragment(traverser, (&mut **node) as *mut _),
        JSXChild::ExpressionContainer(node) => {
            walk_jsx_expression_container(traverser, (&mut **node) as *mut _)
        }
        JSXChild::Spread(node) => walk_jsx_spread_child(traverser, (&mut **node) as *mut _),
    }
    traverser.exit_jsx_child(&mut *node);
}
...
pub(crate) unsafe fn walk_jsx_spread_child<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_jsx_spread_child(&mut *node);
    traverser.ctx().push_stack(Ancestor::JSXSpreadChildExpression(
        ancestor::JSXSpreadChildWithoutExpression(node),
    ));
    walk_expression(
        traverser,
    ...
    traverser.ctx().pop_stack();
    traverser.exit_jsx_spread_child(&mut *node);
}
...
pub(crate) unsafe fn walk_jsx_text<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_jsx_text(&mut *node);
    traverser.exit_jsx_text(&mut *node);
}
...
pub(crate) unsafe fn walk_boolean_literal<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_boolean_literal(&mut *node);
    traverser.exit_boolean_literal(&mut *node);
}
...
pub(crate) unsafe fn walk_null_literal<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_null_literal(&mut *node);
    traverser.exit_null_literal(&mut *node);
}
...
pub(crate) unsafe fn walk_numeric_literal<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_numeric_literal(&mut *node);
    traverser.exit_numeric_literal(&mut *node);
}
...
pub(crate) unsafe fn walk_big_int_literal<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_big_int_literal(&mut *node);
    traverser.exit_big_int_literal(&mut *node);
}
...
pub(crate) unsafe fn walk_reg_exp_literal<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_reg_exp_literal(&mut *node);
    traverser.exit_reg_exp_literal(&mut *node);
}
...
pub(crate) unsafe fn walk_string_literal<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_string_literal(&mut *node);
    traverser.exit_string_literal(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_this_parameter<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_this_parameter(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::TSThisParameterThis(ancestor::TSThisParameterWithoutThis(node)));
    walk_identifier_name(
        traverser,
    ...
    if let Some(field) = &mut *((node as *mut u8)
        ...
        as *mut Option<Box<TSTypeAnnotation>>)
    {
        traverser.ctx().retag_stack(AncestorType::TSThisParameterTypeAnnotation);
        walk_ts_type_annotation(traverser, (&mut **field) as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_ts_this_parameter(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_enum_declaration<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_enum_declaration(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::TSEnumDeclarationId(ancestor::TSEnumDeclarationWithoutId(node)));
    walk_binding_identifier(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_TS_ENUM_DECLARATION_ID) as *mut BindingIdentifier,
    );
    traverser.ctx().retag_stack(AncestorType::TSEnumDeclarationMembers);
    for item in (*((node as *mut u8).add(ancestor::OFFSET_TS_ENUM_DECLARATION_MEMBERS)
        as *mut Vec<TSEnumMember>))
        .iter_mut()
    {
        walk_ts_enum_member(traverser, item as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_ts_enum_declaration(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_enum_member<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_enum_member(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSEnumMemberId(ancestor::TSEnumMemberWithoutId(node)));
    ...
    if let Some(field) = &mut *((node as *mut u8).add(ancestor::OFFSET_TS_ENUM_MEMBER_INITIALIZER)
        as *mut Option<Expression>)
    {
        traverser.ctx().retag_stack(AncestorType::TSEnumMemberInitializer);
        walk_expression(traverser, field as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_ts_enum_member(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_enum_member_name<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_enum_member_name(&mut *node);
    match &mut *node {
        ...
        | TSEnumMemberName::PrivateFieldExpression(_) => walk_expression(traverser, node as *mut _),
    }
    traverser.exit_ts_enum_member_name(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_type_annotation<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_type_annotation(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSTypeAnnotationTypeAnnotation(
    ...
    traverser.ctx().pop_stack();
    traverser.exit_ts_type_annotation(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_literal_type<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_literal_type(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::TSLiteralTypeLiteral(ancestor::TSLiteralTypeWithoutLiteral(node)));
    walk_ts_literal(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_TS_LITERAL_TYPE_LITERAL) as *mut TSLiteral,
    );
    traverser.ctx().pop_stack();
    traverser.exit_ts_literal_type(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_literal<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_literal(&mut *node);
    match &mut *node {
        TSLiteral::BooleanLiteral(node) => walk_boolean_literal(traverser, (&mut **node) as *mut _),
        TSLiteral::NullLiteral(node) => walk_null_literal(traverser, (&mut **node) as *mut _),
        TSLiteral::NumericLiteral(node) => walk_numeric_literal(traverser, (&mut **node) as *mut _),
        TSLiteral::BigintLiteral(node) => walk_big_int_literal(traverser, (&mut **node) as *mut _),
        TSLiteral::RegExpLiteral(node) => walk_reg_exp_literal(traverser, (&mut **node) as *mut _),
        TSLiteral::StringLiteral(node) => walk_string_literal(traverser, (&mut **node) as *mut _),
    ...
    traverser.exit_ts_literal(&mut *node);
}

pub(crate) unsafe fn walk_ts_type<'a, Tr: Traverse<'a>>(traverser: &mut Tr, node: *mut TSType<'a>) {
    traverser.enter_ts_type(&mut *node);
    match &mut *node {
        TSType::TSAnyKeyword(node) => walk_ts_any_keyword(traverser, (&mut **node) as *mut _),
        TSType::TSBigIntKeyword(node) => {
            walk_ts_big_int_keyword(traverser, (&mut **node) as *mut _)
        }
        TSType::TSBooleanKeyword(node) => {
            walk_ts_boolean_keyword(traverser, (&mut **node) as *mut _)
        }
        TSType::TSNeverKeyword(node) => walk_ts_never_keyword(traverser, (&mut **node) as *mut _),
        TSType::TSNullKeyword(node) => walk_ts_null_keyword(traverser, (&mut **node) as *mut _),
        TSType::TSNumberKeyword(node) => walk_ts_number_keyword(traverser, (&mut **node) as *mut _),
        TSType::TSObjectKeyword(node) => walk_ts_object_keyword(traverser, (&mut **node) as *mut _),
        TSType::TSStringKeyword(node) => walk_ts_string_keyword(traverser, (&mut **node) as *mut _),
        TSType::TSSymbolKeyword(node) => walk_ts_symbol_keyword(traverser, (&mut **node) as *mut _),
        TSType::TSThisType(node) => walk_ts_this_type(traverser, (&mut **node) as *mut _),
        TSType::TSUndefinedKeyword(node) => {
            walk_ts_undefined_keyword(traverser, (&mut **node) as *mut _)
        }
        TSType::TSUnknownKeyword(node) => {
            walk_ts_unknown_keyword(traverser, (&mut **node) as *mut _)
        }
        TSType::TSVoidKeyword(node) => walk_ts_void_keyword(traverser, (&mut **node) as *mut _),
        TSType::TSArrayType(node) => walk_ts_array_type(traverser, (&mut **node) as *mut _),
        TSType::TSConditionalType(node) => {
            walk_ts_conditional_type(traverser, (&mut **node) as *mut _)
        }
        TSType::TSConstructorType(node) => {
            walk_ts_constructor_type(traverser, (&mut **node) as *mut _)
        }
        TSType::TSFunctionType(node) => walk_ts_function_type(traverser, (&mut **node) as *mut _),
        TSType::TSImportType(node) => walk_ts_import_type(traverser, (&mut **node) as *mut _),
        TSType::TSIndexedAccessType(node) => {
            walk_ts_indexed_access_type(traverser, (&mut **node) as *mut _)
        }
        TSType::TSInferType(node) => walk_ts_infer_type(traverser, (&mut **node) as *mut _),
        TSType::TSIntersectionType(node) => {
            walk_ts_intersection_type(traverser, (&mut **node) as *mut _)
        }
        TSType::TSLiteralType(node) => walk_ts_literal_type(traverser, (&mut **node) as *mut _),
        TSType::TSMappedType(node) => walk_ts_mapped_type(traverser, (&mut **node) as *mut _),
        TSType::TSNamedTupleMember(node) => {
            walk_ts_named_tuple_member(traverser, (&mut **node) as *mut _)
        }
        TSType::TSQualifiedName(node) => walk_ts_qualified_name(traverser, (&mut **node) as *mut _),
        TSType::TSTemplateLiteralType(node) => {
            walk_ts_template_literal_type(traverser, (&mut **node) as *mut _)
        }
        TSType::TSTupleType(node) => walk_ts_tuple_type(traverser, (&mut **node) as *mut _),
        TSType::TSTypeLiteral(node) => walk_ts_type_literal(traverser, (&mut **node) as *mut _),
        TSType::TSTypeOperatorType(node) => {
            walk_ts_type_operator(traverser, (&mut **node) as *mut _)
        }
        TSType::TSTypePredicate(node) => walk_ts_type_predicate(traverser, (&mut **node) as *mut _),
        TSType::TSTypeQuery(node) => walk_ts_type_query(traverser, (&mut **node) as *mut _),
        TSType::TSTypeReference(node) => walk_ts_type_reference(traverser, (&mut **node) as *mut _),
        TSType::TSUnionType(node) => walk_ts_union_type(traverser, (&mut **node) as *mut _),
        TSType::JSDocNullableType(node) => {
            walk_js_doc_nullable_type(traverser, (&mut **node) as *mut _)
        }
        TSType::JSDocUnknownType(node) => {
            walk_js_doc_unknown_type(traverser, (&mut **node) as *mut _)
        }
    }
    traverser.exit_ts_type(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_conditional_type<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_conditional_type(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSConditionalTypeCheckType(
    ...
    traverser.ctx().retag_stack(AncestorType::TSConditionalTypeExtendsType);
    walk_ts_type(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_TS_CONDITIONAL_TYPE_EXTENDS_TYPE) as *mut TSType,
    );
    traverser.ctx().retag_stack(AncestorType::TSConditionalTypeTrueType);
    walk_ts_type(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_TS_CONDITIONAL_TYPE_TRUE_TYPE) as *mut TSType,
    );
    traverser.ctx().retag_stack(AncestorType::TSConditionalTypeFalseType);
    walk_ts_type(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_TS_CONDITIONAL_TYPE_FALSE_TYPE) as *mut TSType,
    );
    traverser.ctx().pop_stack();
    traverser.exit_ts_conditional_type(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_union_type<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_union_type(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSUnionTypeTypes(ancestor::TSUnionTypeWithoutTypes(node)));
    for item in (*((node as *mut u8).add(ancestor::OFFSET_TS_UNION_TYPE_TYPES) as *mut Vec<TSType>))
        .iter_mut()
    {
        walk_ts_type(traverser, item as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_ts_union_type(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_intersection_type<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_intersection_type(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSIntersectionTypeTypes(
        ancestor::TSIntersectionTypeWithoutTypes(node),
    ));
    ...
    traverser.ctx().pop_stack();
    traverser.exit_ts_intersection_type(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_type_operator<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_type_operator(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSTypeOperatorTypeAnnotation(
    ...
    traverser.ctx().pop_stack();
    traverser.exit_ts_type_operator(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_array_type<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_array_type(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSArrayTypeElementType(
        ancestor::TSArrayTypeWithoutElementType(node),
    ));
    walk_ts_type(
        traverser,
    ...
    traverser.ctx().pop_stack();
    traverser.exit_ts_array_type(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_indexed_access_type<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_indexed_access_type(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSIndexedAccessTypeObjectType(
    ...
    traverser.ctx().retag_stack(AncestorType::TSIndexedAccessTypeIndexType);
    walk_ts_type(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_TS_INDEXED_ACCESS_TYPE_INDEX_TYPE) as *mut TSType,
    );
    traverser.ctx().pop_stack();
    traverser.exit_ts_indexed_access_type(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_tuple_type<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_tuple_type(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSTupleTypeElementTypes(
        ancestor::TSTupleTypeWithoutElementTypes(node),
    ));
    for item in (*((node as *mut u8).add(ancestor::OFFSET_TS_TUPLE_TYPE_ELEMENT_TYPES)
        as *mut Vec<TSTupleElement>))
        .iter_mut()
    {
        walk_ts_tuple_element(traverser, item as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_ts_tuple_type(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_named_tuple_member<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_named_tuple_member(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSNamedTupleMemberElementType(
    ...
    traverser.ctx().retag_stack(AncestorType::TSNamedTupleMemberLabel);
    walk_identifier_name(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_TS_NAMED_TUPLE_MEMBER_LABEL) as *mut IdentifierName,
    );
    traverser.ctx().pop_stack();
    traverser.exit_ts_named_tuple_member(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_optional_type<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_optional_type(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSOptionalTypeTypeAnnotation(
    ...
    traverser.ctx().pop_stack();
    traverser.exit_ts_optional_type(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_rest_type<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_rest_type(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSRestTypeTypeAnnotation(
        ancestor::TSRestTypeWithoutTypeAnnotation(node),
    ));
    walk_ts_type(
        traverser,
    ...
    traverser.ctx().pop_stack();
    traverser.exit_ts_rest_type(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_tuple_element<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_tuple_element(&mut *node);
    match &mut *node {
        TSTupleElement::TSOptionalType(node) => {
            walk_ts_optional_type(traverser, (&mut **node) as *mut _)
        }
        TSTupleElement::TSRestType(node) => walk_ts_rest_type(traverser, (&mut **node) as *mut _),
        ...
        | TSTupleElement::JSDocUnknownType(_) => walk_ts_type(traverser, node as *mut _),
    }
    traverser.exit_ts_tuple_element(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_any_keyword<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_any_keyword(&mut *node);
    traverser.exit_ts_any_keyword(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_string_keyword<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_string_keyword(&mut *node);
    traverser.exit_ts_string_keyword(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_boolean_keyword<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_boolean_keyword(&mut *node);
    traverser.exit_ts_boolean_keyword(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_number_keyword<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_number_keyword(&mut *node);
    traverser.exit_ts_number_keyword(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_never_keyword<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_never_keyword(&mut *node);
    traverser.exit_ts_never_keyword(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_unknown_keyword<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_unknown_keyword(&mut *node);
    traverser.exit_ts_unknown_keyword(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_null_keyword<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_null_keyword(&mut *node);
    traverser.exit_ts_null_keyword(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_undefined_keyword<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_undefined_keyword(&mut *node);
    traverser.exit_ts_undefined_keyword(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_void_keyword<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_void_keyword(&mut *node);
    traverser.exit_ts_void_keyword(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_symbol_keyword<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_symbol_keyword(&mut *node);
    traverser.exit_ts_symbol_keyword(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_this_type<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_this_type(&mut *node);
    traverser.exit_ts_this_type(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_object_keyword<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_object_keyword(&mut *node);
    traverser.exit_ts_object_keyword(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_big_int_keyword<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_big_int_keyword(&mut *node);
    traverser.exit_ts_big_int_keyword(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_type_reference<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_type_reference(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSTypeReferenceTypeName(
        ancestor::TSTypeReferenceWithoutTypeName(node),
    ));
    walk_ts_type_name(
        traverser,
    ...
    if let Some(field) = &mut *((node as *mut u8)
        ...
        as *mut Option<Box<TSTypeParameterInstantiation>>)
    {
        traverser.ctx().retag_stack(AncestorType::TSTypeReferenceTypeParameters);
        walk_ts_type_parameter_instantiation(traverser, (&mut **field) as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_ts_type_reference(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_type_name<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_type_name(&mut *node);
    match &mut *node {
        TSTypeName::IdentifierReference(node) => {
            walk_identifier_reference(traverser, (&mut **node) as *mut _)
        }
        TSTypeName::QualifiedName(node) => {
            walk_ts_qualified_name(traverser, (&mut **node) as *mut _)
        }
    }
    traverser.exit_ts_type_name(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_qualified_name<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_qualified_name(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::TSQualifiedNameLeft(ancestor::TSQualifiedNameWithoutLeft(node)));
    walk_ts_type_name(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_TS_QUALIFIED_NAME_LEFT) as *mut TSTypeName,
    );
    traverser.ctx().retag_stack(AncestorType::TSQualifiedNameRight);
    walk_identifier_name(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_TS_QUALIFIED_NAME_RIGHT) as *mut IdentifierName,
    );
    traverser.ctx().pop_stack();
    traverser.exit_ts_qualified_name(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_type_parameter_instantiation<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_type_parameter_instantiation(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSTypeParameterInstantiationParams(
    ...
    traverser.ctx().pop_stack();
    traverser.exit_ts_type_parameter_instantiation(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_type_parameter<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_type_parameter(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::TSTypeParameterName(ancestor::TSTypeParameterWithoutName(node)));
    walk_binding_identifier(
        traverser,
    ...
    {
        traverser.ctx().retag_stack(AncestorType::TSTypeParameterConstraint);
        walk_ts_type(traverser, field as *mut _);
    }
    if let Some(field) = &mut *((node as *mut u8).add(ancestor::OFFSET_TS_TYPE_PARAMETER_DEFAULT)
        as *mut Option<TSType>)
    {
        traverser.ctx().retag_stack(AncestorType::TSTypeParameterDefault);
        walk_ts_type(traverser, field as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_ts_type_parameter(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_type_parameter_declaration<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_type_parameter_declaration(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSTypeParameterDeclarationParams(
    ...
    for item in (*((node as *mut u8).add(ancestor::OFFSET_TS_TYPE_PARAMETER_DECLARATION_PARAMS)
        ...
        .iter_mut()
    {
        walk_ts_type_parameter(traverser, item as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_ts_type_parameter_declaration(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_type_alias_declaration<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_type_alias_declaration(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSTypeAliasDeclarationId(
        ancestor::TSTypeAliasDeclarationWithoutId(node),
    ));
    walk_binding_identifier(
        traverser,
    ...
    traverser.ctx().retag_stack(AncestorType::TSTypeAliasDeclarationTypeAnnotation);
    walk_ts_type(
        traverser,
    ...
    if let Some(field) = &mut *((node as *mut u8)
        ...
        as *mut Option<Box<TSTypeParameterDeclaration>>)
    {
        traverser.ctx().retag_stack(AncestorType::TSTypeAliasDeclarationTypeParameters);
        walk_ts_type_parameter_declaration(traverser, (&mut **field) as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_ts_type_alias_declaration(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_class_implements<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_class_implements(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSClassImplementsExpression(
    ...
    if let Some(field) = &mut *((node as *mut u8)
        ...
        as *mut Option<Box<TSTypeParameterInstantiation>>)
    {
        traverser.ctx().retag_stack(AncestorType::TSClassImplementsTypeParameters);
        walk_ts_type_parameter_instantiation(traverser, (&mut **field) as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_ts_class_implements(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_interface_declaration<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_interface_declaration(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSInterfaceDeclarationId(
        ancestor::TSInterfaceDeclarationWithoutId(node),
    ));
    walk_binding_identifier(
        traverser,
    ...
    traverser.ctx().retag_stack(AncestorType::TSInterfaceDeclarationBody);
    ...
    if let Some(field) = &mut *((node as *mut u8)
        ...
        as *mut Option<Box<TSTypeParameterDeclaration>>)
    {
        traverser.ctx().retag_stack(AncestorType::TSInterfaceDeclarationTypeParameters);
        walk_ts_type_parameter_declaration(traverser, (&mut **field) as *mut _);
    }
    if let Some(field) = &mut *((node as *mut u8)
    ...
    {
        traverser.ctx().retag_stack(AncestorType::TSInterfaceDeclarationExtends);
        for item in field.iter_mut() {
            walk_ts_interface_heritage(traverser, item as *mut _);
        }
    }
    traverser.ctx().pop_stack();
    traverser.exit_ts_interface_declaration(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_interface_body<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_interface_body(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::TSInterfaceBodyBody(ancestor::TSInterfaceBodyWithoutBody(node)));
    for item in (*((node as *mut u8).add(ancestor::OFFSET_TS_INTERFACE_BODY_BODY)
        as *mut Vec<TSSignature>))
        .iter_mut()
    {
        walk_ts_signature(traverser, item as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_ts_interface_body(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_property_signature<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_property_signature(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSPropertySignatureKey(
        ancestor::TSPropertySignatureWithoutKey(node),
    ));
    walk_property_key(
        traverser,
    ...
    if let Some(field) = &mut *((node as *mut u8)
        ...
        as *mut Option<Box<TSTypeAnnotation>>)
    {
        traverser.ctx().retag_stack(AncestorType::TSPropertySignatureTypeAnnotation);
        walk_ts_type_annotation(traverser, (&mut **field) as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_ts_property_signature(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_signature<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_signature(&mut *node);
    match &mut *node {
        TSSignature::TSIndexSignature(node) => {
            walk_ts_index_signature(traverser, (&mut **node) as *mut _)
        }
        TSSignature::TSPropertySignature(node) => {
            walk_ts_property_signature(traverser, (&mut **node) as *mut _)
        }
        TSSignature::TSCallSignatureDeclaration(node) => {
            walk_ts_call_signature_declaration(traverser, (&mut **node) as *mut _)
        }
        TSSignature::TSConstructSignatureDeclaration(node) => {
            walk_ts_construct_signature_declaration(traverser, (&mut **node) as *mut _)
        }
        TSSignature::TSMethodSignature(node) => {
            walk_ts_method_signature(traverser, (&mut **node) as *mut _)
        }
    }
    traverser.exit_ts_signature(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_index_signature<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_index_signature(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSIndexSignatureParameters(
    ...
    for item in (*((node as *mut u8).add(ancestor::OFFSET_TS_INDEX_SIGNATURE_PARAMETERS)
        ...
        .iter_mut()
    {
        walk_ts_index_signature_name(traverser, item as *mut _);
    }
    traverser.ctx().retag_stack(AncestorType::TSIndexSignatureTypeAnnotation);
    ...
    traverser.ctx().pop_stack();
    traverser.exit_ts_index_signature(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_call_signature_declaration<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_call_signature_declaration(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSCallSignatureDeclarationThisParam(
    ...
    traverser.ctx().retag_stack(AncestorType::TSCallSignatureDeclarationParams);
    walk_formal_parameters(
        traverser,
    ...
    if let Some(field) = &mut *((node as *mut u8)
        ...
        as *mut Option<Box<TSTypeAnnotation>>)
    {
        traverser.ctx().retag_stack(AncestorType::TSCallSignatureDeclarationReturnType);
        walk_ts_type_annotation(traverser, (&mut **field) as *mut _);
    }
    if let Some(field) = &mut *((node as *mut u8)
        ...
        as *mut Option<Box<TSTypeParameterDeclaration>>)
    {
        traverser.ctx().retag_stack(AncestorType::TSCallSignatureDeclarationTypeParameters);
        walk_ts_type_parameter_declaration(traverser, (&mut **field) as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_ts_call_signature_declaration(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_method_signature<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_method_signature(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::TSMethodSignatureKey(ancestor::TSMethodSignatureWithoutKey(node)));
    walk_property_key(
        traverser,
    ...
    if let Some(field) = &mut *((node as *mut u8)
        ...
        as *mut Option<TSThisParameter>)
    {
        traverser.ctx().retag_stack(AncestorType::TSMethodSignatureThisParam);
        walk_ts_this_parameter(traverser, field as *mut _);
    }
    traverser.ctx().retag_stack(AncestorType::TSMethodSignatureParams);
    walk_formal_parameters(
        traverser,
    ...
    if let Some(field) = &mut *((node as *mut u8)
        ...
        as *mut Option<Box<TSTypeAnnotation>>)
    {
        traverser.ctx().retag_stack(AncestorType::TSMethodSignatureReturnType);
        walk_ts_type_annotation(traverser, (&mut **field) as *mut _);
    }
    if let Some(field) = &mut *((node as *mut u8)
        ...
        as *mut Option<Box<TSTypeParameterDeclaration>>)
    {
        traverser.ctx().retag_stack(AncestorType::TSMethodSignatureTypeParameters);
        walk_ts_type_parameter_declaration(traverser, (&mut **field) as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_ts_method_signature(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_construct_signature_declaration<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_construct_signature_declaration(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSConstructSignatureDeclarationParams(
    ...
    if let Some(field) = &mut *((node as *mut u8)
        ...
        as *mut Option<Box<TSTypeAnnotation>>)
    {
        traverser.ctx().retag_stack(AncestorType::TSConstructSignatureDeclarationReturnType);
        walk_ts_type_annotation(traverser, (&mut **field) as *mut _);
    }
    if let Some(field) = &mut *((node as *mut u8)
        ...
        as *mut Option<Box<TSTypeParameterDeclaration>>)
    {
        traverser.ctx().retag_stack(AncestorType::TSConstructSignatureDeclarationTypeParameters);
        walk_ts_type_parameter_declaration(traverser, (&mut **field) as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_ts_construct_signature_declaration(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_index_signature_name<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_index_signature_name(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSIndexSignatureNameTypeAnnotation(
    ...
    traverser.ctx().pop_stack();
    traverser.exit_ts_index_signature_name(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_interface_heritage<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_interface_heritage(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSInterfaceHeritageExpression(
    ...
    if let Some(field) = &mut *((node as *mut u8)
        ...
        as *mut Option<Box<TSTypeParameterInstantiation>>)
    {
        traverser.ctx().retag_stack(AncestorType::TSInterfaceHeritageTypeParameters);
        walk_ts_type_parameter_instantiation(traverser, (&mut **field) as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_ts_interface_heritage(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_type_predicate<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_type_predicate(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSTypePredicateParameterName(
    ...
    if let Some(field) = &mut *((node as *mut u8)
        ...
        as *mut Option<Box<TSTypeAnnotation>>)
    {
        traverser.ctx().retag_stack(AncestorType::TSTypePredicateTypeAnnotation);
        walk_ts_type_annotation(traverser, (&mut **field) as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_ts_type_predicate(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_type_predicate_name<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_type_predicate_name(&mut *node);
    match &mut *node {
        TSTypePredicateName::Identifier(node) => {
            walk_identifier_name(traverser, (&mut **node) as *mut _)
        }
        TSTypePredicateName::This(node) => walk_ts_this_type(traverser, node as *mut _),
    }
    traverser.exit_ts_type_predicate_name(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_module_declaration<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_module_declaration(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::TSModuleDeclarationId(ancestor::TSModuleDeclarationWithoutId(node)));
    ...
    {
        traverser.ctx().retag_stack(AncestorType::TSModuleDeclarationBody);
        walk_ts_module_declaration_body(traverser, field as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_ts_module_declaration(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_module_declaration_name<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_module_declaration_name(&mut *node);
    match &mut *node {
        TSModuleDeclarationName::Identifier(node) => {
            walk_identifier_name(traverser, node as *mut _)
        }
        TSModuleDeclarationName::StringLiteral(node) => {
            walk_string_literal(traverser, node as *mut _)
        }
    }
    traverser.exit_ts_module_declaration_name(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_module_declaration_body<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_module_declaration_body(&mut *node);
    match &mut *node {
        TSModuleDeclarationBody::TSModuleDeclaration(node) => {
            walk_ts_module_declaration(traverser, (&mut **node) as *mut _)
        }
        TSModuleDeclarationBody::TSModuleBlock(node) => {
            walk_ts_module_block(traverser, (&mut **node) as *mut _)
        }
    }
    traverser.exit_ts_module_declaration_body(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_module_block<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_module_block(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::TSModuleBlockBody(ancestor::TSModuleBlockWithoutBody(node)));
    walk_statements(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_TS_MODULE_BLOCK_BODY) as *mut Vec<Statement>,
    );
    traverser.ctx().pop_stack();
    traverser.exit_ts_module_block(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_type_literal<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_type_literal(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::TSTypeLiteralMembers(ancestor::TSTypeLiteralWithoutMembers(node)));
    ...
    traverser.ctx().pop_stack();
    traverser.exit_ts_type_literal(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_infer_type<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_infer_type(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSInferTypeTypeParameter(
        ancestor::TSInferTypeWithoutTypeParameter(node),
    ));
    ...
    traverser.ctx().pop_stack();
    traverser.exit_ts_infer_type(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_type_query<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_type_query(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::TSTypeQueryExprName(ancestor::TSTypeQueryWithoutExprName(node)));
    ...
    if let Some(field) = &mut *((node as *mut u8)
        ...
        as *mut Option<Box<TSTypeParameterInstantiation>>)
    {
        traverser.ctx().retag_stack(AncestorType::TSTypeQueryTypeParameters);
        walk_ts_type_parameter_instantiation(traverser, (&mut **field) as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_ts_type_query(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_type_query_expr_name<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_type_query_expr_name(&mut *node);
    match &mut *node {
        TSTypeQueryExprName::TSImportType(node) => {
            walk_ts_import_type(traverser, (&mut **node) as *mut _)
        }
        TSTypeQueryExprName::IdentifierReference(_) | TSTypeQueryExprName::QualifiedName(_) => {
            walk_ts_type_name(traverser, node as *mut _)
        }
    }
    traverser.exit_ts_type_query_expr_name(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_import_type<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_import_type(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::TSImportTypeArgument(ancestor::TSImportTypeWithoutArgument(node)));
    walk_ts_type(
        traverser,
    ...
    {
        traverser.ctx().retag_stack(AncestorType::TSImportTypeQualifier);
        walk_ts_type_name(traverser, field as *mut _);
    }
    if let Some(field) = &mut *((node as *mut u8).add(ancestor::OFFSET_TS_IMPORT_TYPE_ATTRIBUTES)
        as *mut Option<TSImportAttributes>)
    {
        traverser.ctx().retag_stack(AncestorType::TSImportTypeAttributes);
        walk_ts_import_attributes(traverser, field as *mut _);
    }
    if let Some(field) = &mut *((node as *mut u8)
        .add(ancestor::OFFSET_TS_IMPORT_TYPE_TYPE_PARAMETERS)
        as *mut Option<Box<TSTypeParameterInstantiation>>)
    {
        traverser.ctx().retag_stack(AncestorType::TSImportTypeTypeParameters);
        walk_ts_type_parameter_instantiation(traverser, (&mut **field) as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_ts_import_type(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_import_attributes<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_import_attributes(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSImportAttributesElements(
    ...
    for item in (*((node as *mut u8).add(ancestor::OFFSET_TS_IMPORT_ATTRIBUTES_ELEMENTS)
        ...
        .iter_mut()
    {
        walk_ts_import_attribute(traverser, item as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_ts_import_attributes(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_import_attribute<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_import_attribute(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::TSImportAttributeName(ancestor::TSImportAttributeWithoutName(node)));
    walk_ts_import_attribute_name(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_TS_IMPORT_ATTRIBUTE_NAME)
            as *mut TSImportAttributeName,
    );
    traverser.ctx().retag_stack(AncestorType::TSImportAttributeValue);
    walk_expression(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_TS_IMPORT_ATTRIBUTE_VALUE) as *mut Expression,
    );
    traverser.ctx().pop_stack();
    traverser.exit_ts_import_attribute(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_import_attribute_name<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_import_attribute_name(&mut *node);
    match &mut *node {
        TSImportAttributeName::Identifier(node) => walk_identifier_name(traverser, node as *mut _),
        TSImportAttributeName::StringLiteral(node) => {
            walk_string_literal(traverser, node as *mut _)
        }
    }
    traverser.exit_ts_import_attribute_name(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_function_type<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_function_type(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSFunctionTypeThisParam(
        ancestor::TSFunctionTypeWithoutThisParam(node),
    ));
    ...
    traverser.ctx().retag_stack(AncestorType::TSFunctionTypeParams);
    walk_formal_parameters(
        traverser,
        (&mut **((node as *mut u8).add(ancestor::OFFSET_TS_FUNCTION_TYPE_PARAMS)
            as *mut Box<FormalParameters>)) as *mut _,
    );
    traverser.ctx().retag_stack(AncestorType::TSFunctionTypeReturnType);
    walk_ts_type_annotation(
        traverser,
    ...
    if let Some(field) = &mut *((node as *mut u8)
        ...
        as *mut Option<Box<TSTypeParameterDeclaration>>)
    {
        traverser.ctx().retag_stack(AncestorType::TSFunctionTypeTypeParameters);
        walk_ts_type_parameter_declaration(traverser, (&mut **field) as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_ts_function_type(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_constructor_type<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_constructor_type(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSConstructorTypeParams(
        ancestor::TSConstructorTypeWithoutParams(node),
    ));
    walk_formal_parameters(
        traverser,
    ...
    traverser.ctx().retag_stack(AncestorType::TSConstructorTypeReturnType);
    walk_ts_type_annotation(
        traverser,
    ...
    if let Some(field) = &mut *((node as *mut u8)
        ...
        as *mut Option<Box<TSTypeParameterDeclaration>>)
    {
        traverser.ctx().retag_stack(AncestorType::TSConstructorTypeTypeParameters);
        walk_ts_type_parameter_declaration(traverser, (&mut **field) as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_ts_constructor_type(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_mapped_type<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_mapped_type(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSMappedTypeTypeParameter(
    ...
    if let Some(field) = &mut *((node as *mut u8).add(ancestor::OFFSET_TS_MAPPED_TYPE_NAME_TYPE)
        as *mut Option<TSType>)
    {
        traverser.ctx().retag_stack(AncestorType::TSMappedTypeNameType);
        walk_ts_type(traverser, field as *mut _);
    }
    if let Some(field) = &mut *((node as *mut u8)
        ...
        as *mut Option<TSType>)
    {
        traverser.ctx().retag_stack(AncestorType::TSMappedTypeTypeAnnotation);
        walk_ts_type(traverser, field as *mut _);
    }
    traverser.ctx().pop_stack();
    traverser.exit_ts_mapped_type(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_template_literal_type<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_template_literal_type(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSTemplateLiteralTypeQuasis(
    ...
    traverser.ctx().retag_stack(AncestorType::TSTemplateLiteralTypeTypes);
    ...
    traverser.ctx().pop_stack();
    traverser.exit_ts_template_literal_type(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_as_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_as_expression(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSAsExpressionExpression(
        ancestor::TSAsExpressionWithoutExpression(node),
    ));
    walk_expression(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_TS_AS_EXPRESSION_EXPRESSION) as *mut Expression,
    );
    traverser.ctx().retag_stack(AncestorType::TSAsExpressionTypeAnnotation);
    walk_ts_type(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_TS_AS_EXPRESSION_TYPE_ANNOTATION) as *mut TSType,
    );
    traverser.ctx().pop_stack();
    traverser.exit_ts_as_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_satisfies_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_satisfies_expression(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSSatisfiesExpressionExpression(
    ...
    traverser.ctx().retag_stack(AncestorType::TSSatisfiesExpressionTypeAnnotation);
    walk_ts_type(
        traverser,
    ...
    traverser.ctx().pop_stack();
    traverser.exit_ts_satisfies_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_type_assertion<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_type_assertion(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSTypeAssertionExpression(
    ...
    traverser.ctx().retag_stack(AncestorType::TSTypeAssertionTypeAnnotation);
    walk_ts_type(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_TS_TYPE_ASSERTION_TYPE_ANNOTATION) as *mut TSType,
    );
    traverser.ctx().pop_stack();
    traverser.exit_ts_type_assertion(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_import_equals_declaration<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_import_equals_declaration(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSImportEqualsDeclarationId(
    ...
    traverser.ctx().retag_stack(AncestorType::TSImportEqualsDeclarationModuleReference);
    ...
    traverser.ctx().pop_stack();
    traverser.exit_ts_import_equals_declaration(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_module_reference<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_module_reference(&mut *node);
    match &mut *node {
        TSModuleReference::ExternalModuleReference(node) => {
            walk_ts_external_module_reference(traverser, (&mut **node) as *mut _)
        }
        TSModuleReference::IdentifierReference(_) | TSModuleReference::QualifiedName(_) => {
            walk_ts_type_name(traverser, node as *mut _)
        }
    }
    traverser.exit_ts_module_reference(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_external_module_reference<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_external_module_reference(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSExternalModuleReferenceExpression(
    ...
    traverser.ctx().pop_stack();
    traverser.exit_ts_external_module_reference(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_non_null_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_non_null_expression(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSNonNullExpressionExpression(
    ...
    traverser.ctx().pop_stack();
    traverser.exit_ts_non_null_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_decorator<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_decorator(&mut *node);
    traverser
        .ctx()
        .push_stack(Ancestor::DecoratorExpression(ancestor::DecoratorWithoutExpression(node)));
    walk_expression(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_DECORATOR_EXPRESSION) as *mut Expression,
    );
    traverser.ctx().pop_stack();
    traverser.exit_decorator(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_export_assignment<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_export_assignment(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSExportAssignmentExpression(
    ...
    traverser.ctx().pop_stack();
    traverser.exit_ts_export_assignment(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_namespace_export_declaration<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_namespace_export_declaration(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSNamespaceExportDeclarationId(
    ...
    traverser.ctx().pop_stack();
    traverser.exit_ts_namespace_export_declaration(&mut *node);
}
...
pub(crate) unsafe fn walk_ts_instantiation_expression<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_ts_instantiation_expression(&mut *node);
    traverser.ctx().push_stack(Ancestor::TSInstantiationExpressionExpression(
    ...
    traverser.ctx().retag_stack(AncestorType::TSInstantiationExpressionTypeParameters);
    ...
    traverser.ctx().pop_stack();
    traverser.exit_ts_instantiation_expression(&mut *node);
}
...
pub(crate) unsafe fn walk_js_doc_nullable_type<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_js_doc_nullable_type(&mut *node);
    traverser.ctx().push_stack(Ancestor::JSDocNullableTypeTypeAnnotation(
    ...
    traverser.ctx().pop_stack();
    traverser.exit_js_doc_nullable_type(&mut *node);
}
...
pub(crate) unsafe fn walk_js_doc_unknown_type<'a, Tr: Traverse<'a>>(
    ...
    traverser.enter_js_doc_unknown_type(&mut *node);
    traverser.exit_js_doc_unknown_type(&mut *node);
}
