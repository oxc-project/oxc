...
impl<'t> Mangler<'t> {
    ...
    fn build_with_semantic_impl<const CAPACITY: usize, G: Fn(u32) -> InlineString<CAPACITY, u8>>(
        ...
        for (scope_id, bindings) in scoping.iter_bindings() {
            ...
            for (&symbol_id, &assigned_slot) in tmp_bindings.iter().zip(&reusable_slots) {
                ...
                // declared. `Scoping` records these during the build; for non-hoisted symbols the
                // list is empty and the registered `scope_id` (below) already covers it.
                let hoisted_declaration_scopes = scoping.hoisted_declaration_scopes(symbol_id);
            ...
            if scoping.scope_flags(scope_id).is_function()
                && let Some(fn_symbol_id) = scoping.fn_expr_name_symbol(scope_id)
    ...
    fn collect_keep_name_symbols<'a>(
        ...
        let symbol_ids = collect::collect_keep_name_symbols(keep_names, scoping, program);
        ...
        (symbol_ids.iter().map(|&id| scoping.symbol_name(id)).collect(), Some(ids))
    }
