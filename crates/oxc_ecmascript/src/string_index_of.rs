...
impl StringIndexOf for JSStr<'_> {
    ...
    fn index_of(&self, search_value: Option<JSStr<'_>>, from_index: Option<f64>) -> isize {
        ...
        // Step 10: first occurrence at or after `start`. A lone surrogate on
        // either side needs the code unit search: a surrogate half in the
        // search value can match the same half of a formed pair.
        match (self.as_str(), search.as_str()) {
            (Some(value), Some(search)) => index_of_bytes(value, search, start),
            _ => index_of_code_units(*self, search, start),
        }
    }
}

#[expect(clippy::cast_possible_wrap)]
fn index_of_bytes(value: &str, search: &str, start: usize) -> isize {
    // A UTF-8 match can only begin on a character boundary, so byte matches
    // are exactly the UTF-16 matches. The first boundary at or past `start`
    // also rounds a position inside an astral character up.
    let mut units = 0;
    let from = value
        .char_indices()
        .find_map(|(offset, c)| {
    ...
    value[from..]
        .find(search)
        .map_or(-1, |position| value[..from + position].encode_utf16().count() as isize)
}

#[expect(clippy::cast_possible_wrap)]
fn index_of_code_units(value: JSStr<'_>, search: JSStr<'_>, start: usize) -> isize {
