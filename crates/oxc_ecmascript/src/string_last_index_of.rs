...
impl StringLastIndexOf for JSStr<'_> {
    ...
    fn last_index_of(&self, search_value: Option<JSStr<'_>>, from_index: Option<f64>) -> isize {
        ...
        // Steps 9-10: last occurrence starting at or before the position. A
        // lone surrogate on either side needs the code unit search: a
        // surrogate half in the search value can match the same half of a
        // formed pair.
...
fn last_index_of_bytes(value: &str, search: &str, position: f64) -> isize {
    // A UTF-8 match can only begin on a character boundary, so byte matches
    // are exactly the UTF-16 matches. The last boundary at or before the
    // position also rounds a position inside an astral character down.
    let mut units = 0.0;
    ...
    // The byte window may end inside a character; no match can end there,
    // because match ends also fall on boundaries.
...
fn last_index_of_code_units(value: JSStr<'_>, search: JSStr<'_>, position: f64) -> isize {
    // Compare UTF-16 code units, the domain the specification defines. A lone
    // surrogate is a single unit, so a surrogate half in the search value
    // matches both a lone half and the same half of a formed pair. The
    // saturating float cast turns an infinite position into a bound past any
    // string length.
