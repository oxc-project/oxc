...
fn try_fold_string_substring_or_slice<'a>(
    ...
    // and a NaN start converts to zero, so the result is a plain slice of the
    // value. A boundary inside a surrogate pair has no `str` result and
    // declines; otherwise the slice borrows the arena without allocating.
