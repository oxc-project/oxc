...
/// [`ESTree`] implementation for strings.
impl ESTree for &str {
    fn serialize<S: Serializer>(&self, serializer: &mut S) {
        ...
        for (index, &byte) in bytes.iter().enumerate() {
            ...
            if start < index {
                ...
                unsafe { serializer.buffer_mut().push_bytes(&bytes[start..index]) };
            }

            write_char_escape(escape, byte, serializer.buffer_mut());
        ...
        if start < bytes.len() {
            ...
            // `start` is either the start of string, or after an ASCII character,
            // therefore always the start of a valid UTF8 character sequence.
            unsafe { serializer.buffer_mut().push_bytes(&bytes[start..]) };
        }
