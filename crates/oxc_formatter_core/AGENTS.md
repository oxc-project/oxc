...
- Accumulating across interleaved `write()` calls (multiple builders open at once), or staging that must release the state between writes and consumption: bring a `ScratchBuffer` per accumulator and write through `ScratchBuffer::writer`, finish via `Formatter::intern_elements` (or re-emit into the destination buffer)
  ...
  - source the buffer from `ScratchBuffer::checkout` (thread-local cache), or on hot paths from the state's spare slot (`FormatState::take_spare_scratch` / `return_spare_scratch`) — the per-node thread-local round-trip is measurable there (see `AssignmentLike`)
