...
mod tests {
    ...
    /// Test-only equivalent of `format_args!`, bound to [`SimpleFormatContext`].
    ...
    /// Test-only equivalent of `write!`, bound to [`SimpleFormatContext`].
    macro_rules! test_write {
        ($dst:expr, [$($arg:expr),+ $(,)?]) => {{
            $crate::write($dst, test_args!($($arg),+))
        }};
    }
    ...
    fn it_tracks_the_indent_for_each_token() {
        ...
        let formatted = format_simple(
            ...
            &test_args!(
                token("a"),
                soft_block_indent(&test_args!(
                    token("b"),
                    soft_block_indent(&test_args!(
                        token("c"),
                        soft_block_indent(&test_args!(token("d"), soft_line_break(), token("d"),)),
                        token("c"),
                    )),
                    token("b"),
                )),
...
two lines`,
  "b",
    ...
    fn it_breaks_a_group_if_it_contains_a_hard_line_break() {
        let allocator = Allocator::default();
        let result =
            format_simple(&allocator, &group(&test_args!(token("a"), block_indent(&token("b")))));
  ...
  ["0123456789", "0123456789", "0123456789", "0123456789", "0123456789", "0123456789"],
    ...
    fn it_prints_consecutive_empty_lines_as_one() {
        ...
        let result = format_simple(
            &allocator,
            &test_args!(token("a"), empty_line(), empty_line(), empty_line(), token("b"),),
        );
    ...
    fn test_fill_breaks() {
        ...
        formatter
            ...
            .entry(&soft_line_break_or_space(), &test_args!(token("1"), token(",")))
            .entry(&soft_line_break_or_space(), &test_args!(token("2"), token(",")))
            .entry(&soft_line_break_or_space(), &test_args!(token("3"), token(",")))
            // This one fits on a line by itself,
            .entry(&soft_line_break_or_space(), &test_args!(token("723493294"), token(",")))
            // fits without breaking
            .entry(
                &soft_line_break_or_space(),
                &group(&test_args!(token("["), soft_block_indent(&token("5")), token("],"))),
            )
            // this one must be printed in expanded mode to fit
            .entry(
                &soft_line_break_or_space(),
                &group(
                    &test_args!(token("["), soft_block_indent(&token("123456789")), token("]"),),
                ),
            )
            .finish();
    ...
    fn line_suffix_printed_at_end() {
        ...
        let printed = format_simple(
            ...
            &test_args!(
                group(&test_args!(
                    ...
                    soft_block_indent(&test_format_with(|f| {
                        f.fill()
                            ...
                            .entry(
                                &soft_line_break_or_space(),
                                &test_args!(token("3"), if_group_breaks(&token(","))),
                            )
                            .finish();
                    })),
                    token("]")
                ...
                token(";"),
                &line_suffix(&test_args!(space(), token("// trailing"), space()))
            ),
        );
        ...
        fn fmt(&self, f: &mut crate::Formatter<'_, 'a, TestContext<'a>>) {
            test_write!(
                ...
                [group(&test_args!(
                    ...
                    soft_block_indent(&test_args!(
                        test_format_with(|f| {
                            f.join_with(test_args!(token(","), soft_line_break_or_space()))
                                .entries(&self.items);
                        }),
