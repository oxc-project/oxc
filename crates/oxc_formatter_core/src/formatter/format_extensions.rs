...
use super::{Buffer, FormatContext, prelude::*};
...
pub trait MemoizeFormat<'a> {
    ...
    /// ```
    ///
    fn memoized<Ctx>(self) -> Memoized<'a, Ctx, Self>
    where
        Ctx: FormatContext<'a>,
        Self: Sized + Format<'a, Ctx>,
    {
        Memoized::new(self)
    }
}
...
#[derive(Debug)]
pub struct Memoized<'ast, Ctx, F> {
    inner: F,
    memory: OnceCell<Option<FormatElement<'ast>>>,
    _context: PhantomData<fn() -> Ctx>,
}

impl<'ast, Ctx, F> Memoized<'ast, Ctx, F>
where
    Ctx: FormatContext<'ast>,
    F: Format<'ast, Ctx>,
{
    fn new(inner: F) -> Self {
        Self { inner, memory: OnceCell::new(), _context: PhantomData }
    }
    ...
    ///
    /// ```
    pub fn inspect(&self, f: &mut Formatter<'_, 'ast, Ctx>) -> &[FormatElement<'ast>] {
...
impl<'ast, Ctx, F> Format<'ast, Ctx> for Memoized<'ast, Ctx, F>
where
    Ctx: FormatContext<'ast>,
    F: Format<'ast, Ctx>,
{
