...
pub struct ReactCompilerOptions {
    ...
    /// Use explicit `: T` declarations and `as`/`satisfies` casts to refine
    /// type inference during compilation. Mirrors `enableUseTypeAnnotations` in
...
pub struct ReactCompiler {
    ...
    /// The program source text, captured in `enter_program` and forwarded to
    /// each `Environment` via `set_source_code`. Codegen passes that recover
    /// raw spans (currently `enableChangeDetectionForDebugging` labels and
    /// `enableUseTypeAnnotations` `as`/`satisfies` re-emission) read this from
    /// the environment. Without it, those passes silently drop the source-
...
impl ReactCompiler {
    ...
    pub fn enter_program<'a>(&mut self, program: &mut Program<'a>, ctx: &mut TraverseCtx<'a>) {
        ...
        // Capture the program source text so that codegen passes which recover
        // raw spans (currently change-detection labels and the
        // `enableUseTypeAnnotations` `as`/`satisfies` re-emission) can read it
        // from the environment. Mirrors the fixture harness which sets the
        // source on every `Environment` before lowering — without this the
        // production path would silently drop those source-dependent outputs.
        self.source_text = Some(std::sync::Arc::from(program.source_text));
    ...
    fn compile_function<'a>(
        ...
        // Forward the program source text so codegen passes that recover raw
        // spans (`enableChangeDetectionForDebugging` labels and the
        // `enableUseTypeAnnotations` `as`/`satisfies` re-emission) can read it
        // from `Environment::ctx.code`. The fixture harness mirrors this call.
        if let Some(source) = self.source_text.as_ref() {
            environment.set_source_code(std::sync::Arc::clone(source));
        }
        // Seed the environment with names already generated by previous function
        // compilations so that `generate_globally_unique_identifier_name` does not
        // produce duplicates (e.g. multiple functions all generating `_temp`).
        environment.seed_known_referenced_names(&self.program_context.known_referenced_names);
        ...
        // Propagate newly generated names back to ProgramContext so that
        // subsequent function compilations avoid duplicates.
        // Use hir_function.env (not the original `environment`) because the pipeline
        // (outline_functions, outline_jsx) generates names in hir_function.env.
        for name in hir_function.env.known_referenced_names() {
            self.program_context.add_reference(name);
        }
    ...
    fn compile_outlined_function<'a>(
        ...
        // Propagate newly generated names back to ProgramContext.
        // Use hir_function.env (not the original `environment`) because the pipeline
        // generates names in hir_function.env.
        for name in hir_function.env.known_referenced_names() {
            self.program_context.add_reference(name);
        }
...
fn assign_scope_ids_to_expression(
    ...
    match expr {
        ...
        // Chain expressions (optional chaining like `a?.b()`, `a?.b.c()`)
        Expression::ChainExpression(chain) => {
            assign_scope_ids_to_chain_element(&mut chain.expression, parent_scope_id, ctx);
        }
        // JSX expressions
...
/// Assign scope IDs within a chain element (optional chaining expression).
fn assign_scope_ids_to_chain_element(
    element: &mut ChainElement<'_>,
    parent_scope_id: ScopeId,
    ctx: &mut TraverseCtx<'_>,
) {
    match element {
        ...
        ChainElement::TSNonNullExpression(ts_non_null) => {
            assign_scope_ids_to_expression(&mut ts_non_null.expression, parent_scope_id, ctx);
        }
        _ => {
            // MemberExpression variants — walk the object (and computed expression if any).
            if let Some(member) = element.as_member_expression_mut() {
                assign_scope_ids_to_expression(member.object_mut(), parent_scope_id, ctx);
            }
        }
    }
}
