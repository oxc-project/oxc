...
impl<'a> StyledComponents<'a> {
    ...
    fn handle_pure_annotation(
        ...
        // <https://github.com/rollup/rollup/issues/4035>
        if let Some(call) = declarator.init.as_mut().and_then(|e| e.as_call_expression_mut())
            && (call.callee.as_identifier().is_some_and(|ident| self.is_pure_helper(ident, ctx))
    ...
    fn collect_styled_bindings(&mut self, program: &Program<'a>, _ctx: &mut TraverseCtx<'a>) {
        for statement in &program.body {
            let Some(import) = statement.as_import_declaration() else { continue };
