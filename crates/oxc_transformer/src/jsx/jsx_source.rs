...
impl<'a> JsxSource<'a> {
    ...
    pub fn get_line_column(&mut self, offset: u32, source_text: &str) -> (u32, u32) {
        let source_rope =
            self.source_rope.get_or_insert_with(|| Rope::from_str(source_text));
        let (line, column) = get_line_column(source_rope, offset, source_text);
    ...
    fn add_source_attribute(
        ...
        let (line, column) = self.get_line_column(elem.span.start, ctx.state.source_text);
