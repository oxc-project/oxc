...
impl<'a> ClassProperties<'a> {
    ...
    fn transform_instance_assignment_expression(
        ...
        let assign_expr = expr.take_in(ctx.ast).into_assignment_expression().unbox();
    ...
    fn transform_chain_element_recursively(
        ...
        if expr.is_private_field_expression() {
            self.transform_private_field_expression_of_chain_expression(expr, ctx)
        } else if expr.is_member_expression() {
            self.transform_member_expression_of_chain_expression_expr(expr, ctx)
    ...
    fn transform_first_optional_expression(
        ...
        // Handle CallExpression with optional + member callee separately
        // because we need access to both the matched binding and `expr` itself
        if let Some(call) = expr.as_call_expression() {
            if call.optional && call.callee.is_member_expression() {
                return Some(self.transform_call_expression_to_bind_proper_context(expr, ctx));
            }
        }
        let object = match expr.kind_mut() {
    ...
    fn transform_member_expression_of_chain_expression_expr(
        &mut self,
        expr: &mut Expression<'a>,
        ctx: &mut TraverseCtx<'a>,
    ) -> Option<Expression<'a>> {
        let (is_optional, object) = match expr.kind_mut() {
            ExpressionKindMut::StaticMemberExpression(m) => (m.optional, &mut m.object),
            ExpressionKindMut::ComputedMemberExpression(m) => (m.optional, &mut m.object),
            ExpressionKindMut::PrivateFieldExpression(m) => (m.optional, &mut m.object),
            _ => unreachable!(),
        };
        let object = object.get_inner_expression_mut();
    ...
    fn transform_call_expression_of_chain_expression(
        ...
        let object = match callee.kind_mut() {
            ExpressionKindMut::StaticMemberExpression(m) => &mut m.object,
            ExpressionKindMut::ComputedMemberExpression(m) => &mut m.object,
            ExpressionKindMut::PrivateFieldExpression(m) => &mut m.object,
            _ => unreachable!(),
        };
    ...
    fn convert_chain_expression_to_expression(
        ...
        let chain_expr = expr.take_in(ctx.ast).into_chain_expression();
        match chain_expr.unbox().expression {
            ...
            ChainElement::PrivateFieldExpression(f) => Expression::private_field_expression(f),
        }
    }
    ...
    fn ensure_optional_expression_wrapped_by_chain_expression(
        ...
        if Self::has_optional_expression(&expr) {
            let chain_element = if expr.is_call_expression() {
                ChainElement::CallExpression(expr.into_call_expression())
            } else if expr.is_member_expression() {
    ...
    fn transform_call_expression_to_bind_proper_context(
        ...
        let (is_optional_callee, object) = match callee.kind_mut() {
            ExpressionKindMut::StaticMemberExpression(m) => (m.optional, &mut m.object),
            ExpressionKindMut::ComputedMemberExpression(m) => (m.optional, &mut m.object),
            ExpressionKindMut::PrivateFieldExpression(m) => (m.optional, &mut m.object),
            _ => unreachable!(),
    ...
    pub(super) fn transform_private_in_expression(
        ...
        let private_in = expr.take_in(ctx.ast).into_private_in_expression();
