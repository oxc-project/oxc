...
impl<'a> HelperLoaderStore<'a> {
    ...
    fn transform_for_external_helper(helper: Helper, ctx: &mut TraverseCtx<'a>) -> Expression<'a> {
        ...
        let symbol_id =
            ctx.scoping().find_binding(ctx.current_scope_id(), &Ident::from(HELPER_VAR));
        let object =
            ctx.create_ident_expr(SPAN, Ident::new(HELPER_VAR), symbol_id, ReferenceFlags::Read);
        let property = ctx.ast.identifier_name(SPAN, Ident::new(helper.name()));
