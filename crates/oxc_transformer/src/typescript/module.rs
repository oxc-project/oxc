...
impl<'a> TypeScriptModule<'a, '_> {
    ...
    fn transform_ts_export_assignment(
        ...
        let module_exports = {
            let reference_id =
                ctx.create_reference_in_current_scope(&Ident::from("module"), ReferenceFlags::Read);
            ...
            let property = ctx.ast.identifier_name(SPAN, Ident::new("exports"));
        ...
        let (kind, init) = match &mut decl.module_reference {
            ...
            TSModuleReference::ExternalModuleReference(reference) => {
                ...
                let require_symbol_id =
                    ctx.scoping().find_binding(ctx.current_scope_id(), &Ident::from("require"));
