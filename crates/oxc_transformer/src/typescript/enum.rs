...
use oxc_ecmascript::enum_evaluation::{ConstantEnumValue, EnumConstantEvaluator, EnumMembers};
...
pub struct TypeScriptEnum<'a> {
    evaluator: EnumConstantEvaluator<'a>,
}

impl TypeScriptEnum<'_> {
    pub fn new() -> Self {
        Self { evaluator: EnumConstantEvaluator::new() }
    }
}
...
impl<'a> TypeScriptEnum<'a> {
    ...
    fn transform_ts_enum_members(
        ...
        let mut previous_enum_members =
            self.evaluator.enums_mut().entry(param_binding.name).or_default().clone();
        ...
        for member in members.take_in(ctx.ast) {
            ...
            let init = if let Some(mut initializer) = member.initializer {
                let constant_value = self.evaluator.computed_constant_value(
                    &initializer,
                ...
                previous_enum_members.insert(member_name, Some(ConstantEnumValue::Number(value)));
                ...
                previous_enum_members.insert(member_name, Some(ConstantEnumValue::Number(0.0)));
        ...
        self.evaluator.enums_mut().insert(param_binding.name, previous_enum_members.clone());
...
struct IdentifierReferenceRename<'a, 'ctx> {
    enum_name: Atom<'a>,
    previous_enum_members: EnumMembers<'a>,
