...
impl<'a> TypeScript<'a> {
    ...
    pub fn transform_ts_enum(
        ...
        let kind = self.ctx.borrow().ast.binding_pattern_identifier(ident);
        let id = self.ctx.borrow().ast.binding_pattern(kind, None, false);

        let mut params = self.ctx.borrow().ast.new_vec();

        // ((Foo) => {
        params.push(self.ctx.borrow().ast.formal_parameter(
            SPAN,
            id,
            None,
            false,
        ...
        let params = self.ctx.borrow().ast.formal_parameters(
            SPAN,
        ...
        // })(Foo || {});
        let mut arguments = self.ctx.borrow().ast.new_vec();
        ...
        let right =
            self.ctx.borrow().ast.object_expression(SPAN, self.ctx.borrow().ast.new_vec(), None);
        let expression = self.ctx.borrow().ast.logical_expression(SPAN, left, op, right);
        ...
        let decls = {
            let mut decls = self.ctx.borrow().ast.new_vec();
            ...
            let decl = self.ctx.borrow().ast.variable_declarator(
                SPAN,
                kind,
                binding,
    ...
    pub fn transform_ts_enum_members(
        ...
        let mut default_init = self.ctx.borrow().ast.literal_number_expression(NumericLiteral {
            span: SPAN,
            value: 0.0,
            raw: "0",
            base: NumberBase::Decimal,
        });
        let mut statements = self.ctx.borrow().ast.new_vec();
        ...
        for member in members.iter_mut() {
            ...
            let mut self_ref = {
                let obj = self.ctx.borrow().ast.identifier_reference_expression(
                ...
                self.ctx.borrow().ast.computed_member_expression(SPAN, obj, expr, false)
            };

            if is_valid_identifier(member_name, true) {
                let ident = IdentifierReference::new(member_span, member_name.clone());

                self_ref = self.ctx.borrow().ast.identifier_reference_expression(ident.clone());
                let init = mem::replace(
                    &mut init,
                ...
                let decls = {
                    ...
                    let binding_pattern_kind =
                        self.ctx.borrow().ast.binding_pattern_identifier(binding_identifier);
                    let binding =
                        self.ctx.borrow().ast.binding_pattern(binding_pattern_kind, None, false);
                    let decl = self.ctx.borrow().ast.variable_declarator(
                        SPAN,
                        kind,
                        binding,
                ...
                let decl = self.ctx.borrow().ast.variable_declaration(
                    SPAN,
                    kind,
                    decls,
                    Modifiers::empty(),
            ...
            let member_expr = {
                ...
                self.ctx.borrow().ast.computed_member(SPAN, obj, expr, false)
            };
            let left =
                self.ctx.borrow().ast.simple_assignment_target_member_expression(member_expr);
            let mut expr = self.ctx.borrow().ast.assignment_expression(
                SPAN,
            ...
            if !is_str {
                ...
                let right = self
                    ...
                    .ast
                    .literal_string_expression(StringLiteral::new(SPAN, member_name.clone()));
                expr = self.ctx.borrow().ast.assignment_expression(
                    SPAN,
                    AssignmentOperator::Assign,
                    left,
                    right,
            ...
            statements.push(self.ctx.borrow().ast.expression_statement(member.span, expr));

            // 1 + Foo["x"]
            default_init = {
                let one = self.ctx.borrow().ast.literal_number_expression(NumericLiteral {
                    span: SPAN,
                    value: 1.0,
                    raw: "1",
                    base: NumberBase::Decimal,
                ...
                self.ctx.borrow().ast.binary_expression(
                    SPAN,
                    one,
        ...
        let enum_ref = self
            ...
            .identifier_reference_expression(IdentifierReference::new(SPAN, enum_name.clone()));
        // return Foo;
        let return_stmt = self.ctx.borrow().ast.return_statement(SPAN, Some(enum_ref));
