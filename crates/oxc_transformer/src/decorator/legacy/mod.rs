...
/// Decoration state for a single class level.
/// This struct groups together all the state needed while processing decorators for one class,
/// allowing clean management of nested class scenarios through a stack of these structs.
#[derive(Default)]
...
pub struct LegacyDecorator<'a, 'ctx> {
    ...
    class_decoration_stack: NonEmptyStack<ClassDecorations<'a>>,
...
impl<'a, 'ctx> LegacyDecorator<'a, 'ctx> {
    pub fn new(emit_decorator_metadata: bool, ctx: &'ctx TransformCtx<'a>) -> Self {
        Self {
            ...
            class_decoration_stack: NonEmptyStack::new(ClassDecorations::default()),
            ctx,
        }
    }
}
...
impl<'a> Traverse<'a, TransformState<'a>> for LegacyDecorator<'a, '_> {
    ...
    fn enter_class(&mut self, class: &mut Class<'a>, ctx: &mut TraverseCtx<'a>) {
        self.class_decoration_stack.push(ClassDecorations::default());
    ...
    fn exit_method_definition(
        ...
        if method.value.is_typescript_syntax() || method.kind.is_constructor() {
            return;
        }
    ...
    fn enter_decorator(&mut self, node: &mut Decorator<'a>, _ctx: &mut TraverseCtx<'a>) {
        let current_class = self.class_decoration_stack.last_mut();
        if !current_class.class_has_private_in_expression_in_decorator {
...
impl<'a> LegacyDecorator<'a, '_> {
    ...
    fn handle_decorated_class_element(
        ...
        self.class_decoration_stack.last_mut().decoration_stmts.push(decorator_stmt);
    }
    ...
    fn transform_class(&mut self, class: &mut Class<'a>, ctx: &mut TraverseCtx<'a>) {
        ...
        let current_class_decorations = self.class_decoration_stack.pop();

        let class_or_constructor_parameter_is_decorated = Self::check_class_has_decorated(class);
