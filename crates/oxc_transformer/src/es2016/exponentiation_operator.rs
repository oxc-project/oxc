...
impl<'a> ExponentiationOperator<'a, '_> {
    ...
    fn math_pow(
        ...
        let math_symbol_id =
            ctx.scoping().find_binding(ctx.current_scope_id(), &Ident::from("Math"));
        let object =
            ctx.create_ident_expr(SPAN, Ident::new("Math"), math_symbol_id, ReferenceFlags::Read);
        let property = ctx.ast.identifier_name(SPAN, Ident::new("pow"));
        let callee =
