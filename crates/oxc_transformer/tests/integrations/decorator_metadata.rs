...
fn transform_ts_with(source_text: &str, decorator: DecoratorOptions) -> String {
    ...
    let options = TransformOptions { decorator, ..TransformOptions::default() };
...
fn transform_nonstrict(source_text: &str) -> String {
    transform_ts_with(
        source_text,
        DecoratorOptions { legacy: true, emit_decorator_metadata: true, strict_null_checks: false },
    )
}

#[test]
fn nullable_string_with_strict_null_checks_emits_object() {
...
#[test]
fn nullable_string_without_strict_null_checks_emits_string() {
    let src = format!("{PREFIX} class Source {{ @D() value!: string | null; }}");
    let output = transform_nonstrict(&src);
    assert!(output.contains("decorateMetadata('design:type', String)"), "got:\n{output}");
}

#[test]
fn undefined_union_without_strict_null_checks_emits_underlying_primitive() {
...
#[test]
fn nullable_and_undefined_without_strict_null_checks_emits_underlying_primitive() {
