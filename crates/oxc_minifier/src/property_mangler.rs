...
pub type PropertyMapping<'a> = ArenaIdentHashMap<'a, Ident<'a>>;
...
struct PropertyCollectState<'a> {
    ...
    frequencies: ArenaIdentHashMap<'a, u32>,
    /// Spellings that remain in the output and therefore cannot be automatic output names.
    occupied: ArenaIdentHashSet<'a>,
}

impl<'a> PropertyCollectState<'a> {
    fn new_in(allocator: &'a Allocator) -> Self {
        Self {
            frequencies: ArenaIdentHashMap::new_in(allocator),
            occupied: ArenaIdentHashSet::new_in(allocator),
        }
    }
}
...
/// program.
#[derive(Debug)]
pub struct PropertyMangleCollection<'a>(&'a PropertyCollectState<'a>);
...
impl<'a> PropertyMangleCollection<'a> {
    ...
    pub fn from_program(
        ...
        Self(allocator.alloc(collect(options, program, &key_annotated, allocator)))
    }
}
...
impl<'o, 'a> PropertyCollector<'o, 'a> {
    fn new(
        ...
        Self { options, key_annotated, allocator, state: PropertyCollectState::new_in(allocator) }
    }
...
fn assign<'a>(
    ...
    let mut occupied = ArenaIdentHashSet::from_iter_in(state.occupied.iter().copied(), allocator);
    ...
    let mut mapping = PropertyMapping::new_in(allocator);
...
impl<'a> PropertyMangler<'a> {
    pub fn new_in(options: ManglePropertiesOptions, allocator: &'a Allocator) -> Self {
        Self {
            allocator,
            cache: ManglePropertyCache::default(),
            options,
            state: PropertyCollectState::new_in(allocator),
            mapping: PropertyMapping::new_in(allocator),
        }
    }
    ...
    pub fn collect(&mut self, program: &Program<'a>) {
        let collection =
            PropertyMangleCollection::from_program(&self.options, program, self.allocator);
    ...
    pub fn merge_collected(&mut self, collection: &PropertyMangleCollection<'a>) {
        ...
        self.state.occupied.extend(collection.0.occupied.iter().copied());
    }

    /// Assign deterministic names by descending occurrence frequency and lexical tie-break.
    pub fn assign(&mut self) -> &PropertyMapping<'a> {
        let (mapping, cache) = assign(&self.options, &self.state, self.allocator);
    ...
    pub fn rewrite(&self, program: &mut Program<'a>) {
...
mod tests {
    ...
    #[test]
    fn collection_is_send() {
        fn assert_send<T: Send>() {}
        assert_send::<PropertyMangleCollection<'static>>();
    }
    ...
    fn assignment_prefers_frequency_then_name() {
        ...
        let mut state = PropertyCollectState::new_in(&allocator);
        state.frequencies.extend([
    ...
    fn assignment_honors_cache_and_allows_duplicate_targets() {
        ...
        state.occupied.insert(static_ident!("_keep"));
