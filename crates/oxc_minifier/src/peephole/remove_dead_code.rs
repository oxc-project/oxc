...
impl<'a> PeepholeOptimizations {
    ...
    pub fn try_fold_labeled(stmt: &mut Statement<'a>, ctx: &mut TraverseCtx<'a>) {
        ...
        match &mut s.body {
            ...
            Statement::BlockStatement(block) if block.body.is_empty() || block.body.first().is_some_and(|first| matches!(first, Statement::BreakStatement(break_stmt) if break_stmt.label.as_ref().is_some_and(|l| l.name == id))) => {}
