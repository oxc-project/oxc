...
impl<'a> PeepholeOptimizations {
    ...
    pub fn substitute_arrow_expression(
        ...
        if !arrow_expr.expression
            ...
            && body.is_return_statement()
        {
            let ret_stmt = body.as_return_statement_mut().unwrap();
    ...
    pub fn substitute_typeof_undefined(expr: &mut Expression<'a>, ctx: &mut TraverseCtx<'a>) {
        ...
        *expr = if let Some(ident) = unary_expr.argument.as_identifier()
    ...
    pub fn substitute_rotate_logical_expression(
        ...
        let right_taken = e.right.take_in(ctx.ast);
        let mut right = right_taken.into_logical_expression().unbox();
    ...
    pub fn substitute_rotate_binary_expression(
        ...
        {
            let right_taken = e.right.take_in(ctx.ast);
            let mut right = right_taken.into_binary_expression().unbox();
        ...
        let pair = Self::commutative_pair(
            ...
            |b| {
                ...
                let (id, ()) = Self::commutative_pair(
                    (&b.left, &b.right),
                    |a_a| {
                        let Some(id) = a_a.as_identifier() else { return None };
                        Some(id)
                    },
    ...
    fn try_rewrite_arguments_copy_loop(for_stmt: &mut ForStatement<'a>, ctx: &mut TraverseCtx<'a>) {
        ...
        // Parse update: `a++`
        {
            let Some(u) = for_stmt.update.as_ref().and_then(|e| e.as_update_expression()) else {
                return;
            };
        ...
        let e_id_info = {
            let Some(b) = for_stmt.test.as_ref().and_then(|e| e.as_binary_expression()) else {
                return;
            };
    ...
    pub fn substitute_return_statement(stmt: &mut ReturnStatement<'a>, ctx: &mut TraverseCtx<'a>) {
        ...
        if !(if let Some(ident) = argument.as_identifier() {
            ctx.is_identifier_undefined(ident)
        } else if let Some(e) = argument.as_unary_expression() {
    ...
    pub fn substitute_object_or_array_constructor(
        ...
        let callee = if let Some(e) = expr.as_new_expression() {
            &e.callee
        } else if let Some(e) = expr.as_call_expression() {
            &e.callee
        } else {
            return;
        };
    ...
    pub fn substitute_array_expression(expr: &mut Expression<'a>, ctx: &mut TraverseCtx<'a>) {
        ...
        let strings = array.elements.iter().map(|element| {
            let str = element.to_expression().as_string_literal().unwrap();
            str.value.as_str()
        });
