...
impl<'a> PeepholeOptimizations {
        ...
        if let Some(lval) = left_val {
            ...
            if if lval { op.is_or() } else { op.is_and() } {
                ...
                if should_keep_indirect_access {
                    ...
                    let vec = ctx.ast.vec_from_array([num, right]);
                    return Some(ctx.ast.expression_sequence(logical_expr.span, vec));
                }
                // (FALSE || x) => x
                // (TRUE && x) => x
        ...
        match left_val {
            ...
            | ValueType::Object => {
                ...
                if should_keep_indirect_access {
                    ...
                    let vec = ctx.ast.vec_from_array([num, left]);
                    return Some(ctx.ast.expression_sequence(logical_expr.span, vec));
                }
    ...
    fn try_fold_left_child_op(
        ...
        let expr = expr_to_move.take_in(&ctx.ast);
        let value_expr = ctx.value_to_expr(
            left.right.span().merge_within(e.right.span(), e.span).unwrap_or(SPAN),
            v,
        );
        Some(ctx.ast.expression_binary(e.span, expr, op, value_expr))
    }
        ...
        let value = ConstantValue::Number(match arg {
            ...
            Expression::StringLiteral(n) => {
                if let Some(n) = arg.evaluate_value_to_number(ctx) {
                    ...
                    let string_lit = ctx.ast.expression_string_literal(n.span, n.value, n.raw);
                    *expr = ctx.ast.expression_unary(e.span, UnaryOperator::UnaryPlus, string_lit);
                    ctx.state.changed = true;
                    return;
                }
            }
