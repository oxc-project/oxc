...
impl<'a> JSStr<'a> {
    ...
    pub fn utf16_len(self) -> usize {
        self.code_points().map(|value| if value > 0xFFFF { 2 } else { 1 }).sum()
    }
    ...
    pub fn encode_utf16(self) -> impl Iterator<Item = u16> + 'a {
        self.code_points().flat_map(|code_point| {
    ...
    ///
    /// Each item is either a Unicode scalar value or a lone surrogate encoded
    /// as `u32`. This lets output consumers preserve every JavaScript string
    /// without exposing the underlying WTF-8 bytes.
    #[inline]
    pub fn code_points(self) -> impl Iterator<Item = u32> + 'a {
    ...
    fn from_valid_bytes(bytes: &'a [u8], has_lone_surrogate: bool) -> Self {
        ...
        // and valid for `'a`. The flag was provided by a validating constructor.
...
impl ESTree for JSStr<'_> {
    fn serialize<S: ESTreeSerializer>(&self, serializer: S) {
        ...
        // ESTree's existing lone-surrogate writer accepts Oxc's old marker
        // representation. Build that representation only on this rare path.
        ...
        LoneSurrogatesString(&escaped).serialize(serializer);
    }
}
...
mod tests {
    ...
    fn utf8_fast_path() {
        ...
        assert_eq!(
            value.code_points().collect::<Vec<_>>(),
    ...
    fn ill_formed_utf16() {
        ...
        assert_eq!(value.code_points().collect::<Vec<_>>(), [0x61, 0xD800, 0x62, 0xDC00]);
    ...
    fn utf16_pair_becomes_scalar() {
        ...
        assert!(!value.has_lone_surrogate());
        assert_eq!(value.code_points().collect::<Vec<_>>(), [0x1F525]);
    }
    ...
    fn builder_preserves_other_lone_surrogates() {
        ...
        assert!(value.has_lone_surrogate());
        assert_eq!(value.code_points().collect::<Vec<_>>(), [0xD800, 0x1F525, 0xDC00]);
    }
    ...
    fn builder_flushes_lead_before_utf8() {
        ...
        assert!(value.has_lone_surrogate());
        assert_eq!(value.code_points().collect::<Vec<_>>(), [0xD800, 0x61, 0x1F525]);
    }
