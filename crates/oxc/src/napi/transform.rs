...
use crate::CompilerInterface;

use super::{
    errors::wrap_diagnostics, isolated_declarations::IsolatedDeclarationsOptions,
...
#[derive(Default)]
struct Compiler {
    transform_options: oxc_transformer::TransformOptions,
    sourcemap: bool,
...
impl Compiler {
    fn new(options: Option<TransformOptions>) -> Result<Self, Vec<OxcDiagnostic>> {
        ...
        let transform_options =
            options.map(oxc_transformer::TransformOptions::from).unwrap_or_default();
        Ok(Self {
            transform_options,
            sourcemap,
...
impl CompilerInterface for Compiler {
    ...
    fn transform_options(&self) -> Option<oxc_transformer::TransformOptions> {
