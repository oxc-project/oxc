...
use crate::{
    ...
    rule::BuiltinRule, suppression::DiffManager,
};

/// State required to initialize the `tsgolint` linter.
#[derive(Debug, Clone)]
pub struct TsGoLintState<R: BuiltinRule> {
...
impl<R: BuiltinRule> TsGoLintState<R> {
    pub fn new(cwd: &Path, config_store: ConfigStore<R>, fix_kind: FixKind) -> Self {
        let executable_path =
    ...
    pub fn lint(
        ...
        let mut resolved_configs: FxHashMap<PathBuf, ResolvedLinterState<R>> = FxHashMap::default();
    ...
    fn json_input(
        ...
        resolved_configs: &mut FxHashMap<PathBuf, ResolvedLinterState<R>>,
    ) -> Payload {
