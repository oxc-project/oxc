...
pub mod external_linter;
pub mod external_plugin_store;
pub mod fixer;
pub mod frameworks;
pub mod globals;
...
pub use crate::{
    config::{
        Config, ConfigBuilderError, ConfigStore, ConfigStoreBuilder, ESLintRule, LintConfig,
        LintIgnoreMatcher, LintPlugins, OxlintRules, Oxlintrc, ResolvedLinterState,
    },
    ...
    rule::{
        BuiltinRule, RuleCategory, RuleFixMeta, RuleMeta, RuleRunFunctionsImplemented, RuleRunner,
    },
...
fn execute_rules<'a, R: BuiltinRule, const TIMINGS: bool>(
    rules: &[(&R, LintContext<'a>)],
    semantic: &Semantic<'a>,
...
pub struct Linter<R: BuiltinRule> {
    options: LintOptions,
    pub(crate) config: ConfigStore<R>,
    external_linter: Option<ExternalLinter>,
    workspace_uri: Option<Box<str>>,
}

impl<R: BuiltinRule> Linter<R> {
    pub fn new(
        options: LintOptions,
        config: ConfigStore<R>,
        external_linter: Option<ExternalLinter>,
    ) -> Self {
    ...
    pub fn run_with_disable_directives<'a, const TIMINGS: bool>(
        ...
        loop {
            ...
            #[cfg(debug_assertions)]
            {
                ...
                execute_rules::<R, false>(&rules, semantic, should_run_on_jest_node, false, None);
