...
pub struct ConfigStoreBuilder<R: BuiltinRule> {
    pub(super) rules: FxHashMap<R, AllowWarnDeny>,
...
impl<R: BuiltinRule> Default for ConfigStoreBuilder<R> {
    fn default() -> Self {
        Self { rules: Self::warn_correctness(LintPlugins::default()), ..Self::empty() }
    }
}

impl<R: BuiltinRule> ConfigStoreBuilder<R> {
    ...
    pub fn all() -> Self {
        ...
        let overrides = OxlintOverrides::default();
        let categories: OxlintCategories = OxlintCategories::default();
        let rules = R::all_rules().iter().map(|rule| (rule.clone(), AllowWarnDeny::Warn)).collect();
        let external_rules = FxHashMap::default();
        let extended_paths = Vec::new();
        Self { rules, external_rules, config, categories, overrides, extended_paths }
    }
    ...
    pub fn with_rule(mut self, rule: R, severity: AllowWarnDeny) -> Self {
    ...
    /// `true`.
    fn get_all_rules(&self) -> Vec<R> {
        self.get_all_rules_for_plugins(None)
    }

    fn get_all_rules_for_plugins(&self, override_plugins: Option<LintPlugins>) -> Vec<R> {
    ...
    pub fn build(
        mut self,
        external_plugin_store: &mut ExternalPluginStore,
    ) -> Result<Config<R>, ConfigBuilderError> {
    ...
    fn resolve_overrides(
        ...
        overrides: OxlintOverrides,
        external_plugin_store: &mut ExternalPluginStore,
    ) -> Result<ResolvedOxlintOverrides<R>, Vec<OverrideRulesError>> {
    ...
    fn warn_correctness(plugins: LintPlugins) -> FxHashMap<R, AllowWarnDeny> {
        R::all_rules()
            .iter()
            .filter(|rule| {
...
impl<R: BuiltinRule> Debug for ConfigStoreBuilder<R> {
