...
#[derive(Debug, Clone)]
pub struct ResolvedLinterState<R: BuiltinRule> {
    // TODO: Arc + Vec -> SyncVec? It would save a pointer dereference.
    pub rules: Arc<[(R, AllowWarnDeny)]>,
...
pub struct ResolvedOxlintOverrides<R: BuiltinRule>(Vec<ResolvedOxlintOverride<R>>);

impl<R: BuiltinRule> ResolvedOxlintOverrides<R> {
    pub(crate) fn new(overrides: Vec<ResolvedOxlintOverride<R>>) -> Self {
        Self(overrides)
    }
    ...
    pub(crate) fn iter(&self) -> std::slice::Iter<'_, ResolvedOxlintOverride<R>> {
        self.0.iter()
    }
}

#[derive(Debug, Clone)]
pub struct ResolvedOxlintOverride<R: BuiltinRule> {
    pub files: GlobSet,
    pub exclude_files: GlobSet,
    pub env: Option<OxlintEnv>,
    pub globals: Option<OxlintGlobals>,
    pub plugins: Option<LintPlugins>,
    pub rules: ResolvedOxlintOverrideRules<R>,
}

#[derive(Debug, Clone)]
pub struct ResolvedOxlintOverrideRules<R: BuiltinRule> {
    pub(crate) builtin_rules: Vec<(R, AllowWarnDeny)>,
    pub(crate) external_rules: Vec<(ExternalRuleId, ExternalOptionsId, AllowWarnDeny)>,
}

#[derive(Debug, Clone)]
pub struct Config<R: BuiltinRule> {
    /// The basic linter state for this configuration.
    /// For files that match no overrides, this lint config will be used.
    pub(crate) base: ResolvedLinterState<R>,
    ...
    pub(crate) base_rules: Vec<(R, AllowWarnDeny)>,
    ...
    pub(crate) overrides: ResolvedOxlintOverrides<R>,
}

impl<R: BuiltinRule> Config<R> {
    fn builtin_rule_plugins(plugins: LintPlugins) -> LintPlugins {
        plugins
    }

    pub fn new(
        rules: Vec<(R, AllowWarnDeny)>,
        mut external_rules: Vec<(ExternalRuleId, ExternalOptionsId, AllowWarnDeny)>,
        categories: OxlintCategories,
        config: LintConfig,
        overrides: ResolvedOxlintOverrides<R>,
    ) -> Self {
        Config {
    ...
    pub fn rules(&self) -> &Arc<[(R, AllowWarnDeny)]> {
        &self.base.rules
    }
    ...
    pub fn apply_overrides(&self, path: &Path) -> ResolvedLinterState<R> {
...
#[derive(Debug, Clone)]
pub struct ConfigStore<R: BuiltinRule> {
    base: Config<R>,
    nested_configs: FxHashMap<PathBuf, Config<R>>,
    external_plugin_store: Arc<ExternalPluginStore>,
}

impl<R: BuiltinRule> ConfigStore<R> {
    pub fn new(
        base_config: Config<R>,
        nested_configs: FxHashMap<PathBuf, Config<R>>,
    ...
    pub(crate) fn get_related_config(&self, path: &Path) -> &Config<R> {
        if self.nested_configs.is_empty() {
            &self.base
    ...
    pub fn resolve(&self, path: &Path) -> ResolvedLinterState<R> {
        Config::apply_overrides(self.get_related_config(path), path)
    }

    fn get_nearest_config(&self, path: &Path) -> Option<&Config<R>> {
