...
use oxc_markdown_parser::{
    ast::{Block, CodeBlockKind, List, ListItem, ListMarker, TaskCheckbox},
    lexical,
};
...
fn write_list_item<'a>(
    ...
    let parent = block::Parent::ListItem { loose, after_checkbox: false };
    ...
    for (i, child) in item.children.iter().enumerate() {
        ...
        let child_parent = if i == 0 && !checkbox.is_empty() {
            block::Parent::ListItem { loose, after_checkbox: true }
        } else {
            parent
        };
...
fn marker_number(item: &ListItem<'_>, f: &MarkdownFormatter<'_, '_>) -> u64 {
    lexical::ordered_number(f.context().slice(item.marker)).map_or(0, u64::from)
}
