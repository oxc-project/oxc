...
use super::{
    MarkdownFormatter, blockquote, code, format_with, inline, join_pieces, line_shape, link, list,
    table, with_depth, write_indented_lines, write_lines, write_verbatim,
};
...
fn double_gap(
    ...
    let remainder_after_definition = || {
        matches!((previous, node),
            (Block::Definition(_), Block::Paragraph(p)) if line_shape::first_line_opens_block(&p.children, f))
    };
