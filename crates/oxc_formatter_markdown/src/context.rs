...
pub struct MarkdownFormatContext<'a> {
    ...
    /// Enclosing nodes whose content never wraps (ATX heading, link text).
    ...
    /// (code spans, HTML, autolinks, math, liquid, wiki links, link labels and titles),
    /// in source order; `true` marks code spans. Wrapping never cuts inside one.
    /// Both tables are refilled by every `inline::collect_phrasing`.
