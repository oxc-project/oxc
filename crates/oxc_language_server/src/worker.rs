...
use tower_lsp_server::{
    UriExt,
    lsp_types::{CodeActionOrCommand, Diagnostic, FileChangeType, FileEvent, Range, Uri},
};
...
impl WorkspaceWorker {
    ...
    async fn revalidate_diagnostics(&self) -> ConcurrentHashMap<String, Vec<DiagnosticReport>> {
        let old_mapper = self.diagnostics_report_map.read().await;
        let diagnostics_map =
            ConcurrentHashMap::with_capacity_and_hasher(old_mapper.len(), FxBuildHasher);
        let linter = self.server_linter.read().await;

        for (uri, _) in &old_mapper.pin() {
