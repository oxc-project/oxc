...
pub enum WorkspaceType {
    ...
    // e.g. the user did not open a folder or the file is outside of the opened folder(s).
    Global,
}
...
impl WorkspaceWorker {
    ...
    pub async fn start_worker(&self, options: serde_json::Value) {
        *self.tools.write().await = self
            ...
            .map(|builder| {
                builder
                    .build_boxed(ToolStartInput { r#type: &self.r#type, options: options.clone() })
            })
            .collect();
    ...
    pub async fn did_change_watched_files(
        ...
        self.handle_tool_changes(file_system, needs_diagnostic_refresh, |tool, builder| {
            tool.handle_watched_file_change(
                builder,
                &file_event.uri,
                ToolStartInput { r#type: &self.r#type, options: options.clone() },
            )
        })
        .await
    }
        ...
        let result = self
            .handle_tool_changes(file_system, needs_diagnostic_refresh, |tool, builder| {
                tool.handle_configuration_change(
                    builder,
                    &old_options,
                    ToolStartInput { r#type: &self.r#type, options: changed_options_json.clone() },
                )
            })
            .await;
