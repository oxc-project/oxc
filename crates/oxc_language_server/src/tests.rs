...
mod test_suite {
    ...
    #[tokio::test]
    async fn test_nested_workspaces_diagnostics() {
        // Test that nested workspaces route diagnostics to the most specific worker
        let init_options = InitializeRequestOptions {
            workspace_folders: Some(vec![
                WorkspaceFolder {
                ...
                WorkspaceFolder {
                    uri: "file:///path/to/workspace/deeper".parse().unwrap(),
                    name: "workspace_deeper".to_string(),
                },
            ]),
            ..Default::default()
        };
        ...
        // Test file in the deeper workspace
        let file_in_deeper = "file:///path/to/workspace/deeper/diagnostics.config";
        let content = "nested workspace";
        server.send_request(did_open(file_in_deeper, content)).await;

        let diagnostic_response = server.recv_notification().await;
        assert_eq!(diagnostic_response.method(), "textDocument/publishDiagnostics");
        let params: PublishDiagnosticsParams =
            serde_json::from_value(diagnostic_response.params().unwrap().clone()).unwrap();
        assert_eq!(params.uri, file_in_deeper.parse().unwrap());
        assert_eq!(params.diagnostics.len(), 1);
        assert_eq!(
            params.diagnostics[0].message,
            format!("Fake diagnostic for content: {content}")
        ...
        // Test file in the parent workspace
        let file_in_parent = "file:///path/to/workspace/diagnostics.config";
        let content2 = "parent workspace";
        server.send_request(did_open(file_in_parent, content2)).await;

        let diagnostic_response = server.recv_notification().await;
        assert_eq!(diagnostic_response.method(), "textDocument/publishDiagnostics");
        let params: PublishDiagnosticsParams =
            serde_json::from_value(diagnostic_response.params().unwrap().clone()).unwrap();
        assert_eq!(params.uri, file_in_parent.parse().unwrap());
        assert_eq!(params.diagnostics.len(), 1);
        assert_eq!(
            params.diagnostics[0].message,
            format!("Fake diagnostic for content: {content2}")
        );

        // Shutdown and collect cleared URIs in any order
        server.send_request(shutdown_request(4)).await;

        let mut cleared_uris = vec![];
        for _ in 0..2 {
            let publish_diagnostics = server.recv_notification().await;
            assert_eq!(publish_diagnostics.method(), "textDocument/publishDiagnostics");
            let params: PublishDiagnosticsParams =
                serde_json::from_value(publish_diagnostics.params().unwrap().clone()).unwrap();
            assert!(params.diagnostics.is_empty());
            cleared_uris.push(params.uri);
        }

        // Check that both URIs were cleared
        assert!(cleared_uris.contains(&file_in_deeper.parse().unwrap()));
        assert!(cleared_uris.contains(&file_in_parent.parse().unwrap()));

        let shutdown_result = server.recv_response().await;
        assert!(shutdown_result.is_ok());
        assert_eq!(shutdown_result.id(), &Id::Number(4));
    }

    #[tokio::test]
    async fn test_similar_named_workspaces() {
        // Test that similar-named workspaces like workspace and workspace-2 route correctly
        let init_options = InitializeRequestOptions {
            workspace_folders: Some(vec![
                WorkspaceFolder {
                    uri: "file:///path/to/workspace".parse().unwrap(),
                ...
                WorkspaceFolder {
                    uri: "file:///path/to/workspace-2".parse().unwrap(),
                    name: "workspace_2".to_string(),
                },
            ]),
            ..Default::default()
        ...
        // Test file in workspace-2
        let file_in_workspace2 = "file:///path/to/workspace-2/diagnostics.config";
        let content = "workspace-2 content";
        server.send_request(did_open(file_in_workspace2, content)).await;

        let diagnostic_response = server.recv_notification().await;
        assert_eq!(diagnostic_response.method(), "textDocument/publishDiagnostics");
        let params: PublishDiagnosticsParams =
            serde_json::from_value(diagnostic_response.params().unwrap().clone()).unwrap();
        assert_eq!(params.uri, file_in_workspace2.parse().unwrap());
        assert_eq!(params.diagnostics.len(), 1);

        // Test file in workspace
        let file_in_workspace = "file:///path/to/workspace/diagnostics.config";
        let content2 = "workspace content";
        server.send_request(did_open(file_in_workspace, content2)).await;

        let diagnostic_response = server.recv_notification().await;
        assert_eq!(diagnostic_response.method(), "textDocument/publishDiagnostics");
        let params: PublishDiagnosticsParams =
            serde_json::from_value(diagnostic_response.params().unwrap().clone()).unwrap();
        assert_eq!(params.uri, file_in_workspace.parse().unwrap());
        assert_eq!(params.diagnostics.len(), 1);
        ...
        // Check that both URIs were cleared
        assert!(cleared_uris.contains(&file_in_workspace2.parse().unwrap()));
        assert!(cleared_uris.contains(&file_in_workspace.parse().unwrap()));
