...
impl LanguageServer for Backend {
    ...
    async fn did_change_configuration(&self, params: DidChangeConfigurationParams) {
            ...
            .is_some_and(|capabilities| capabilities.workspace_configuration)
        {
            ...
            // ToDo: send request, collect them and pass them to the right workers
    ...
    async fn did_change_watched_files(&self, params: DidChangeWatchedFilesParams) {
        ...
        for file_event in &params.changes {
            ...
            let _diagnostics = worker.did_change_watched_files(file_event).await;
            // Todo: Collect diagnostics and publish then as a batch
        }
    }
    ...
    async fn code_action(&self, params: CodeActionParams) -> Result<Option<CodeActionResponse>> {
        ...
        let workers = self.workspace_workers.lock().await;
        let Some(worker) = workers.iter().find(|worker| worker.is_responsible_for_uri(uri)) else {
            return Err(Error::invalid_params("no corresponding workspace folder found"));
        };
