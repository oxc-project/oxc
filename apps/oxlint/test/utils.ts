...
export interface Fixture {
  ...
  options: {
    ...
    args: string[];
    // Additional environment variables to set when running. Default: `{}`.
...
export function getFixtures(): Fixture[] {
  ...
  for (const fileObj of fileObjs) {
    ...
    if (
      ...
      throw new TypeError("`env` property in `options.json` must be an object of string values");
    }
