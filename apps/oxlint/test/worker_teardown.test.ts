...
// Smoke test for `oxc_napi::pin_module_image` (see `crates/oxc_napi/src/module_pin.rs`):
// oxlint's addon runs on napi-rs's Tokio runtime, so unloading it after worker teardown
// risks the same execute-after-unload crash as oxfmt (`0xC0000005` on Windows).
...
// live-environment count really reaches zero and the shared runtime restarts.
