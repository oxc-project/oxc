...
function assertSuggestionsAreCorrect(
  ...
  assert.strictEqual(
    ...
    `Error should have ${expectedSuggestions.length} suggestion(s). Instead found ${actualSuggestions.length} suggestion(s)`,
  );
  ...
  for (let i = 0; i < expectedSuggestions.length; i++) {
    ...
    // Validate output
    assert(Object.hasOwn(expected, "output"), `${prefix}: The "output" property is required.`);
    ...
    assert.strictEqual(
      ...
      `Expected the applied suggestion fix to match the test suggestion output ` +
        `for suggestion at index: ${i}`,
    );

    assert.notStrictEqual(
      expected.output,
      test.code,
      `The output of a suggestion should differ from the original source code ` +
        `for suggestion at index: ${i}`,
    );
  }
}
...
function assertSuggestionMessageIsCorrect(
  ...
  if (Object.hasOwn(expected, "messageId")) {
    ...
    const messageId: string = expected.messageId!;
    if (!Object.hasOwn(messages!, messageId)) {
      const legalMessageIds = `[${Object.keys(messages!)
        .map((key) => `'${key}'`)
        .join(", ")}]`;
      assert.fail(
        `${prefix}: Invalid messageId '${messageId}'. Expected one of ${legalMessageIds}`,
      );
    }
    ...
    assert.strictEqual(
      actual.messageId,
      messageId,
      `${prefix}: messageId should be '${messageId}' but got '${actual.messageId}' instead.`,
    );

    const reportedMessage = actual.message;
    const ruleMessage = messages![messageId];
      ...
      assert.fail(
        `${prefix}: The reported message has ` +
          (unsubstitutedPlaceholders.length > 1
            ...
            : `an unsubstituted placeholder '${unsubstitutedPlaceholders[0]}'`) +
          `. Please provide the missing ${unsubstitutedPlaceholders.length > 1 ? "values" : "value"} ` +
          "via the `data` property on the suggestion.",
      );
    }

    if (Object.hasOwn(expected, "data")) {
      const rehydratedMessage = replacePlaceholders(ruleMessage, expected.data!);
      assert.strictEqual(
        reportedMessage,
        rehydratedMessage,
        `${prefix}: Hydrated test desc "${rehydratedMessage}" does not match received desc "${reportedMessage}"`,
      );
    }
