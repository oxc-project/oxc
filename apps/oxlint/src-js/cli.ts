...
let loadParser: typeof import("./plugins/index.ts").loadParser | null = null;
...
let lintFileWithJsParser: typeof import("./plugins/index.ts").lintFileWithJsParser | null = null;
...
function loadPluginWrapper(
  ...
  if (loadPlugin === null) {
    ...
    return import("./plugins/index.ts").then((mod) => {
      ({ loadPlugin, loadParser, lintFile, lintFileWithJsParser, setupRuleConfigs } = mod);
 ...
 *
 * Lazy-loads plugins code on first call, so that overhead is skipped if user doesn't use custom parsers.
 *
 * @param url - File URL of parser module
 * @returns Parser details or error serialized to JSON string
 */
function loadParserWrapper(url: string): Promise<string> {
  if (loadParser === null) {
...
function lintFileWithJsParserWrapper(
  ...
  // Type generated by NAPI-RS for `lint` is inaccurate.
  // `Option::None` on Rust side becomes `null` on JS side, not `undefined`.
  debugAssertIsNotUndefined(parserOptionsJSON, "`parserOptionsJSON` should not be `undefined`");
  debugAssertIsNotUndefined(workspaceUri, "`workspaceUri` should not be `undefined`");

  // `lintFileWithJsParserWrapper` is never called without `loadParserWrapper` being called first,
  // so `lintFileWithJsParser` must be defined here
  debugAssertIsNonNull(lintFileWithJsParser);
