...
pub async fn lint(
    ...
    lint_impl(args, load_plugin, setup_configs, lint_file, create_workspace, destroy_workspace)
        .await
        .report()
        == ExitCode::SUCCESS
}
...
async fn lint_impl(
    ...
    let external_linter = {
        let (_, _, _, _, _) =
            (load_plugin, setup_configs, lint_file, create_workspace, destroy_workspace);
        None
    };
