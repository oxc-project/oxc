...
mod test {
    ...
    use super::{FormatCommand, Mode, OutputMode};

    #[test]
    fn defaults_to_write_mode() {
        let command = FormatCommand::parse_from(&["src"]).unwrap();

        assert!(matches!(command.mode, Mode::Cli(OutputMode::Write)));
        assert_eq!(command.paths, [PathBuf::from("src")]);
    }

    #[test]
    fn parses_check_mode_and_options() {
        let command = FormatCommand::parse_from(&[
            "--check",
            "--config",
            "oxfmt.json",
            "--ignore-path",
        ...
        assert_eq!(command.config_options.config, Some(PathBuf::from("oxfmt.json")));
        assert_eq!(
            command.ignore_options.ignore_path,
            [PathBuf::from(".prettierignore"), PathBuf::from(".gitignore")]
        );
        assert_eq!(command.runtime_options.threads, Some(2));
        assert_eq!(command.paths, [PathBuf::from("src")]);
    }

    #[test]
    fn rejects_multiple_modes() {
        assert!(FormatCommand::parse_from(&["--write", "--check"]).is_err());
    }

    #[cfg(feature = "napi")]
    #[test]
    fn parses_napi_modes() {
        let migrate = FormatCommand::parse_from(&["--migrate", "PRETTIER"]).unwrap();
        assert!(matches!(migrate.mode, Mode::Migrate(super::MigrateSource::Prettier)));

        let stdin = FormatCommand::parse_from(&["--stdin-filepath", "file.ts"]).unwrap();
        assert!(matches!(
            stdin.mode,
            Mode::Stdin(path) if path == std::path::Path::new("file.ts")
        ));

        assert!(FormatCommand::parse_from(&["--init", "--check"]).is_err());
    }

    #[test]
    fn rejects_parent_paths() {
        let result = FormatCommand::parse_from(&["../src"]);
