...
import { getRealSvelteDefaultRunStatePath as getMetadataDefaultRunStatePath } from "./svelte-real-package-metadata.ts";
...
export type RealSvelteManagedRunStepStatus = "running" | "passed" | "failed";
...
export interface RealSvelteManagedRunStepState {
  ...
  status: RealSvelteManagedRunStepStatus;
...
export function getRealSvelteDefaultRunStatePath(profileName: RealSveltePackageProfileName): string {
  return getMetadataDefaultRunStatePath(profileName);
}
