...
use oxc_linter::{LintPlugins, rules::RULES};
...
fn render_built_in_rules_ts() -> String {
    let mut rules = RULES
        ...
        .map(|rule| {
            (
                public_plugin_name(rule.plugin_name()),
                rule.name().to_string(),
...
fn public_plugin_name(plugin_name: &str) -> String {
    if let Ok(plugin) = LintPlugins::try_from(plugin_name) {
        let public_name: &'static str = plugin.into();
