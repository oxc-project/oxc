...
impl TestCase for ConformanceTestCase {
    ...
    fn test(&self, filtered: bool) -> bool {
        ...
        if result.is_ok() {
            transformed_code =
                Codegen::<false>::new(input.len(), CodegenOptions::default(), None).build(program);
        } else {
            actual_errors =
        ...
        let output = output_path.and_then(|path| fs::read_to_string(path).ok()).map_or_else(
            ...
            |output| {
                ...
                Codegen::<false>::new(output.len(), CodegenOptions::default(), None).build(&program)
            },
        );
...
impl ExecTestCase {
    ...
    fn write_to_test_files(&self, content: &str) -> PathBuf {
        ...
        let result = Codegen::<false>::new(source_text.len(), CodegenOptions::default(), None)
            .build(&transformed_program);
