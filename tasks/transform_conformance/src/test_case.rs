...
pub trait TestCase {
    ...
    fn transform(&self, path: &Path) -> Result<String, Vec<Error>> {
        ...
        result.map(|()| {
            Codegen::<false>::new(source_text.len(), CodegenOptions::default()).build(transformed_program)
        })
    }
}
...
impl TestCase for ConformanceTestCase {
    ...
    fn test(&self, filtered: bool) -> bool {
        ...
        if result.is_ok() {
            transformed_code = Codegen::<false>::new(input.len(), CodegenOptions::default()).build(program);
        } else {
            actual_errors =
