...
impl Case for EsbuildTest262Case {
    ...
    fn run(&mut self) {
        ...
        let mut driver = Driver { codegen: true, remove_whitespace: true, ..Driver::default() };
        ...
        let esbuild =
            Command::new("esbuild").arg("--minify-whitespace=true").arg(&path).output().unwrap();
        ...
        self.base.set_result(TestResult::GenericError("whitespace", format!("{diff}")));
    }
}
