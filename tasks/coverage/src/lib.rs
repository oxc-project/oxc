...
impl AppArgs {
    ...
    pub fn run_parser(&self) {
        ...
        let test262_paths = self.get_test262_paths(|path| {
            Test262Suite::<Test262Case>::new().skip_test_path(path)
        });
        let babel_paths = self.get_babel_paths(|path| {
            BabelSuite::<BabelCase>::new().skip_test_path(path)
        });
        let typescript_paths = self.get_typescript_paths(|path| {
            TypeScriptSuite::<TypeScriptCase>::new().skip_test_path(path)
        });
        let misc_paths = self.get_misc_paths(|path| {
            MiscSuite::<MiscCase>::new().skip_test_path(path)
        });

        // Run suites with cached paths
        self.run_suite_with_paths(Test262Suite::<Test262Case>::new(), test262_paths, "parser_test262");
        self.run_suite_with_paths(BabelSuite::<BabelCase>::new(), babel_paths, "parser_babel");
        self.run_suite_with_paths(TypeScriptSuite::<TypeScriptCase>::new(), typescript_paths, "parser_typescript");
    ...
    pub fn run_semantic(&self) {
        ...
        let babel_paths = self.get_babel_paths(|path| {
            BabelSuite::<SemanticBabelCase>::new().skip_test_path(path)
        });
        let typescript_paths = self.get_typescript_paths(|path| {
            TypeScriptSuite::<SemanticTypeScriptCase>::new().skip_test_path(path)
        });
        let misc_paths = self.get_misc_paths(|path| {
            MiscSuite::<SemanticMiscCase>::new().skip_test_path(path)
        });

        self.run_suite_with_paths(Test262Suite::<SemanticTest262Case>::new(), test262_paths, "semantic_test262");
        self.run_suite_with_paths(BabelSuite::<SemanticBabelCase>::new(), babel_paths, "semantic_babel");
        self.run_suite_with_paths(TypeScriptSuite::<SemanticTypeScriptCase>::new(), typescript_paths, "semantic_typescript");
        self.run_suite_with_paths(MiscSuite::<SemanticMiscCase>::new(), misc_paths, "semantic_misc");
    }
    ...
    pub fn run_codegen(&self) {
        ...
        let babel_paths = self.get_babel_paths(|path| {
            BabelSuite::<CodegenBabelCase>::new().skip_test_path(path)
        });
        let typescript_paths = self.get_typescript_paths(|path| {
            TypeScriptSuite::<CodegenTypeScriptCase>::new().skip_test_path(path)
        });
        let misc_paths = self.get_misc_paths(|path| {
            MiscSuite::<CodegenMiscCase>::new().skip_test_path(path)
        });

        self.run_suite_with_paths(Test262Suite::<CodegenTest262Case>::new(), test262_paths, "codegen_test262");
        self.run_suite_with_paths(BabelSuite::<CodegenBabelCase>::new(), babel_paths, "codegen_babel");
        self.run_suite_with_paths(TypeScriptSuite::<CodegenTypeScriptCase>::new(), typescript_paths, "codegen_typescript");
    ...
    pub fn run_formatter(&self) {
        ...
        let babel_paths = self.get_babel_paths(|path| {
            BabelSuite::<FormatterBabelCase>::new().skip_test_path(path)
        });
        let typescript_paths = self.get_typescript_paths(|path| {
            TypeScriptSuite::<FormatterTypeScriptCase>::new().skip_test_path(path)
        });
        let misc_paths = self.get_misc_paths(|path| {
            MiscSuite::<FormatterMiscCase>::new().skip_test_path(path)
        });

        self.run_suite_with_paths(Test262Suite::<FormatterTest262Case>::new(), test262_paths, "formatter_test262");
        self.run_suite_with_paths(BabelSuite::<FormatterBabelCase>::new(), babel_paths, "formatter_babel");
        self.run_suite_with_paths(TypeScriptSuite::<FormatterTypeScriptCase>::new(), typescript_paths, "formatter_typescript");
        self.run_suite_with_paths(MiscSuite::<FormatterMiscCase>::new(), misc_paths, "formatter_misc");
    }
    ...
    pub fn run_transformer(&self) {
        ...
        let babel_paths = self.get_babel_paths(|path| {
            BabelSuite::<TransformerBabelCase>::new().skip_test_path(path)
        });
        let typescript_paths = self.get_typescript_paths(|path| {
            TypeScriptSuite::<TransformerTypeScriptCase>::new().skip_test_path(path)
        });
        let misc_paths = self.get_misc_paths(|path| {
            MiscSuite::<TransformerMiscCase>::new().skip_test_path(path)
        });

        self.run_suite_with_paths(Test262Suite::<TransformerTest262Case>::new(), test262_paths, "transformer_test262");
        self.run_suite_with_paths(BabelSuite::<TransformerBabelCase>::new(), babel_paths, "transformer_babel");
        self.run_suite_with_paths(TypeScriptSuite::<TransformerTypeScriptCase>::new(), typescript_paths, "transformer_typescript");
        self.run_suite_with_paths(MiscSuite::<TransformerMiscCase>::new(), misc_paths, "transformer_misc");
    }
    ...
    pub fn run_estree(&self) {
        ...
        self.run_suite_with_paths(Test262Suite::<EstreeTest262Case>::new(), test262_paths, "estree_test262");
        AcornJsxSuite::<EstreeJsxCase>::new().run("estree_acorn_jsx", self);
        self.run_suite_with_paths(TypeScriptSuite::<EstreeTypescriptCase>::new(), typescript_paths, "estree_typescript");
    }
    ...
    pub fn run_minifier(&self) {
        ...
        let babel_paths = self.get_babel_paths(|path| {
            BabelSuite::<MinifierBabelCase>::new().skip_test_path(path)
        });

        self.run_suite_with_paths(Test262Suite::<MinifierTest262Case>::new(), test262_paths, "minifier_test262");
        self.run_suite_with_paths(BabelSuite::<MinifierBabelCase>::new(), babel_paths, "minifier_babel");
