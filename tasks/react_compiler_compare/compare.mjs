...
const bprcVersion = (() => {
  try {
    return JSON.parse(
      readFileSync(
        new URL("./node_modules/babel-plugin-react-compiler/package.json", import.meta.url),
      ),
    ).version;
  } catch {
    return "unknown";
...
const oxcTransformPath =
  process.env.OXC_TRANSFORM || new URL("../../napi/transform/index.js", import.meta.url).href;
const { reactCompilerSync } = await import(oxcTransformPath);

const REPOS_DIR =
  process.env.REPOS || fileURLToPath(new URL("../../../oxc-ecosystem-ci/repos", import.meta.url));
...
function stripFormatting(node) {
  ...
  if (Array.isArray(node)) {
    for (const c of node) stripFormatting(c);
    return;
  }
  delete node.extra;
  delete node.start;
  ...
  for (const k in node) {
    const v = node[k];
    if (v && typeof v === "object") stripFormatting(v);
  }
}
...
function canon(code, plugins) {
  ...
  stripFormatting(ast);
  return generate(ast, { compact: false, concise: false, retainLines: false, comments: false })
    .code;
}

const memoized = (code) => code.includes("react/compiler-runtime") || /\b_c\(\d/.test(code);
const cacheCount = (code) => {
...
let files = execSync(findCmd, { maxBuffer: 1 << 30 })
  .toString()
  .trim()
  .split("\n")
  .filter(Boolean);
if (reposFilter)
  files = files.filter((f) => reposFilter.has(f.slice(REPOS_DIR.length + 1).split("/")[0]));
files.sort();
...
console.log(
  `candidates=${total} sampled=${files.length} cap=${CAP === Infinity ? "all" : CAP} bprc=${bprcVersion}`,
);
...
for (const file of files) {
  ...
  if (n % 500 === 0)
    console.log(
      `  ${n}/${files.length}  memoMatch=${stats.memoMatch} memoMismatch=${stats.memoMismatch} babelErr=${stats.babelError}`,
    );
  ...
  let babelCode;
  try {
    babelCode = babelCompile(src, file, plugins);
  } catch {
    stats.babelError++;
    continue;
  }
  ...
  let oxc;
  try {
    oxc = reactCompilerSync(file, src);
  } catch {
    stats.oxcError++;
    continue;
  }
  ...
  let cb, co;
  try {
    cb = canon(babelCode, plugins);
    co = canon(oxc.code, plugins);
  } catch {
    stats.canonError++;
    continue;
  }
  ...
  if (equal) {
    ...
    if (bMemo || oMemo) {
      ...
      bucketSamples[cause] ??= [];
      if (bucketSamples[cause].length < 4)
        bucketSamples[cause].push({
          file: file.slice(REPOS_DIR.length + 1),
          diff: shortDiff(cb, co),
        });
    } else {
      stats.noopMismatch++;
    }
...
const lines = [];
lines.push(
  `# react-compiler: oxc \`reactCompilerSync\` vs babel-plugin-react-compiler@${bprcVersion}`,
);
lines.push("");
...
for (const [cause, count] of Object.entries(buckets).sort((a, b) => b[1] - a[1]))
