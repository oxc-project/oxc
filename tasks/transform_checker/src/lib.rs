...
impl PostTransformChecker<'_, '_> {
    fn check_scopes(&mut self) {
        for scope_ids in self.scope_ids_map.pairs() {
            ...
            // Check flags match
            let flags = self.get_pair(scope_ids, |scoping, scope_id| scoping.scope_flags(scope_id));
            ...
            // Check parents match
            let parent_ids =
                self.get_pair(scope_ids, |scoping, scope_id| scoping.get_scope_parent_id(scope_id));
    ...
    fn check_symbols(&mut self) {
        for symbol_ids in self.symbol_ids_map.pairs() {
            ...
            // Check flags match
            let flags =
                self.get_pair(symbol_ids, |scoping, symbol_id| scoping.symbol_flags(symbol_id));
            if flags.is_mismatch() {
            ...
            // Check spans match
            let spans =
                self.get_pair(symbol_ids, |scoping, symbol_id| scoping.symbol_span(symbol_id));
            ...
            let scope_ids = self
                .get_pair(symbol_ids, |scoping, symbol_id| scoping.get_symbol_scope_id(symbol_id));
