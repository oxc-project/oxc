...
//! the node can be built with `BindingRestElement::new(span, argument, accessor)`, where `accessor` is anything
...
fn generate_builder_methods_for_struct_impl(
    ...
    let new_method = quote! {
        ...
        #params_docs
        #[inline]
        pub fn #new_fn_name #generic_params (#fn_params, accessor: &A) -> Self #where_clause {
    ...
    quote! {
        ...
        #params_docs
        #[inline]
        pub fn #boxed_fn_name #generic_params (#fn_params, accessor: &A) -> Box<'a, Self> #where_clause {
            Box::new_in(Self::#new_fn_name(#(#args),*, accessor), accessor.builder().allocator())
        }
    }
}
...
///
/// Also generates the generic params and `where` clause for the methods. These always include the
/// accessor generic `A` (and `'a` when `has_lifetime` is `false`), plus any `Into`/`IntoIn`
/// generics derived from the struct's fields.
///
/// ```
/// //        ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓ generic params
/// pub fn new<A: GetAstBuilder<'a>, T1>(span: Span, function: T1, accessor: &A) -> Self
...
fn get_struct_params<'s>(
    ...
    let generic_params = quote!( <#lifetime A: GetAstBuilder<'a> #(, #generic_idents)*> );
...
///
/// The generated field values reference a local `builder` binding (`let builder = accessor.builder();`)
/// for the allocator and node ID.
///
/// ```
/// //         ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓ function params
/// pub fn new(span: Span, bar: Bar<'a>, accessor: &A) -> Self {
///     let builder = accessor.builder();
...
fn generate_builder_method_for_enum_variant(
    ...
    let method_name = format!("new_{}", snake_name(variant.name()));
...
fn generate_builder_method_for_enum_variant_impl(
    ...
    quote! {
        ...
        #params_docs
        #[inline]
        pub fn #fn_name #generic_params (#(#fn_params),*, accessor: &A) -> Self #where_clause {
            Self::#variant_ident(#struct_ident::#inner_fn_name(#(#args),*, accessor))
        }
    }
}
...
///
/// Special case for names beginning with `V8` - produces `v8_...` not `v_8_...`.
fn snake_name(name: &str) -> String {
    if name.starts_with("V8") {
        format!("v8_{}", (&name[2..]).to_case(Case::Snake))
    } else {
