...
1. `AstBuilderGenerator` (in `tasks/ast_tools`) emits two data files, regenerated with `just ast`:
   ...
   - [`generated/shorten_mappings.json`](generated/shorten_mappings.json) holds the structural data
     the shortening rules need: which structs are boxable, each enum variant's inner builder, the
     enum inheritance graph, and which methods have an `ArenaVec` param.
...
2. [`custom_mappings.json`](custom_mappings.json) adds, by hand, the methods that are written by hand
   rather than emitted by codegen (e.g. `void_0` -> `Expression::new_void_0`). Its `vecArgMethods`
   list does the same for the `ArenaVec`-param data - note those entries are _new_ builder paths,
   whereas `mappings` and `zeroArg` are keyed by old `AstBuilder` method name.
...
4. **`AST_TYPE_PREFIX`**: The module path AST types are referred to through. All rules use bare type
   ...
   `use oxc_ast::ast as oxc;` -> `oxc::Expression`), set this to that prefix _including the trailing
   `::`_, e.g. `"oxc::"`. It is prepended to every AST type name, but not to the arena primitives
...
They are purely structural - they never mention `<accessor>.ast` - and come in four kinds:
...
// vec-arg: an `ArenaVec` argument  ->  the equivalent array literal
Statement::new_block_statement(span, ArenaVec::new_in(b), b)              // -> (span, [], b)
Statement::new_block_statement(span, ArenaVec::from_value_in(stmt, b), b) // -> (span, [stmt], b)
Statement::new_block_statement(span, ArenaVec::from_array_in([x, y], b), b) // -> (span, [x, y], b)
```
...
`vec-arg` is valid because a builder method's `ArenaVec` params are generic over
`IntoIn<'a, ArenaVec<'a, T>>`, which arrays implement. It applies only where the `ArenaVec` is a
_direct_ argument of a method known to take one, so `Some(ArenaVec::new_in(b))` is left alone -
`Option<ArenaVec<_>>` params get no such generic and still need a real `ArenaVec`.
`ArenaVec::with_capacity_in` and `ArenaVec::from_iter_in` are left alone too, having no array form.
